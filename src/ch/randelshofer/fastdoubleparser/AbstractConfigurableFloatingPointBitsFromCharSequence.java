...
abstract class AbstractConfigurableFloatingPointBitsFromCharSequence extends AbstractFloatValueParser {
   ...
   public final long parseFloatingPointLiteral(CharSequence str, int offset, int length) {
      ...
      if (index < endIndex && !isSignificandSigned) {
         boolean isNegative2 = this.minusSignChar.containsKey(ch);
   ...
   private long parseNaNOrInfinity(CharSequence str, int index, int endIndex, boolean isNegative, boolean isSignificandSigned) {
      ...
      if (nanMatch > 0) {
         ...
         if (infinityMatch > 0) {
            ...
            if (index < endIndex && !isSignificandSigned) {
               char ch = str.charAt(index);
               isNegative = this.minusSignChar.containsKey(ch);
               if (isNegative || this.plusSignChar.containsKey(ch)) {
                  index++;
               }
            }
   ...
   abstract long valueOfFloatLiteral(
      CharSequence var1, int var2, int var3, int var4, int var5, boolean var6, long var7, int var9, boolean var10, int var11, int var12, int var13, int var14
   );
   ...
   protected double slowPathToDouble(
      ...
      return SlowDoubleConversionPath.toDouble(
         str, this.digitSet, integerStartIndex, integerEndIndex, fractionStartIndex, fractionEndIndex, isSignificandNegative, exponentValue
      );
   }
}
