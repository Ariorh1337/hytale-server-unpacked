...
final class JavaBigDecimalFromByteArray extends AbstractBigDecimalParser {
   ...
   public BigDecimal parseBigDecimalString(byte[] str, int offset, int length) {
      try {
         ...
         return digitCount < 19
            ? new BigDecimal(isNegative ? -significand : significand).scaleByPowerOfTen((int)exponent)
            : this.valueOfBigDecimalString(str, integerPartIndex, decimalPointIndex, decimalPointIndex + 1, exponentIndicatorIndex, isNegative, (int)exponent);
   ...
   BigDecimal parseBigDecimalStringWithManyDigits(byte[] str, int offset, int length) {
      ...
      return this.valueOfBigDecimalString(
         str, nonZeroIntegerPartIndex, decimalPointIndex, nonZeroFractionalPartIndex, exponentIndicatorIndex, isNegative, (int)exponent
      );
   }

   BigDecimal valueOfBigDecimalString(
      byte[] str, int integerPartIndex, int decimalPointIndex, int nonZeroFractionalPartIndex, int exponentIndicatorIndex, boolean isNegative, int exponent
   ) {
