...
public class Arrays {
   ...
   public static void ensureOffsetLength(int arrayLength, int offset, int length) {
      ...
      if (length > arrayLength - offset) {
         throw new ArrayIndexOutOfBoundsException("Last index (" + ((long)offset + length) + ") is greater than array length (" + arrayLength + ")");
      }
   }
               ...
               for (; b > c || (comparison = this.comp.compare(b, m)) > 0; this.swapper.swap(b++, c--)) {
                  ...
                  if (b > c) {
                     ...
                     if (s > 1 && comparison > 1) {
                        ...
                        invokeAll(new Arrays.ForkJoinGenericQuickSort(this.from, this.from + s, this.comp, this.swapper));
                     } else {
                        invokeAll(new Arrays.ForkJoinGenericQuickSort(this.to - comparison, this.to, this.comp, this.swapper));
                     }
