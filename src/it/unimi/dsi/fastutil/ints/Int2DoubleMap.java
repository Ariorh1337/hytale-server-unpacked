...
public interface Int2DoubleMap extends Int2DoubleFunction, Map<Integer, Double> {
   ...
   default void forEach(BiConsumer<? super Integer, ? super Double> consumer) {
      ObjectSet<Int2DoubleMap.Entry> entrySet = this.int2DoubleEntrySet();
      Consumer<Int2DoubleMap.Entry> wrappingConsumer = entry -> consumer.accept(entry.getIntKey(), entry.getDoubleValue());
      if (entrySet instanceof Int2DoubleMap.FastEntrySet) {
         ((Int2DoubleMap.FastEntrySet)entrySet).fastForEach(wrappingConsumer);
      } else {
   ...
   default double mergeDouble(int key, double value, it.unimi.dsi.fastutil.doubles.DoubleBinaryOperator remappingFunction) {
   ...
   interface Entry extends java.util.Map.Entry<Integer, Double> {
      int getIntKey();
