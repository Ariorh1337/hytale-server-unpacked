...
public interface Int2ByteMap extends Int2ByteFunction, Map<Integer, Byte> {
   ...
   default void forEach(BiConsumer<? super Integer, ? super Byte> consumer) {
      ObjectSet<Int2ByteMap.Entry> entrySet = this.int2ByteEntrySet();
      Consumer<Int2ByteMap.Entry> wrappingConsumer = entry -> consumer.accept(entry.getIntKey(), entry.getByteValue());
      if (entrySet instanceof Int2ByteMap.FastEntrySet) {
         ((Int2ByteMap.FastEntrySet)entrySet).fastForEach(wrappingConsumer);
      } else {
   ...
   default byte computeIfAbsent(int key, java.util.function.IntUnaryOperator mappingFunction) {
   ...
   default byte compute(int key, BiFunction<? super Integer, ? super Byte, ? extends Byte> remappingFunction) {
      ...
      Byte newValue = remappingFunction.apply(key, contained ? oldValue : null);
      if (newValue == null) {
         if (contained) {
            this.remove(key);
         }
   ...
   default byte merge(int key, byte value, BiFunction<? super Byte, ? super Byte, ? extends Byte> remappingFunction) {
      ...
      if (oldValue == drv && !this.containsKey(key)) {
         ...
         Byte mergedValue = remappingFunction.apply(oldValue, value);
         if (mergedValue == null) {
            this.remove(key);
            return drv;
         }
   ...
   default byte mergeByte(int key, byte value, ByteBinaryOperator remappingFunction) {
      ...
      byte oldValue = this.get(key);
      byte drv = this.defaultReturnValue();
      byte newValue = oldValue == drv && !this.containsKey(key) ? value : remappingFunction.apply(oldValue, value);
      this.put(key, newValue);
      return newValue;
   }

   default byte mergeByte(int key, byte value, java.util.function.IntBinaryOperator remappingFunction) {
      return this.mergeByte(
         key,
         value,
         remappingFunction instanceof ByteBinaryOperator
            ? (ByteBinaryOperator)remappingFunction
            : (x, y) -> SafeMath.safeIntToByte(remappingFunction.applyAsInt(x, y))
      );
   }
   ...
   interface Entry extends java.util.Map.Entry<Integer, Byte> {
      int getIntKey();
