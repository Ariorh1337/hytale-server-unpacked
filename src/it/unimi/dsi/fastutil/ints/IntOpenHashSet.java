   ...
   private final class SetSpliterator implements IntSpliterator {
      ...
      public long estimateSize() {
         return !this.hasSplit
            ? IntOpenHashSet.this.size - this.c
            : Math.min(
               IntOpenHashSet.this.size - this.c,
               (long)((double)IntOpenHashSet.this.realSize() / IntOpenHashSet.this.n * (this.max - this.pos)) + (this.mustReturnNull ? 1 : 0)
            );
      }
      ...
      public IntOpenHashSet.SetSpliterator trySplit() {
         ...
         int retPos = this.pos;
         int retMax = myNewPos;
         IntOpenHashSet.SetSpliterator split = IntOpenHashSet.this.new SetSpliterator(retPos, retMax, this.mustReturnNull, true);
         this.pos = myNewPos;
         this.mustReturnNull = false;
         this.hasSplit = true;
         return split;
