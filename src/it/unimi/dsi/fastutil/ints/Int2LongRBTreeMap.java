...
public class Int2LongRBTreeMap extends AbstractInt2LongSortedMap implements Serializable, Cloneable {
   ...
   final Int2LongRBTreeMap.Entry findKey(int k) {
      Int2LongRBTreeMap.Entry e = this.tree;
   ...
   final Int2LongRBTreeMap.Entry locateKey(int k) {
      Int2LongRBTreeMap.Entry e = this.tree;
      Int2LongRBTreeMap.Entry last = this.tree;
   ...
   public long addTo(int k, long incr) {
      Int2LongRBTreeMap.Entry e = this.add(k);
   ...
   private Int2LongRBTreeMap.Entry add(int k) {
      this.modified = false;
      int maxDepth = 0;
      Int2LongRBTreeMap.Entry e;
      if (this.tree == null) {
         this.count++;
         e = this.tree = this.lastEntry = this.firstEntry = new Int2LongRBTreeMap.Entry(k, this.defRetValue);
      } else {
         Int2LongRBTreeMap.Entry p = this.tree;
         int i = 0;
         ...
         while (true) {
            ...
            if (this.dirPath[i++] = cmp > 0) {
               ...
               this.count++;
               e = new Int2LongRBTreeMap.Entry(k, this.defRetValue);
               if (p.right == null) {
                  this.lastEntry = e;
               }
            ...
            while (true) {
               ...
               if (!this.dirPath[i - 1]) {
                  Int2LongRBTreeMap.Entry y = this.nodePath[i - 1].right;
                  if (this.nodePath[i - 1].succ() || y.black()) {
                     if (!this.dirPath[i]) {
                        y = this.nodePath[i];
                     } else {
                        Int2LongRBTreeMap.Entry x = this.nodePath[i];
                        y = x.right;
                        x.right = y.left;
                        y.left = x;
                     ...
                     Int2LongRBTreeMap.Entry x = this.nodePath[i - 1];
                     x.black(false);
                     y.black(true);
                     x.left = y.right;
                     y.right = x;
                  ...
                  Int2LongRBTreeMap.Entry y = this.nodePath[i - 1].left;
   ...
   public long remove(int k) {
      ...
      if (p.succ()) {
         ...
         Int2LongRBTreeMap.Entry r = p.right;
         if (r.pred()) {
            r.left = p.left;
            r.pred(p.pred());
            if (!r.pred()) {
            ...
            while (true) {
               ...
               this.nodePath[i++] = r;
               Int2LongRBTreeMap.Entry s = r.left;
               if (s.pred()) {
                  this.dirPath[j] = true;
                  this.nodePath[j] = s;
                  if (s.succ()) {
      ...
      if (p.black()) {
         for (; i > 0; i--) {
            if (this.dirPath[i - 1] && !this.nodePath[i - 1].succ() || !this.dirPath[i - 1] && !this.nodePath[i - 1].pred()) {
               Int2LongRBTreeMap.Entry x = this.dirPath[i - 1] ? this.nodePath[i - 1].right : this.nodePath[i - 1].left;
               if (!x.black()) {
                  x.black(true);
                  break;
               }
            ...
            if (!this.dirPath[i - 1]) {
               Int2LongRBTreeMap.Entry w = this.nodePath[i - 1].right;
               if (!w.black()) {
                  w.black(true);
                  this.nodePath[i - 1].black(false);
               ...
               if (!w.pred() && !w.left.black() || !w.succ() && !w.right.black()) {
                  if (w.succ() || w.right.black()) {
                     Int2LongRBTreeMap.Entry y = w.left;
                     y.black(true);
                     w.black(false);
                     w.left = y.right;
                     y.right = w;
               ...
               Int2LongRBTreeMap.Entry w = this.nodePath[i - 1].left;
               if (!w.black()) {
                  w.black(true);
                  this.nodePath[i - 1].black(false);
               ...
               if (!w.pred() && !w.left.black() || !w.succ() && !w.right.black()) {
                  if (w.pred() || w.left.black()) {
                     Int2LongRBTreeMap.Entry y = w.right;
                     y.black(true);
                     w.black(false);
                     w.right = y.left;
                     y.left = w;
   ...
   public boolean containsValue(long v) {
      Int2LongRBTreeMap.ValueIterator i = new Int2LongRBTreeMap.ValueIterator();
      int j = this.count;
   ...
   public long get(int k) {
      Int2LongRBTreeMap.Entry e = this.findKey(k);
   ...
   public ObjectSortedSet<Int2LongMap.Entry> int2LongEntrySet() {
      if (this.entries == null) {
         this.entries = new AbstractObjectSortedSet<Int2LongMap.Entry>() {
            final Comparator<? super Int2LongMap.Entry> comparator = Int2LongRBTreeMap.this.actualComparator == null
               ? (x, y) -> Integer.compare(x.getIntKey(), y.getIntKey())
               : (x, y) -> Int2LongRBTreeMap.this.actualComparator.compare(x.getIntKey(), y.getIntKey());
            ...
            public ObjectBidirectionalIterator<Int2LongMap.Entry> iterator(Int2LongMap.Entry from) {
               return Int2LongRBTreeMap.this.new EntryIterator(from.getIntKey());
            }
            ...
            public boolean contains(Object o) {
               if (o != null && o instanceof java.util.Map.Entry) {
                  ...
                  if (e.getKey() == null) {
                     ...
                     Int2LongRBTreeMap.Entry f = Int2LongRBTreeMap.this.findKey((Integer)e.getKey());
                     return e.equals(f);
                  } else {
                     return false;
                  }
   ...
   public Int2LongSortedMap headMap(int to) {
      return new Int2LongRBTreeMap.Submap(0, true, to, false);
   }

   @Override
   public Int2LongSortedMap tailMap(int from) {
      return new Int2LongRBTreeMap.Submap(from, false, 0, true);
   }

   @Override
   public Int2LongSortedMap subMap(int from, int to) {
      return new Int2LongRBTreeMap.Submap(from, false, to, false);
   }
   ...
   public Int2LongRBTreeMap clone() {
      ...
      Int2LongRBTreeMap.Entry rp = new Int2LongRBTreeMap.Entry();
      Int2LongRBTreeMap.Entry rq = new Int2LongRBTreeMap.Entry();
      Int2LongRBTreeMap.Entry p = rp;
      rp.left(this.tree);
      Int2LongRBTreeMap.Entry q = rq;
      rq.pred(null);

      while (true) {
         if (!p.pred()) {
            Int2LongRBTreeMap.Entry e = p.left.clone();
            e.pred(q.left);
            e.succ(q);
            q.left(e);
            p = p.left;
         ...
         if (!p.succ()) {
            Int2LongRBTreeMap.Entry e = p.right.clone();
            e.succ(q.right);
            e.pred(q);
            q.right(e);
         }
   ...
   private void writeObject(ObjectOutputStream s) throws IOException {
      int n = this.count;
      Int2LongRBTreeMap.EntryIterator i = new Int2LongRBTreeMap.EntryIterator();
      s.defaultWriteObject();

      while (n-- != 0) {
         Int2LongRBTreeMap.Entry e = i.nextEntry();
         s.writeInt(e.key);
         s.writeLong(e.value);
      }
   }

   private Int2LongRBTreeMap.Entry readTree(ObjectInputStream s, int n, Int2LongRBTreeMap.Entry pred, Int2LongRBTreeMap.Entry succ) throws IOException, ClassNotFoundException {
      if (n == 1) {
         Int2LongRBTreeMap.Entry top = new Int2LongRBTreeMap.Entry(s.readInt(), s.readLong());
         top.pred(pred);
         top.succ(succ);
         top.black(true);
         return top;
      ...
      if (n == 2) {
         Int2LongRBTreeMap.Entry top = new Int2LongRBTreeMap.Entry(s.readInt(), s.readLong());
         top.black(true);
         top.right(new Int2LongRBTreeMap.Entry(s.readInt(), s.readLong()));
         top.right.pred(top);
         top.pred(pred);
         top.right.succ(succ);
         return top;
      ...
      Int2LongRBTreeMap.Entry top = new Int2LongRBTreeMap.Entry();
   ...
   private static final class Entry extends AbstractInt2LongMap.BasicEntry implements Cloneable {
      ...
      void pred(Int2LongRBTreeMap.Entry pred) {
         this.info |= 1073741824;
         this.left = pred;
      }

      void succ(Int2LongRBTreeMap.Entry succ) {
         this.info |= Integer.MIN_VALUE;
         this.right = succ;
      }

      void left(Int2LongRBTreeMap.Entry left) {
         this.info &= -1073741825;
         this.left = left;
      }

      void right(Int2LongRBTreeMap.Entry right) {
         this.info &= Integer.MAX_VALUE;
         this.right = right;
      }
      ...
      Int2LongRBTreeMap.Entry next() {
         Int2LongRBTreeMap.Entry next = this.right;
      ...
      Int2LongRBTreeMap.Entry prev() {
         Int2LongRBTreeMap.Entry prev = this.left;
      ...
      public Int2LongRBTreeMap.Entry clone() {
         Int2LongRBTreeMap.Entry c;
         try {
            c = (Int2LongRBTreeMap.Entry)super.clone();
         } catch (CloneNotSupportedException cantHappen) {
            throw new InternalError();
         }
   ...
   private class KeySet extends AbstractInt2LongSortedMap.KeySet {
      ...
      public IntBidirectionalIterator iterator(int from) {
         return Int2LongRBTreeMap.this.new KeyIterator(from);
      }
   }
   ...
   private final class Submap extends AbstractInt2LongSortedMap implements Serializable {
      ...
      @Override
      public void clear() {
         Int2LongRBTreeMap.Submap.SubmapIterator i = new Int2LongRBTreeMap.Submap.SubmapIterator();
      ...
      public Int2LongSortedMap headMap(int to) {
         if (this.top) {
            return Int2LongRBTreeMap.this.new Submap(this.from, this.bottom, to, false);
         } else {
            return Int2LongRBTreeMap.this.compare(to, this.to) < 0 ? Int2LongRBTreeMap.this.new Submap(this.from, this.bottom, to, false) : this;
         }
      }
      ...
      public Int2LongSortedMap tailMap(int from) {
         if (this.bottom) {
            return Int2LongRBTreeMap.this.new Submap(from, false, this.to, this.top);
         } else {
            return Int2LongRBTreeMap.this.compare(from, this.from) > 0 ? Int2LongRBTreeMap.this.new Submap(from, false, this.to, this.top) : this;
         }
      }
      ...
      public Int2LongSortedMap subMap(int from, int to) {
         if (this.top && this.bottom) {
            return Int2LongRBTreeMap.this.new Submap(from, false, to, false);
         }
         ...
         return !this.top && !this.bottom && from == this.from && to == this.to ? this : Int2LongRBTreeMap.this.new Submap(from, false, to, false);
      }
      ...
      public Int2LongRBTreeMap.Entry firstEntry() {
         ...
         return e != null && (this.top || Int2LongRBTreeMap.this.compare(e.key, this.to) < 0) ? e : null;
      }
      ...
      public Int2LongRBTreeMap.Entry lastEntry() {
         ...
         return e != null && (this.bottom || Int2LongRBTreeMap.this.compare(e.key, this.from) >= 0) ? e : null;
      }

      @Override
      public int firstIntKey() {
         Int2LongRBTreeMap.Entry e = this.firstEntry();
         if (e == null) {
            throw new NoSuchElementException();
         } else {
            return e.key;
      ...
      @Override
      public int lastIntKey() {
         Int2LongRBTreeMap.Entry e = this.lastEntry();
         if (e == null) {
            throw new NoSuchElementException();
         } else {
            return e.key;
      ...
      private class SubmapIterator extends Int2LongRBTreeMap.TreeIterator {
         ...
         SubmapIterator(int k) {
            this();
            if (this.next != null) {
               if (!Submap.this.bottom && Int2LongRBTreeMap.this.compare(k, this.next.key) < 0) {
                  this.prev = null;
               } else if (!Submap.this.top && Int2LongRBTreeMap.this.compare(k, (this.prev = Submap.this.lastEntry()).key) >= 0) {
                  this.next = null;
               } else {
   ...
   private class TreeIterator {
      ...
      TreeIterator(int k) {
         if ((this.next = Int2LongRBTreeMap.this.locateKey(k)) != null) {
            if (Int2LongRBTreeMap.this.compare(this.next.key, k) <= 0) {
               this.prev = this.next;
               this.next = this.next.next();
            } else {
