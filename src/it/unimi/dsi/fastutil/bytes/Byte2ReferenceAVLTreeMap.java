...
public class Byte2ReferenceAVLTreeMap<V> extends AbstractByte2ReferenceSortedMap<V> implements Serializable, Cloneable {
   ...
   private Byte2ReferenceAVLTreeMap.Entry<V> parent(Byte2ReferenceAVLTreeMap.Entry<V> e) {
      ...
      Byte2ReferenceAVLTreeMap.Entry<V> x = e;
      ...
      while (q != null) {
         ...
         if (!dir) {
            ...
            if (y.balance() == -2) {
               ...
               if (x.balance() == 1) {
                  assert x.balance() == 1;
                  Byte2ReferenceAVLTreeMap.Entry<V> w = x.right;
                  x.right = w.left;
                  w.left = x;
                  y.left = w.right;
                  w.right = y;
   ...
   public ObjectSortedSet<Byte2ReferenceMap.Entry<V>> byte2ReferenceEntrySet() {
      if (this.entries == null) {
         this.entries = new AbstractObjectSortedSet<Byte2ReferenceMap.Entry<V>>() {
            final Comparator<? super Byte2ReferenceMap.Entry<V>> comparator = Byte2ReferenceAVLTreeMap.this.actualComparator == null
   ...
   private Byte2ReferenceAVLTreeMap.Entry<V> readTree(
      ObjectInputStream s, int n, Byte2ReferenceAVLTreeMap.Entry<V> pred, Byte2ReferenceAVLTreeMap.Entry<V> succ
   ...
   private final class Submap extends AbstractByte2ReferenceSortedMap<V> implements Serializable {
      ...
      final boolean in(byte k) {
         return (this.bottom || Byte2ReferenceAVLTreeMap.this.compare(k, this.from) >= 0)
            && (this.top || Byte2ReferenceAVLTreeMap.this.compare(k, this.to) < 0);
      }
