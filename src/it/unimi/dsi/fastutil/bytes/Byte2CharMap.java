...
public interface Byte2CharMap extends Byte2CharFunction, Map<Byte, Character> {
   ...
   default void forEach(BiConsumer<? super Byte, ? super Character> consumer) {
      ObjectSet<Byte2CharMap.Entry> entrySet = this.byte2CharEntrySet();
      Consumer<Byte2CharMap.Entry> wrappingConsumer = entry -> consumer.accept(entry.getByteKey(), entry.getCharValue());
      if (entrySet instanceof Byte2CharMap.FastEntrySet) {
         ((Byte2CharMap.FastEntrySet)entrySet).fastForEach(wrappingConsumer);
      } else {
   ...
   interface Entry extends java.util.Map.Entry<Byte, Character> {
      byte getByteKey();
