...
public interface Byte2BooleanMap extends Byte2BooleanFunction, Map<Byte, Boolean> {
   ...
   default void forEach(BiConsumer<? super Byte, ? super Boolean> consumer) {
      ObjectSet<Byte2BooleanMap.Entry> entrySet = this.byte2BooleanEntrySet();
      Consumer<Byte2BooleanMap.Entry> wrappingConsumer = entry -> consumer.accept(entry.getByteKey(), entry.getBooleanValue());
      if (entrySet instanceof Byte2BooleanMap.FastEntrySet) {
         ((Byte2BooleanMap.FastEntrySet)entrySet).fastForEach(wrappingConsumer);
      } else {
   ...
   interface Entry extends java.util.Map.Entry<Byte, Boolean> {
      byte getByteKey();
