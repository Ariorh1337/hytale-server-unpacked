...
public class ByteOpenCustomHashSet extends AbstractByteSet implements Serializable, Cloneable, Hash {
   ...
   public ByteSpliterator spliterator() {
      return new ByteOpenCustomHashSet.SetSpliterator();
   }
   ...
   private final class SetSpliterator implements ByteSpliterator {
      ...
      public long estimateSize() {
         return !this.hasSplit
            ? ByteOpenCustomHashSet.this.size - this.c
            : Math.min(
               ByteOpenCustomHashSet.this.size - this.c,
               (long)((double)ByteOpenCustomHashSet.this.realSize() / ByteOpenCustomHashSet.this.n * (this.max - this.pos)) + (this.mustReturnNull ? 1 : 0)
            );
      }

      public ByteOpenCustomHashSet.SetSpliterator trySplit() {
         ...
         int retPos = this.pos;
         int retMax = myNewPos;
         ByteOpenCustomHashSet.SetSpliterator split = ByteOpenCustomHashSet.this.new SetSpliterator(retPos, retMax, this.mustReturnNull, true);
         this.pos = myNewPos;
         this.mustReturnNull = false;
         this.hasSplit = true;
         return split;
