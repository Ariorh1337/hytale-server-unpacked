...
public interface Byte2ObjectMap<V> extends Byte2ObjectFunction<V>, Map<Byte, V> {
   ...
   default void forEach(BiConsumer<? super Byte, ? super V> consumer) {
      ObjectSet<Byte2ObjectMap.Entry<V>> entrySet = this.byte2ObjectEntrySet();
      Consumer<Byte2ObjectMap.Entry<V>> wrappingConsumer = entry -> consumer.accept(entry.getByteKey(), entry.getValue());
      if (entrySet instanceof Byte2ObjectMap.FastEntrySet) {
         ((Byte2ObjectMap.FastEntrySet)entrySet).fastForEach(wrappingConsumer);
      } else {
   ...
   interface Entry<V> extends java.util.Map.Entry<Byte, V> {
      byte getByteKey();
