...
public class Short2ReferenceRBTreeMap<V> extends AbstractShort2ReferenceSortedMap<V> implements Serializable, Cloneable {
            ...
            while (true) {
               ...
               if (!this.dirPath[i - 1]) {
                  ...
                  if (this.nodePath[i - 1].succ() || y.black()) {
                     ...
                     Short2ReferenceRBTreeMap.Entry<V> x = this.nodePath[i - 1];
                     x.black(false);
                     y.black(true);
                     x.left = y.right;
                     y.right = x;
      ...
      if (p.black()) {
         for (; i > 0; i--) {
            ...
            if (!this.dirPath[i - 1]) {
               ...
               Short2ReferenceRBTreeMap.Entry<V> w = this.nodePath[i - 1].left;
               if (!w.black()) {
                  w.black(true);
                  this.nodePath[i - 1].black(false);
               ...
               if (!w.pred() && !w.left.black() || !w.succ() && !w.right.black()) {
                  if (w.pred() || w.left.black()) {
                     Short2ReferenceRBTreeMap.Entry<V> y = w.right;
                     y.black(true);
                     w.black(false);
                     w.right = y.left;
                     y.left = w;
   ...
   public ObjectSortedSet<Short2ReferenceMap.Entry<V>> short2ReferenceEntrySet() {
      if (this.entries == null) {
         this.entries = new AbstractObjectSortedSet<Short2ReferenceMap.Entry<V>>() {
            final Comparator<? super Short2ReferenceMap.Entry<V>> comparator = Short2ReferenceRBTreeMap.this.actualComparator == null
               ? (x, y) -> Short.compare(x.getShortKey(), y.getShortKey())
               : (x, y) -> Short2ReferenceRBTreeMap.this.actualComparator.compare(x.getShortKey(), y.getShortKey());
   ...
   private Short2ReferenceRBTreeMap.Entry<V> readTree(
      ObjectInputStream s, int n, Short2ReferenceRBTreeMap.Entry<V> pred, Short2ReferenceRBTreeMap.Entry<V> succ
   ...
   private final class Submap extends AbstractShort2ReferenceSortedMap<V> implements Serializable {
      ...
      final boolean in(short k) {
         return (this.bottom || Short2ReferenceRBTreeMap.this.compare(k, this.from) >= 0)
            && (this.top || Short2ReferenceRBTreeMap.this.compare(k, this.to) < 0);
      }
