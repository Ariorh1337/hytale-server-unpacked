...
public interface Short2CharMap extends Short2CharFunction, Map<Short, Character> {
   ...
   default void forEach(BiConsumer<? super Short, ? super Character> consumer) {
      ObjectSet<Short2CharMap.Entry> entrySet = this.short2CharEntrySet();
      Consumer<Short2CharMap.Entry> wrappingConsumer = entry -> consumer.accept(entry.getShortKey(), entry.getCharValue());
      if (entrySet instanceof Short2CharMap.FastEntrySet) {
         ((Short2CharMap.FastEntrySet)entrySet).fastForEach(wrappingConsumer);
      } else {
   ...
   interface Entry extends java.util.Map.Entry<Short, Character> {
      short getShortKey();
