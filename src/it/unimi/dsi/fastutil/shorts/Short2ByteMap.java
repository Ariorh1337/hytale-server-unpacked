...
public interface Short2ByteMap extends Short2ByteFunction, Map<Short, Byte> {
   ...
   default void forEach(BiConsumer<? super Short, ? super Byte> consumer) {
      ObjectSet<Short2ByteMap.Entry> entrySet = this.short2ByteEntrySet();
      Consumer<Short2ByteMap.Entry> wrappingConsumer = entry -> consumer.accept(entry.getShortKey(), entry.getByteValue());
      if (entrySet instanceof Short2ByteMap.FastEntrySet) {
         ((Short2ByteMap.FastEntrySet)entrySet).fastForEach(wrappingConsumer);
      } else {
   ...
   interface Entry extends java.util.Map.Entry<Short, Byte> {
      short getShortKey();
