...
public class ShortLinkedOpenCustomHashSet extends AbstractShortSortedSet implements Serializable, Cloneable, Hash {
   ...
   public ShortListIterator iterator(short from) {
      return new ShortLinkedOpenCustomHashSet.SetIterator(from);
   }
   ...
   private final class SetIterator implements ShortListIterator {
      int prev = -1;
      int next = -1;
      int curr = -1;
      int index = -1;
      ...
      SetIterator(short from) {
         if (ShortLinkedOpenCustomHashSet.this.strategy.equals(from, (short)0)) {
            ...
            for (int pos = HashCommon.mix(ShortLinkedOpenCustomHashSet.this.strategy.hashCode(from)) & ShortLinkedOpenCustomHashSet.this.mask;
               key[pos] != 0;
      ...
      public void remove() {
         ...
         if (this.prev == -1) {
            ...
            ShortLinkedOpenCustomHashSet.this.link[this.prev] = ShortLinkedOpenCustomHashSet.this.link[this.prev]
               ^ (ShortLinkedOpenCustomHashSet.this.link[this.prev] ^ this.next & 4294967295L) & 4294967295L;
         }

         if (this.next == -1) {
            ShortLinkedOpenCustomHashSet.this.last = this.prev;
         } else {
            ShortLinkedOpenCustomHashSet.this.link[this.next] = ShortLinkedOpenCustomHashSet.this.link[this.next]
               ^ (ShortLinkedOpenCustomHashSet.this.link[this.next] ^ (this.prev & 4294967295L) << 32) & -4294967296L;
         }
         ...
         if (pos == ShortLinkedOpenCustomHashSet.this.n) {
            ...
            while (true) {
               ...
               short curr;
               for (pos = pos + 1 & ShortLinkedOpenCustomHashSet.this.mask; (curr = key[pos]) != 0; pos = pos + 1 & ShortLinkedOpenCustomHashSet.this.mask) {
