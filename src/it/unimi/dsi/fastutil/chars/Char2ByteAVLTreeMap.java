      ...
      while (q != null) {
         ...
         if (!dir) {
            ...
            if (y.balance() == -2) {
               ...
               if (x.balance() == 1) {
                  assert x.balance() == 1;
                  Char2ByteAVLTreeMap.Entry w = x.right;
                  x.right = w.left;
                  w.left = x;
                  y.left = w.right;
                  w.right = y;
   ...
   public ObjectSortedSet<Char2ByteMap.Entry> char2ByteEntrySet() {
      if (this.entries == null) {
         this.entries = new AbstractObjectSortedSet<Char2ByteMap.Entry>() {
            final Comparator<? super Char2ByteMap.Entry> comparator = Char2ByteAVLTreeMap.this.actualComparator == null
               ? (x, y) -> Character.compare(x.getCharKey(), y.getCharKey())
               : (x, y) -> Char2ByteAVLTreeMap.this.actualComparator.compare(x.getCharKey(), y.getCharKey());
