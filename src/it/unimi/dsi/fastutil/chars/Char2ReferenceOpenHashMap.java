...
public class Char2ReferenceOpenHashMap<V> extends AbstractChar2ReferenceMap<V> implements Serializable, Cloneable, Hash {
      ...
      if (this.values == null) {
         this.values = new AbstractReferenceCollection<V>() {
            ...
            public ObjectSpliterator<V> spliterator() {
               return Char2ReferenceOpenHashMap.this.new ValueSpliterator();
            }
   ...
   public int hashCode() {
      ...
      while (j-- != 0) {
         ...
         if (this != this.value[i]) {
            var5 ^= this.value[i] == null ? 0 : System.identityHashCode(this.value[i]);
         }
   ...
   private void writeObject(ObjectOutputStream s) throws IOException {
      char[] key = this.key;
      V[] value = this.value;
      Char2ReferenceOpenHashMap<V>.EntryIterator i = new Char2ReferenceOpenHashMap.EntryIterator();
   ...
   private final class EntryIterator
      extends Char2ReferenceOpenHashMap<V>.MapIterator<Consumer<? super Char2ReferenceMap.Entry<V>>>
      ...
      final void acceptOnIndex(Consumer<? super Char2ReferenceMap.Entry<V>> action, int index) {
         action.accept(this.entry = Char2ReferenceOpenHashMap.this.new MapEntry(index));
      }
   ...
   private final class EntrySpliterator
      extends Char2ReferenceOpenHashMap<V>.MapSpliterator<Consumer<? super Char2ReferenceMap.Entry<V>>, Char2ReferenceOpenHashMap<V>.EntrySpliterator>
      ...
      final void acceptOnIndex(Consumer<? super Char2ReferenceMap.Entry<V>> action, int index) {
         action.accept(Char2ReferenceOpenHashMap.this.new MapEntry(index));
      }

      final Char2ReferenceOpenHashMap<V>.EntrySpliterator makeForSplit(int pos, int max, boolean mustReturnNull) {
         return Char2ReferenceOpenHashMap.this.new EntrySpliterator(pos, max, mustReturnNull, true);
      }
   }
   ...
   private final class FastEntryIterator
      extends Char2ReferenceOpenHashMap<V>.MapIterator<Consumer<? super Char2ReferenceMap.Entry<V>>>
      implements ObjectIterator<Char2ReferenceMap.Entry<V>> {
      private final Char2ReferenceOpenHashMap<V>.MapEntry entry = Char2ReferenceOpenHashMap.this.new MapEntry();
   ...
   private final class KeyIterator extends Char2ReferenceOpenHashMap<V>.MapIterator<CharConsumer> implements CharIterator {
      public KeyIterator() {
      }
   ...
   private final class KeySet extends AbstractCharSet {
      ...
      public CharSpliterator spliterator() {
         return Char2ReferenceOpenHashMap.this.new KeySpliterator();
      }
   ...
   private final class KeySpliterator
      extends Char2ReferenceOpenHashMap<V>.MapSpliterator<CharConsumer, Char2ReferenceOpenHashMap<V>.KeySpliterator>
      ...
      final Char2ReferenceOpenHashMap<V>.KeySpliterator makeForSplit(int pos, int max, boolean mustReturnNull) {
         return Char2ReferenceOpenHashMap.this.new KeySpliterator(pos, max, mustReturnNull, true);
      }
   }
   ...
   final class MapEntry implements Char2ReferenceMap.Entry<V>, Entry<Character, V>, CharReferencePair<V> {
      ...
      public boolean equals(Object o) {
         ...
         return Char2ReferenceOpenHashMap.this.key[this.index] == e.getKey() && Char2ReferenceOpenHashMap.this.value[this.index] == e.getValue();
      }

      @Override
      public int hashCode() {
         return Char2ReferenceOpenHashMap.this.key[this.index]
            ^ (Char2ReferenceOpenHashMap.this.value[this.index] == null ? 0 : System.identityHashCode(Char2ReferenceOpenHashMap.this.value[this.index]));
      }
   ...
   private final class MapEntrySet extends AbstractObjectSet<Char2ReferenceMap.Entry<V>> implements Char2ReferenceMap.FastEntrySet<V> {
      ...
      public ObjectSpliterator<Char2ReferenceMap.Entry<V>> spliterator() {
         return Char2ReferenceOpenHashMap.this.new EntrySpliterator();
      }
      ...
      public void forEach(Consumer<? super Char2ReferenceMap.Entry<V>> consumer) {
         if (Char2ReferenceOpenHashMap.this.containsNullKey) {
            consumer.accept(Char2ReferenceOpenHashMap.this.new MapEntry(Char2ReferenceOpenHashMap.this.n));
         }
         ...
         while (pos-- != 0) {
            if (Char2ReferenceOpenHashMap.this.key[pos] != 0) {
               consumer.accept(Char2ReferenceOpenHashMap.this.new MapEntry(pos));
            }
         }
      }
      ...
      public void fastForEach(Consumer<? super Char2ReferenceMap.Entry<V>> consumer) {
         Char2ReferenceOpenHashMap<V>.MapEntry entry = Char2ReferenceOpenHashMap.this.new MapEntry();
         if (Char2ReferenceOpenHashMap.this.containsNullKey) {
   ...
   private abstract class MapIterator<ConsumerType> {
      int pos = Char2ReferenceOpenHashMap.this.n;
      int last = -1;
      int c = Char2ReferenceOpenHashMap.this.size;
      ...
      public void forEachRemaining(ConsumerType action) {
         if (this.mustReturnNullKey) {
            this.mustReturnNullKey = false;
            this.acceptOnIndex(action, this.last = Char2ReferenceOpenHashMap.this.n);
            this.c--;
         }
      ...
      private void shiftKeys(int pos) {
         ...
         while (true) {
            ...
            char curr;
            for (pos = pos + 1 & Char2ReferenceOpenHashMap.this.mask; (curr = key[pos]) != 0; pos = pos + 1 & Char2ReferenceOpenHashMap.this.mask) {
   ...
   private abstract class MapSpliterator<ConsumerType, SplitType extends Char2ReferenceOpenHashMap<V>.MapSpliterator<ConsumerType, SplitType>> {
      int pos = 0;
      int max;
      int c;
      ...
      public long estimateSize() {
         return !this.hasSplit
            ? Char2ReferenceOpenHashMap.this.size - this.c
            : Math.min(
               Char2ReferenceOpenHashMap.this.size - this.c,
               (long)((double)Char2ReferenceOpenHashMap.this.realSize() / Char2ReferenceOpenHashMap.this.n * (this.max - this.pos))
   ...
   private final class ValueIterator extends Char2ReferenceOpenHashMap<V>.MapIterator<Consumer<? super V>> implements ObjectIterator<V> {
   ...
   private final class ValueSpliterator
      extends Char2ReferenceOpenHashMap<V>.MapSpliterator<Consumer<? super V>, Char2ReferenceOpenHashMap<V>.ValueSpliterator>
      ...
      final Char2ReferenceOpenHashMap<V>.ValueSpliterator makeForSplit(int pos, int max, boolean mustReturnNull) {
         return Char2ReferenceOpenHashMap.this.new ValueSpliterator(pos, max, mustReturnNull, true);
      }
   }
}
