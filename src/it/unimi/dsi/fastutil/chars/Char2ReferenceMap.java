...
public interface Char2ReferenceMap<V> extends Char2ReferenceFunction<V>, Map<Character, V> {
   ...
   default void forEach(BiConsumer<? super Character, ? super V> consumer) {
      ObjectSet<Char2ReferenceMap.Entry<V>> entrySet = this.char2ReferenceEntrySet();
      Consumer<Char2ReferenceMap.Entry<V>> wrappingConsumer = entry -> consumer.accept(entry.getCharKey(), entry.getValue());
      if (entrySet instanceof Char2ReferenceMap.FastEntrySet) {
         ((Char2ReferenceMap.FastEntrySet)entrySet).fastForEach(wrappingConsumer);
      } else {
   ...
   interface Entry<V> extends java.util.Map.Entry<Character, V> {
      char getCharKey();
