...
public interface Char2ByteMap extends Char2ByteFunction, Map<Character, Byte> {
   ...
   default void forEach(BiConsumer<? super Character, ? super Byte> consumer) {
      ObjectSet<Char2ByteMap.Entry> entrySet = this.char2ByteEntrySet();
      Consumer<Char2ByteMap.Entry> wrappingConsumer = entry -> consumer.accept(entry.getCharKey(), entry.getByteValue());
      if (entrySet instanceof Char2ByteMap.FastEntrySet) {
         ((Char2ByteMap.FastEntrySet)entrySet).fastForEach(wrappingConsumer);
      } else {
   ...
   interface Entry extends java.util.Map.Entry<Character, Byte> {
      char getCharKey();
