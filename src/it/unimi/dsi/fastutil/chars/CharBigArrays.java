   ...
   public static void radixSort(char[][] a, long from, long to) {
      ...
      while (offsetPos > 0) {
         ...
         if (length < 40L) {
            ...
            while (ix < end) {
               char t = BigArrays.get(a, ix + first);
   ...
   public static void radixSort(char[][] a, char[][] b, long from, long to) {
      ...
      while (offsetPos > 0) {
         ...
         if (length < 40L) {
            ...
            while (ix < end) {
               char t = BigArrays.get(a, ix + first);
               char u = BigArrays.get(b, ix + first);
               c = BigArrays.get(digit, ix) & 255;
   ...
   public static void radixSortIndirect(long[][] perm, char[][] a, char[][] b, long from, long to, boolean stable) {
      if (to - from < 1024L) {
         ...
         while (stackPos > 0) {
            ...
            while (i-- != first) {
               count[BigArrays.get(k, BigArrays.get(perm, i)) >>> shift & 0xFF ^ 0]++;
            }
            ...
            if (stable) {
               ...
               while (ix-- != first) {
                  BigArrays.set(support, --pos[BigArrays.get(k, BigArrays.get(perm, ix)) >>> shift & 0xFF ^ 0], BigArrays.get(perm, ix));
               }
            ...
            while (true) {
               ...
               while (b > c || (comparison = Character.compare(BigArrays.get(x, b), v)) > 0) {
                  ...
                  if (b > c) {
                     ...
                     if (s > 1L && t > 1L) {
                        invokeAll(
                           new CharBigArrays.ForkJoinQuickSort(x, this.from, this.from + s), new CharBigArrays.ForkJoinQuickSort(x, this.to - t, this.to)
                        );
                     } else if (s > 1L) {
                        invokeAll(new CharBigArrays.ForkJoinQuickSort(x, this.from, this.from + s));
                     } else {
                        invokeAll(new CharBigArrays.ForkJoinQuickSort(x, this.to - t, this.to));
                     }
            ...
            while (true) {
               ...
               while (b > c || (comparison = this.comp.compare(BigArrays.get(x, b), v)) > 0) {
                  ...
                  if (b > c) {
                     ...
                     if (s > 1L && t > 1L) {
                        invokeAll(
                           new CharBigArrays.ForkJoinQuickSortComp(x, this.from, this.from + s, this.comp),
                           new CharBigArrays.ForkJoinQuickSortComp(x, this.to - t, this.to, this.comp)
                        );
                     } else if (s > 1L) {
                        invokeAll(new CharBigArrays.ForkJoinQuickSortComp(x, this.from, this.from + s, this.comp));
                     } else {
                        invokeAll(new CharBigArrays.ForkJoinQuickSortComp(x, this.to - t, this.to, this.comp));
                     }
