...
public class Char2LongAVLTreeMap extends AbstractChar2LongSortedMap implements Serializable, Cloneable {
   ...
   final Char2LongAVLTreeMap.Entry findKey(char k) {
      Char2LongAVLTreeMap.Entry e = this.tree;
   ...
   final Char2LongAVLTreeMap.Entry locateKey(char k) {
      Char2LongAVLTreeMap.Entry e = this.tree;
      Char2LongAVLTreeMap.Entry last = this.tree;
   ...
   public long addTo(char k, long incr) {
      Char2LongAVLTreeMap.Entry e = this.add(k);
   ...
   private Char2LongAVLTreeMap.Entry add(char k) {
      this.modified = false;
      Char2LongAVLTreeMap.Entry e = null;
      if (this.tree == null) {
         this.count++;
         e = this.tree = this.lastEntry = this.firstEntry = new Char2LongAVLTreeMap.Entry(k, this.defRetValue);
         this.modified = true;
      } else {
         Char2LongAVLTreeMap.Entry p = this.tree;
         Char2LongAVLTreeMap.Entry q = null;
         Char2LongAVLTreeMap.Entry y = this.tree;
         Char2LongAVLTreeMap.Entry z = null;
         Char2LongAVLTreeMap.Entry w = null;
         int i = 0;
         ...
         while (true) {
            ...
            if (this.dirPath[i++] = cmp > 0) {
               if (p.succ()) {
                  this.count++;
                  e = new Char2LongAVLTreeMap.Entry(k, this.defRetValue);
                  this.modified = true;
                  if (p.right == null) {
                     this.lastEntry = e;
                  }
         ...
         if (y.balance() == -2) {
            Char2LongAVLTreeMap.Entry x = y.left;
            if (x.balance() == -1) {
               w = x;
               if (x.succ()) {
                  x.succ(false);
            ...
            Char2LongAVLTreeMap.Entry x = y.right;
            if (x.balance() == 1) {
               w = x;
               if (x.pred()) {
                  x.pred(false);
   ...
   private Char2LongAVLTreeMap.Entry parent(Char2LongAVLTreeMap.Entry e) {
      if (e == this.tree) {
         return null;
      }

      Char2LongAVLTreeMap.Entry y = e;
      Char2LongAVLTreeMap.Entry x = e;

      while (!y.succ()) {
         if (x.pred()) {
            Char2LongAVLTreeMap.Entry p = x.left;
            if (p == null || p.right != e) {
               while (!y.succ()) {
                  y = y.right;
               }
      ...
      Char2LongAVLTreeMap.Entry p = y.right;
      if (p == null || p.left != e) {
         while (!x.pred()) {
            x = x.left;
         }
   ...
   public long remove(char k) {
      ...
      if (p.succ()) {
         ...
         Char2LongAVLTreeMap.Entry r = p.right;
         if (r.pred()) {
            r.left = p.left;
            r.pred(p.pred());
            if (!r.pred()) {
            ...
            while (true) {
               Char2LongAVLTreeMap.Entry s = r.left;
               if (s.pred()) {
                  if (s.succ()) {
                     r.pred(s);
                  } else {
      ...
      while (q != null) {
         ...
         if (!dir) {
            ...
            if (y.balance() == 2) {
               ...
               if (x.balance() == -1) {
                  assert x.balance() == -1;
                  Char2LongAVLTreeMap.Entry w = x.left;
                  x.left = w.right;
                  w.right = x;
                  y.right = w.left;
                  w.left = y;
            ...
            if (y.balance() == -2) {
               ...
               if (x.balance() == 1) {
                  assert x.balance() == 1;
                  Char2LongAVLTreeMap.Entry w = x.right;
                  x.right = w.left;
                  w.left = x;
                  y.left = w.right;
                  w.right = y;
   ...
   public boolean containsValue(long v) {
      Char2LongAVLTreeMap.ValueIterator i = new Char2LongAVLTreeMap.ValueIterator();
      int j = this.count;
   ...
   public long get(char k) {
      Char2LongAVLTreeMap.Entry e = this.findKey(k);
   ...
   public ObjectSortedSet<Char2LongMap.Entry> char2LongEntrySet() {
      if (this.entries == null) {
         this.entries = new AbstractObjectSortedSet<Char2LongMap.Entry>() {
            final Comparator<? super Char2LongMap.Entry> comparator = Char2LongAVLTreeMap.this.actualComparator == null
               ? (x, y) -> Character.compare(x.getCharKey(), y.getCharKey())
               : (x, y) -> Char2LongAVLTreeMap.this.actualComparator.compare(x.getCharKey(), y.getCharKey());
            ...
            public ObjectBidirectionalIterator<Char2LongMap.Entry> iterator(Char2LongMap.Entry from) {
               return Char2LongAVLTreeMap.this.new EntryIterator(from.getCharKey());
            }
            ...
            public boolean contains(Object o) {
               if (o != null && o instanceof java.util.Map.Entry) {
                  ...
                  if (e.getKey() == null) {
                     ...
                     Char2LongAVLTreeMap.Entry f = Char2LongAVLTreeMap.this.findKey((Character)e.getKey());
                     return e.equals(f);
                  } else {
                     return false;
                  }
   ...
   public Char2LongSortedMap headMap(char to) {
      return new Char2LongAVLTreeMap.Submap('\u0000', true, to, false);
   }

   @Override
   public Char2LongSortedMap tailMap(char from) {
      return new Char2LongAVLTreeMap.Submap(from, false, '\u0000', true);
   }

   @Override
   public Char2LongSortedMap subMap(char from, char to) {
      return new Char2LongAVLTreeMap.Submap(from, false, to, false);
   }
   ...
   public Char2LongAVLTreeMap clone() {
      ...
      Char2LongAVLTreeMap.Entry rp = new Char2LongAVLTreeMap.Entry();
      Char2LongAVLTreeMap.Entry rq = new Char2LongAVLTreeMap.Entry();
      Char2LongAVLTreeMap.Entry p = rp;
      rp.left(this.tree);
      Char2LongAVLTreeMap.Entry q = rq;
      rq.pred(null);

      while (true) {
         if (!p.pred()) {
            Char2LongAVLTreeMap.Entry e = p.left.clone();
            e.pred(q.left);
            e.succ(q);
            q.left(e);
            p = p.left;
         ...
         if (!p.succ()) {
            Char2LongAVLTreeMap.Entry e = p.right.clone();
            e.succ(q.right);
            e.pred(q);
            q.right(e);
         }
   ...
   private void writeObject(ObjectOutputStream s) throws IOException {
      int n = this.count;
      Char2LongAVLTreeMap.EntryIterator i = new Char2LongAVLTreeMap.EntryIterator();
      s.defaultWriteObject();

      while (n-- != 0) {
         Char2LongAVLTreeMap.Entry e = i.nextEntry();
         s.writeChar(e.key);
         s.writeLong(e.value);
      }
   }

   private Char2LongAVLTreeMap.Entry readTree(ObjectInputStream s, int n, Char2LongAVLTreeMap.Entry pred, Char2LongAVLTreeMap.Entry succ) throws IOException, ClassNotFoundException {
      if (n == 1) {
         Char2LongAVLTreeMap.Entry top = new Char2LongAVLTreeMap.Entry(s.readChar(), s.readLong());
         top.pred(pred);
         top.succ(succ);
         return top;
      }
      ...
      if (n == 2) {
         Char2LongAVLTreeMap.Entry top = new Char2LongAVLTreeMap.Entry(s.readChar(), s.readLong());
         top.right(new Char2LongAVLTreeMap.Entry(s.readChar(), s.readLong()));
         top.right.pred(top);
         top.balance(1);
         top.pred(pred);
         top.right.succ(succ);
      ...
      Char2LongAVLTreeMap.Entry top = new Char2LongAVLTreeMap.Entry();
   ...
   private static final class Entry extends AbstractChar2LongMap.BasicEntry implements Cloneable {
      ...
      void pred(Char2LongAVLTreeMap.Entry pred) {
         this.info |= 1073741824;
         this.left = pred;
      }

      void succ(Char2LongAVLTreeMap.Entry succ) {
         this.info |= Integer.MIN_VALUE;
         this.right = succ;
      }

      void left(Char2LongAVLTreeMap.Entry left) {
         this.info &= -1073741825;
         this.left = left;
      }

      void right(Char2LongAVLTreeMap.Entry right) {
         this.info &= Integer.MAX_VALUE;
         this.right = right;
      }
      ...
      Char2LongAVLTreeMap.Entry next() {
         Char2LongAVLTreeMap.Entry next = this.right;
      ...
      Char2LongAVLTreeMap.Entry prev() {
         Char2LongAVLTreeMap.Entry prev = this.left;
      ...
      public Char2LongAVLTreeMap.Entry clone() {
         Char2LongAVLTreeMap.Entry c;
         try {
            c = (Char2LongAVLTreeMap.Entry)super.clone();
         } catch (CloneNotSupportedException cantHappen) {
            throw new InternalError();
         }
   ...
   private class KeySet extends AbstractChar2LongSortedMap.KeySet {
      ...
      public CharBidirectionalIterator iterator(char from) {
         return Char2LongAVLTreeMap.this.new KeyIterator(from);
      }
   }
   ...
   private final class Submap extends AbstractChar2LongSortedMap implements Serializable {
      ...
      @Override
      public void clear() {
         Char2LongAVLTreeMap.Submap.SubmapIterator i = new Char2LongAVLTreeMap.Submap.SubmapIterator();
      ...
      public Char2LongSortedMap headMap(char to) {
         if (this.top) {
            return Char2LongAVLTreeMap.this.new Submap(this.from, this.bottom, to, false);
         } else {
            return Char2LongAVLTreeMap.this.compare(to, this.to) < 0 ? Char2LongAVLTreeMap.this.new Submap(this.from, this.bottom, to, false) : this;
         }
      }
      ...
      public Char2LongSortedMap tailMap(char from) {
         if (this.bottom) {
            return Char2LongAVLTreeMap.this.new Submap(from, false, this.to, this.top);
         } else {
            return Char2LongAVLTreeMap.this.compare(from, this.from) > 0 ? Char2LongAVLTreeMap.this.new Submap(from, false, this.to, this.top) : this;
         }
      }
      ...
      public Char2LongSortedMap subMap(char from, char to) {
         if (this.top && this.bottom) {
            return Char2LongAVLTreeMap.this.new Submap(from, false, to, false);
         }

         if (!this.top) {
            to = Char2LongAVLTreeMap.this.compare(to, this.to) < 0 ? to : this.to;
         }
         ...
         return !this.top && !this.bottom && from == this.from && to == this.to ? this : Char2LongAVLTreeMap.this.new Submap(from, false, to, false);
      }
      ...
      public Char2LongAVLTreeMap.Entry firstEntry() {
         ...
         return e != null && (this.top || Char2LongAVLTreeMap.this.compare(e.key, this.to) < 0) ? e : null;
      }
      ...
      public Char2LongAVLTreeMap.Entry lastEntry() {
         ...
         return e != null && (this.bottom || Char2LongAVLTreeMap.this.compare(e.key, this.from) >= 0) ? e : null;
      }

      @Override
      public char firstCharKey() {
         Char2LongAVLTreeMap.Entry e = this.firstEntry();
         if (e == null) {
            throw new NoSuchElementException();
         } else {
            return e.key;
      ...
      @Override
      public char lastCharKey() {
         Char2LongAVLTreeMap.Entry e = this.lastEntry();
         if (e == null) {
            throw new NoSuchElementException();
         } else {
            return e.key;
      ...
      private class SubmapIterator extends Char2LongAVLTreeMap.TreeIterator {
         ...
         SubmapIterator(char k) {
            this();
            if (this.next != null) {
               if (!Submap.this.bottom && Char2LongAVLTreeMap.this.compare(k, this.next.key) < 0) {
                  this.prev = null;
               } else if (!Submap.this.top && Char2LongAVLTreeMap.this.compare(k, (this.prev = Submap.this.lastEntry()).key) >= 0) {
                  this.next = null;
               } else {
   ...
   private class TreeIterator {
      ...
      TreeIterator(char k) {
         if ((this.next = Char2LongAVLTreeMap.this.locateKey(k)) != null) {
            if (Char2LongAVLTreeMap.this.compare(this.next.key, k) <= 0) {
               this.prev = this.next;
               this.next = this.next.next();
            } else {
