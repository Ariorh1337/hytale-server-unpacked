...
public interface Double2ReferenceMap<V> extends Double2ReferenceFunction<V>, Map<Double, V> {
   ...
   default void forEach(BiConsumer<? super Double, ? super V> consumer) {
      ObjectSet<Double2ReferenceMap.Entry<V>> entrySet = this.double2ReferenceEntrySet();
      Consumer<Double2ReferenceMap.Entry<V>> wrappingConsumer = entry -> consumer.accept(entry.getDoubleKey(), entry.getValue());
      if (entrySet instanceof Double2ReferenceMap.FastEntrySet) {
         ((Double2ReferenceMap.FastEntrySet)entrySet).fastForEach(wrappingConsumer);
      } else {
   ...
   interface Entry<V> extends java.util.Map.Entry<Double, V> {
      double getDoubleKey();
