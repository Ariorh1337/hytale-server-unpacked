            ...
            while (true) {
               ...
               if (!this.dirPath[i - 1]) {
                  ...
                  if (this.nodePath[i - 1].succ() || y.black()) {
                     ...
                     Double2FloatRBTreeMap.Entry x = this.nodePath[i - 1];
                     x.black(false);
                     y.black(true);
                     x.left = y.right;
                     y.right = x;
      ...
      if (p.black()) {
         for (; i > 0; i--) {
            ...
            if (!this.dirPath[i - 1]) {
               ...
               Double2FloatRBTreeMap.Entry w = this.nodePath[i - 1].left;
               if (!w.black()) {
                  w.black(true);
                  this.nodePath[i - 1].black(false);
               ...
               if (!w.pred() && !w.left.black() || !w.succ() && !w.right.black()) {
                  if (w.pred() || w.left.black()) {
                     Double2FloatRBTreeMap.Entry y = w.right;
                     y.black(true);
                     w.black(false);
                     w.right = y.left;
                     y.left = w;
   ...
   public ObjectSortedSet<Double2FloatMap.Entry> double2FloatEntrySet() {
      if (this.entries == null) {
         this.entries = new AbstractObjectSortedSet<Double2FloatMap.Entry>() {
            final Comparator<? super Double2FloatMap.Entry> comparator = Double2FloatRBTreeMap.this.actualComparator == null
               ? (x, y) -> Double.compare(x.getDoubleKey(), y.getDoubleKey())
               : (x, y) -> Double2FloatRBTreeMap.this.actualComparator.compare(x.getDoubleKey(), y.getDoubleKey());
   ...
   private static final class Entry extends AbstractDouble2FloatMap.BasicEntry implements Cloneable {
      ...
      public boolean equals(Object o) {
         ...
         return Double.doubleToLongBits(this.key) == Double.doubleToLongBits(e.getKey())
            && Float.floatToIntBits(this.value) == Float.floatToIntBits(e.getValue());
      }
