...
public final class DoubleBigArrays {
         ...
         while (true) {
            ...
            while (b > c || (comparison = Double.compare(BigArrays.get(x, b), v)) > 0) {
               ...
               if (b > c) {
                  ...
                  long s = Math.min(a - from, b - a);
                  swap(x, from, b - s, s);
                  long var28 = Math.min(d - c, n - d - 1L);
                  swap(x, b, n - var28, var28);
                  long var29;
                  if ((var29 = b - a) > 1L) {
   ...
   public static void radixSort(double[][] a, long from, long to) {
      ...
      while (offsetPos > 0) {
         ...
         if (length < 40L) {
            ...
            while (i-- != 0L) {
               BigArrays.set(digit, i, (byte)(fixDouble(BigArrays.get(a, first + i)) >>> shift & 255L ^ signMask));
            }
            ...
            while (ix < end) {
               double t = BigArrays.get(a, ix + first);
   ...
   private static void selectionSort(double[][] a, double[][] b, long from, long to) {
      for (long i = from; i < to - 1L; i++) {
         ...
         for (long j = i + 1L; j < to; j++) {
            if (Double.compare(BigArrays.get(a, j), BigArrays.get(a, m)) < 0
               || Double.compare(BigArrays.get(a, j), BigArrays.get(a, m)) == 0 && Double.compare(BigArrays.get(b, j), BigArrays.get(b, m)) < 0) {
               m = j;
            }
         }
   ...
   public static void radixSort(double[][] a, double[][] b, long from, long to) {
      ...
      while (offsetPos > 0) {
         ...
         if (length < 40L) {
            ...
            while (i-- != 0L) {
               BigArrays.set(digit, i, (byte)(fixDouble(BigArrays.get(k, first + i)) >>> shift & 255L ^ signMask));
            }
            ...
            while (ix < end) {
               double t = BigArrays.get(a, ix + first);
               double u = BigArrays.get(b, ix + first);
               c = BigArrays.get(digit, ix) & 255;
   ...
   private static void insertionSortIndirect(long[][] perm, double[][] a, double[][] b, long from, long to) {
      ...
      while (++i < to) {
         ...
         for (long u = BigArrays.get(perm, j - 1L);
            Double.compare(BigArrays.get(a, t), BigArrays.get(a, u)) < 0
               || Double.compare(BigArrays.get(a, t), BigArrays.get(a, u)) == 0 && Double.compare(BigArrays.get(b, t), BigArrays.get(b, u)) < 0;
   ...
   public static void radixSortIndirect(long[][] perm, double[][] a, double[][] b, long from, long to, boolean stable) {
      if (to - from < 1024L) {
         ...
         while (stackPos > 0) {
            ...
            while (i-- != first) {
               count[(int)(fixDouble(BigArrays.get(k, BigArrays.get(perm, i))) >>> shift & 255L ^ signMask)]++;
            }
            ...
            if (stable) {
               ...
               while (ix-- != first) {
                  BigArrays.set(
                     support, --pos[(int)(fixDouble(BigArrays.get(k, BigArrays.get(perm, ix))) >>> shift & 255L ^ signMask)], BigArrays.get(perm, ix)
                  );
               }
               ...
               while (ix <= end) {
                  ...
                  c = (int)(fixDouble(BigArrays.get(k, t)) >>> shift & 255L ^ signMask);
                  if (ix < end) {
                     long d;
                     while ((d = --pos[c]) > ix) {
                        long z = t;
            ...
            while (true) {
               ...
               while (b > c || (comparison = Double.compare(BigArrays.get(x, b), v)) > 0) {
                  ...
                  if (b > c) {
                     ...
                     if (s > 1L && t > 1L) {
                        invokeAll(
                           new DoubleBigArrays.ForkJoinQuickSort(x, this.from, this.from + s), new DoubleBigArrays.ForkJoinQuickSort(x, this.to - t, this.to)
                        );
                     } else if (s > 1L) {
                        invokeAll(new DoubleBigArrays.ForkJoinQuickSort(x, this.from, this.from + s));
                     } else {
                        invokeAll(new DoubleBigArrays.ForkJoinQuickSort(x, this.to - t, this.to));
                     }
            ...
            while (true) {
               ...
               while (b > c || (comparison = this.comp.compare(BigArrays.get(x, b), v)) > 0) {
                  ...
                  if (b > c) {
                     ...
                     if (s > 1L && t > 1L) {
                        invokeAll(
                           new DoubleBigArrays.ForkJoinQuickSortComp(x, this.from, this.from + s, this.comp),
                           new DoubleBigArrays.ForkJoinQuickSortComp(x, this.to - t, this.to, this.comp)
                        );
                     } else if (s > 1L) {
                        invokeAll(new DoubleBigArrays.ForkJoinQuickSortComp(x, this.from, this.from + s, this.comp));
                     } else {
                        invokeAll(new DoubleBigArrays.ForkJoinQuickSortComp(x, this.to - t, this.to, this.comp));
                     }
