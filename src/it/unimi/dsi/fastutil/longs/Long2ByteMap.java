...
public interface Long2ByteMap extends Long2ByteFunction, Map<Long, Byte> {
   ...
   default void forEach(BiConsumer<? super Long, ? super Byte> consumer) {
      ObjectSet<Long2ByteMap.Entry> entrySet = this.long2ByteEntrySet();
      Consumer<Long2ByteMap.Entry> wrappingConsumer = entry -> consumer.accept(entry.getLongKey(), entry.getByteValue());
      if (entrySet instanceof Long2ByteMap.FastEntrySet) {
         ((Long2ByteMap.FastEntrySet)entrySet).fastForEach(wrappingConsumer);
      } else {
   ...
   interface Entry extends java.util.Map.Entry<Long, Byte> {
      long getLongKey();
