...
public class Float2FloatRBTreeMap extends AbstractFloat2FloatSortedMap implements Serializable, Cloneable {
            ...
            while (true) {
               ...
               if (!this.dirPath[i - 1]) {
                  ...
                  if (this.nodePath[i - 1].succ() || y.black()) {
                     ...
                     Float2FloatRBTreeMap.Entry x = this.nodePath[i - 1];
                     x.black(false);
                     y.black(true);
                     x.left = y.right;
                     y.right = x;
      ...
      if (p.black()) {
         for (; i > 0; i--) {
            ...
            if (!this.dirPath[i - 1]) {
               ...
               Float2FloatRBTreeMap.Entry w = this.nodePath[i - 1].left;
               if (!w.black()) {
                  w.black(true);
                  this.nodePath[i - 1].black(false);
               ...
               if (!w.pred() && !w.left.black() || !w.succ() && !w.right.black()) {
                  if (w.pred() || w.left.black()) {
                     Float2FloatRBTreeMap.Entry y = w.right;
                     y.black(true);
                     w.black(false);
                     w.right = y.left;
                     y.left = w;
   ...
   public ObjectSortedSet<Float2FloatMap.Entry> float2FloatEntrySet() {
      if (this.entries == null) {
         this.entries = new AbstractObjectSortedSet<Float2FloatMap.Entry>() {
            final Comparator<? super Float2FloatMap.Entry> comparator = Float2FloatRBTreeMap.this.actualComparator == null
               ? (x, y) -> Float.compare(x.getFloatKey(), y.getFloatKey())
               : (x, y) -> Float2FloatRBTreeMap.this.actualComparator.compare(x.getFloatKey(), y.getFloatKey());
   ...
   private static final class Entry extends AbstractFloat2FloatMap.BasicEntry implements Cloneable {
      ...
      public boolean equals(Object o) {
         ...
         return Float.floatToIntBits(this.key) == Float.floatToIntBits(e.getKey()) && Float.floatToIntBits(this.value) == Float.floatToIntBits(e.getValue());
      }
