...
public interface Float2ReferenceMap<V> extends Float2ReferenceFunction<V>, Map<Float, V> {
   ...
   default void forEach(BiConsumer<? super Float, ? super V> consumer) {
      ObjectSet<Float2ReferenceMap.Entry<V>> entrySet = this.float2ReferenceEntrySet();
      Consumer<Float2ReferenceMap.Entry<V>> wrappingConsumer = entry -> consumer.accept(entry.getFloatKey(), entry.getValue());
      if (entrySet instanceof Float2ReferenceMap.FastEntrySet) {
         ((Float2ReferenceMap.FastEntrySet)entrySet).fastForEach(wrappingConsumer);
      } else {
   ...
   interface Entry<V> extends java.util.Map.Entry<Float, V> {
      float getFloatKey();
