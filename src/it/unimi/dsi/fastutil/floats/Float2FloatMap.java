...
public interface Float2FloatMap extends Float2FloatFunction, Map<Float, Float> {
   ...
   default void forEach(BiConsumer<? super Float, ? super Float> consumer) {
      ObjectSet<Float2FloatMap.Entry> entrySet = this.float2FloatEntrySet();
      Consumer<Float2FloatMap.Entry> wrappingConsumer = entry -> consumer.accept(entry.getFloatKey(), entry.getFloatValue());
      if (entrySet instanceof Float2FloatMap.FastEntrySet) {
         ((Float2FloatMap.FastEntrySet)entrySet).fastForEach(wrappingConsumer);
      } else {
   ...
   interface Entry extends java.util.Map.Entry<Float, Float> {
      float getFloatKey();
