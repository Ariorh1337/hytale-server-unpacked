...
public class FloatBigArrayBigList extends AbstractFloatBigList implements RandomAccess, Cloneable, Serializable {
   ...
   public FloatBigArrayBigList(FloatCollection c) {
      ...
      if (c instanceof FloatBigList) {
         ((FloatBigList)c).getElements(0L, this.a, 0L, this.size = Size64.sizeOf(c));
      } else {
   ...
   public long indexOf(float k) {
      for (long i = 0L; i < this.size; i++) {
         if (Float.floatToIntBits(k) == Float.floatToIntBits(BigArrays.get(this.a, i))) {
            return i;
         }
      }
   ...
   public FloatBigList subList(long from, long to) {
      if (from == 0L && to == this.size64()) {
         ...
         if (from > to) {
            ...
            return new FloatBigArrayBigList.SubList(from, to);
         }
      }
   }
      ...
      return new FloatBigListIterator() {
         ...
         public float nextFloat() {
            if (!this.hasNext()) {
               ...
               return BigArrays.get(FloatBigArrayBigList.this.a, this.last = this.pos++);
            }
         }
         ...
         public void forEachRemaining(FloatConsumer action) {
            while (this.pos < FloatBigArrayBigList.this.size) {
               action.accept(BigArrays.get(FloatBigArrayBigList.this.a, this.last = this.pos++));
            }
         }
   ...
   public FloatSpliterator spliterator() {
      return new FloatBigArrayBigList.Spliterator();
   }
   ...
   public boolean equals(Object o) {
      if (o == this) {
         ...
         return false;
      } else if (o instanceof FloatBigArrayBigList) {
         return this.equals((FloatBigArrayBigList)o);
      } else {
         return o instanceof FloatBigArrayBigList.SubList ? ((FloatBigArrayBigList.SubList)o).equals(this) : super.equals(o);
      }
   }
   ...
   public int compareTo(FloatBigArrayBigList l) {
      ...
      int i;
      for (i = 0; i < s1 && i < s2; i++) {
         float e1 = BigArrays.get(a1, i);
         float e2 = BigArrays.get(a2, i);
         int r;
   ...
   public int compareTo(BigList<? extends Float> l) {
      if (l instanceof FloatBigArrayBigList) {
         ...
         return l instanceof FloatBigArrayBigList.SubList ? -((FloatBigArrayBigList.SubList)l).compareTo(this) : super.compareTo(l);
      }
   }
   ...
   private void writeObject(ObjectOutputStream s) throws IOException {
      ...
      for (int i = 0; i < this.size; i++) {
         s.writeFloat(BigArrays.get(this.a, i));
      }
   }
   ...
   private void readObject(ObjectInputStream s) throws IOException, ClassNotFoundException {
      ...
      for (int i = 0; i < this.size; i++) {
         BigArrays.set(this.a, i, s.readFloat());
      }
   }
   ...
   private final class Spliterator implements FloatSpliterator {
      ...
      public Spliterator() {
         this(0L, FloatBigArrayBigList.this.size, false);
      }
      ...
      public FloatSpliterator trySplit() {
         ...
         this.pos = myNewPos;
         this.hasSplit = true;
         return FloatBigArrayBigList.this.new Spliterator(oldPos, retMax, true);
      }
   }
   ...
   private class SubList extends AbstractFloatBigList.FloatRandomAccessSubList {
      ...
      public FloatBigListIterator listIterator(long index) {
         return new FloatBigArrayBigList.SubList.SubListIterator(index);
      }

      @Override
      public FloatSpliterator spliterator() {
         return new FloatBigArrayBigList.SubList.SubListSpliterator();
      }
      ...
      boolean contentsEquals(float[][] otherA, long otherAFrom, long otherATo) {
         ...
         while (--pos >= this.from) {
            if (BigArrays.get(FloatBigArrayBigList.this.a, pos) != BigArrays.get(otherA, --otherPos)) {
               return false;
            }
         }
      ...
      public boolean equals(Object o) {
         ...
         } else if (o instanceof FloatBigArrayBigList.SubList) {
      ...
      int contentsCompareTo(float[][] otherA, long otherAFrom, long otherATo) {
         ...
         for (long j = otherAFrom; i < this.to && i < otherATo; j++) {
            ...
            float e2 = BigArrays.get(otherA, j);
            int r;
            if ((r = Float.compare(e1, e2)) != 0) {
               return r;
            }
      ...
      public int compareTo(BigList<? extends Float> l) {
         ...
         } else if (l instanceof FloatBigArrayBigList.SubList) {
      ...
      private final class SubListIterator extends FloatBigListIterators.AbstractIndexBasedBigListIterator {
         ...
         public float nextFloat() {
            if (!this.hasNext()) {
               ...
               return BigArrays.get(FloatBigArrayBigList.this.a, SubList.this.from + (this.lastReturned = this.pos++));
            }
         }
         ...
         public void forEachRemaining(FloatConsumer action) {
            ...
            while (this.pos < max) {
               action.accept(BigArrays.get(FloatBigArrayBigList.this.a, SubList.this.from + (this.lastReturned = this.pos++)));
            }
         }
      }
      ...
      private final class SubListSpliterator extends FloatBigSpliterators.LateBindingSizeIndexBasedSpliterator {
         ...
         protected final FloatBigArrayBigList.SubList.SubListSpliterator makeForSplit(long pos, long maxPos) {
