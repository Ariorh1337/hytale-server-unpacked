...
public class FloatOpenHashBigSet extends AbstractFloatSet implements Serializable, Cloneable, Hash, Size64 {
   ...
   public boolean add(float k) {
      if (Float.floatToIntBits(k) == 0) {
         ...
         int base;
         float curr;
         if (Float.floatToIntBits(curr = key[base = (int)((h & this.mask) >>> 27)][displ = (int)(h & this.segmentMask)]) != 0) {
   ...
   protected final void shiftKeys(long pos) {
      ...
      while (true) {
         ...
         for (pos = pos + 1L & this.mask; Float.floatToIntBits(BigArrays.get(key, pos)) != 0; pos = pos + 1L & this.mask) {
   ...
   public FloatSpliterator spliterator() {
      return new FloatOpenHashBigSet.SetSpliterator();
   }
   ...
   public void forEach(FloatConsumer action) {
      ...
      while (pos < max) {
         float gotten = BigArrays.get(key, pos++);
   ...
   protected void rehash(long newN) {
      ...
      for (long i = this.realSize(); i-- != 0L; base += (displ = displ + 1 & this.segmentMask) == 0 ? 1 : 0) {
         ...
         int b;
         int d;
         if (Float.floatToIntBits(newKey[b = (int)((h & mask) >>> 27)][d = (int)(h & newSegmentMask)]) != 0) {
   ...
   private void readObject(ObjectInputStream s) throws IOException, ClassNotFoundException {
      ...
      while (i-- != 0L) {
         ...
         if (Float.floatToIntBits(k) == 0) {
            ...
            int base;
            int displ;
            if (Float.floatToIntBits(key[base = (int)((h & this.mask) >>> 27)][displ = (int)(h & this.segmentMask)]) != 0) {
      ...
      private final void shiftKeys(long pos) {
         ...
         while (true) {
            ...
            for (pos = pos + 1L & FloatOpenHashBigSet.this.mask;
               Float.floatToIntBits(curr = BigArrays.get(key, pos)) != 0;
   ...
   private class SetSpliterator implements FloatSpliterator {
      ...
      public long estimateSize() {
         return !this.hasSplit
            ? FloatOpenHashBigSet.this.size - this.c
            : Math.min(
               FloatOpenHashBigSet.this.size - this.c,
               (long)((double)FloatOpenHashBigSet.this.realSize() / FloatOpenHashBigSet.this.n * (this.max - this.pos)) + (this.mustReturnNull ? 1 : 0)
            );
      }

      public FloatOpenHashBigSet.SetSpliterator trySplit() {
         ...
         FloatOpenHashBigSet.SetSpliterator split = FloatOpenHashBigSet.this.new SetSpliterator(retPos, retMax, this.mustReturnNull, true);
         this.pos = myNewPos;
         this.mustReturnNull = false;
         this.hasSplit = true;
         return split;
      ...
      public long skip(long n) {
         ...
         while (this.pos < this.max && n > 0L) {
            if (Float.floatToIntBits(BigArrays.get(key, this.pos++)) != 0) {
               skipped++;
               n--;
            }
         }
