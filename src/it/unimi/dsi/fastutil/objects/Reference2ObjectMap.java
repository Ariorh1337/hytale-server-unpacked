...
public interface Reference2ObjectMap<K, V> extends Reference2ObjectFunction<K, V>, Map<K, V> {
   ...
   default void forEach(BiConsumer<? super K, ? super V> consumer) {
      ObjectSet<Reference2ObjectMap.Entry<K, V>> entrySet = this.reference2ObjectEntrySet();
      Consumer<Reference2ObjectMap.Entry<K, V>> wrappingConsumer = entry -> consumer.accept(entry.getKey(), entry.getValue());
      if (entrySet instanceof Reference2ObjectMap.FastEntrySet) {
         ((Reference2ObjectMap.FastEntrySet)entrySet).fastForEach(wrappingConsumer);
      } else {
