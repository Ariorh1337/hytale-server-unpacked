...
public interface Reference2IntMap<K> extends Reference2IntFunction<K>, Map<K, Integer> {
   ...
   default void forEach(BiConsumer<? super K, ? super Integer> consumer) {
      ObjectSet<Reference2IntMap.Entry<K>> entrySet = this.reference2IntEntrySet();
      Consumer<Reference2IntMap.Entry<K>> wrappingConsumer = entry -> consumer.accept(entry.getKey(), entry.getIntValue());
      if (entrySet instanceof Reference2IntMap.FastEntrySet) {
         ((Reference2IntMap.FastEntrySet)entrySet).fastForEach(wrappingConsumer);
      } else {
   ...
   interface Entry<K> extends java.util.Map.Entry<K, Integer> {
      int getIntValue();
