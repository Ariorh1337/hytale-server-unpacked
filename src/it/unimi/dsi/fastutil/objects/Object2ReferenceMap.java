...
public interface Object2ReferenceMap<K, V> extends Object2ReferenceFunction<K, V>, Map<K, V> {
   ...
   default void forEach(BiConsumer<? super K, ? super V> consumer) {
      ObjectSet<Object2ReferenceMap.Entry<K, V>> entrySet = this.object2ReferenceEntrySet();
      Consumer<Object2ReferenceMap.Entry<K, V>> wrappingConsumer = entry -> consumer.accept(entry.getKey(), entry.getValue());
      if (entrySet instanceof Object2ReferenceMap.FastEntrySet) {
         ((Object2ReferenceMap.FastEntrySet)entrySet).fastForEach(wrappingConsumer);
      } else {
