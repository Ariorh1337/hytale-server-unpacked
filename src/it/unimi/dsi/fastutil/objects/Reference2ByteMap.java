...
public interface Reference2ByteMap<K> extends Reference2ByteFunction<K>, Map<K, Byte> {
   ...
   default void forEach(BiConsumer<? super K, ? super Byte> consumer) {
      ObjectSet<Reference2ByteMap.Entry<K>> entrySet = this.reference2ByteEntrySet();
      Consumer<Reference2ByteMap.Entry<K>> wrappingConsumer = entry -> consumer.accept(entry.getKey(), entry.getByteValue());
      if (entrySet instanceof Reference2ByteMap.FastEntrySet) {
         ((Reference2ByteMap.FastEntrySet)entrySet).fastForEach(wrappingConsumer);
      } else {
   ...
   interface Entry<K> extends java.util.Map.Entry<K, Byte> {
      byte getByteValue();
