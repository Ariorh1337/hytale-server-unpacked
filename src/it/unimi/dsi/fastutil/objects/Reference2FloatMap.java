...
public interface Reference2FloatMap<K> extends Reference2FloatFunction<K>, Map<K, Float> {
   ...
   default void forEach(BiConsumer<? super K, ? super Float> consumer) {
      ObjectSet<Reference2FloatMap.Entry<K>> entrySet = this.reference2FloatEntrySet();
      Consumer<Reference2FloatMap.Entry<K>> wrappingConsumer = entry -> consumer.accept(entry.getKey(), entry.getFloatValue());
      if (entrySet instanceof Reference2FloatMap.FastEntrySet) {
         ((Reference2FloatMap.FastEntrySet)entrySet).fastForEach(wrappingConsumer);
      } else {
   ...
   interface Entry<K> extends java.util.Map.Entry<K, Float> {
      float getFloatValue();
