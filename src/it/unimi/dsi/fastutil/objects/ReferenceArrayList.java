...
public class ReferenceArrayList<K> extends AbstractReferenceList<K> implements RandomAccess, Cloneable, Serializable {
   ...
   protected int size;
   private static final Collector<Object, ?, ReferenceArrayList<Object>> TO_LIST_COLLECTOR = Collector.of(
      ReferenceArrayList::new, ReferenceArrayList::add, ReferenceArrayList::combine
   );
   ...
   public ReferenceArrayList(Collection<? extends K> c) {
      if (c instanceof ReferenceArrayList) {
         this.a = (K[])copyArrayFromSafe((ReferenceArrayList<? extends K>)c);
   ...
   public ReferenceArrayList(ReferenceList<? extends K> l) {
      if (l instanceof ReferenceArrayList) {
         this.a = (K[])copyArrayFromSafe((ReferenceArrayList<? extends K>)l);
         this.size = this.a.length;
      } else {
   ...
   private void grow(int capacity) {
      if (capacity > this.a.length) {
         if (this.a != ObjectArrays.DEFAULT_EMPTY_ARRAY) {
            capacity = (int)Math.max(Math.min((long)this.a.length + (this.a.length >> 1), 2147483639L), capacity);
         } else if (capacity < 10) {
            capacity = 10;
         }
   ...
   public ReferenceList<K> subList(int from, int to) {
      if (from == 0 && to == this.size()) {
         ...
         if (from > to) {
            ...
            return new ReferenceArrayList.SubList(from, to);
         }
      }
   }
      ...
      return new ObjectListIterator<K>() {
         ...
         public void forEachRemaining(Consumer<? super K> action) {
            while (this.pos < ReferenceArrayList.this.size) {
               action.accept(ReferenceArrayList.this.a[this.last = this.pos++]);
            }
         }
   ...
   public ObjectSpliterator<K> spliterator() {
      return new ReferenceArrayList.Spliterator();
   }
   ...
   public ReferenceArrayList<K> clone() {
      ...
      if (this.getClass() == ReferenceArrayList.class) {
         cloned = new ReferenceArrayList<>((K[])copyArraySafe(this.a, this.size), false);
   ...
   public boolean equals(Object o) {
      if (o == this) {
         ...
         return this.equals((ReferenceArrayList<K>)o);
      } else {
         return o instanceof ReferenceArrayList.SubList ? ((ReferenceArrayList.SubList)o).equals(this) : super.equals(o);
      }
   }
   ...
   private final class Spliterator implements ObjectSpliterator<K> {
      ...
      public ObjectSpliterator<K> trySplit() {
         ...
         this.pos = myNewPos;
         this.hasSplit = true;
         return ReferenceArrayList.this.new Spliterator(oldPos, retMax, true);
      }
   }
   ...
   private class SubList extends AbstractReferenceList.ReferenceRandomAccessSubList<K> {
      ...
      public ObjectListIterator<K> listIterator(int index) {
         return new ReferenceArrayList.SubList.SubListIterator(index);
      }

      @Override
      public ObjectSpliterator<K> spliterator() {
         return new ReferenceArrayList.SubList.SubListSpliterator();
      }
      ...
      public boolean equals(Object o) {
         ...
         } else if (o instanceof ReferenceArrayList.SubList) {
            ReferenceArrayList<K>.SubList other = (ReferenceArrayList.SubList)o;
      ...
      private final class SubListIterator extends ObjectIterators.AbstractIndexBasedListIterator<K> {
         ...
         public void forEachRemaining(Consumer<? super K> action) {
            ...
            while (this.pos < max) {
               action.accept(ReferenceArrayList.this.a[SubList.this.from + (this.lastReturned = this.pos++)]);
            }
         }
      }
      ...
      private final class SubListSpliterator extends ObjectSpliterators.LateBindingSizeIndexBasedSpliterator<K> {
         ...
         protected final ReferenceArrayList<K>.SubList.SubListSpliterator makeForSplit(int pos, int maxPos) {
