...
public interface Reference2ReferenceMap<K, V> extends Reference2ReferenceFunction<K, V>, Map<K, V> {
   ...
   default void forEach(BiConsumer<? super K, ? super V> consumer) {
      ObjectSet<Reference2ReferenceMap.Entry<K, V>> entrySet = this.reference2ReferenceEntrySet();
      Consumer<Reference2ReferenceMap.Entry<K, V>> wrappingConsumer = entry -> consumer.accept(entry.getKey(), entry.getValue());
      if (entrySet instanceof Reference2ReferenceMap.FastEntrySet) {
         ((Reference2ReferenceMap.FastEntrySet)entrySet).fastForEach(wrappingConsumer);
      } else {
   ...
   interface Entry<K, V> extends java.util.Map.Entry<K, V> {
   }
