...
public class ObjectRBTreeSet<K> extends AbstractObjectSortedSet<K> implements Serializable, Cloneable, ObjectSortedSet<K> {
            ...
            while (true) {
               ...
               if (!this.dirPath[i - 1]) {
                  ...
                  if (this.nodePath[i - 1].succ() || y.black()) {
                     ...
                     ObjectRBTreeSet.Entry<K> x = this.nodePath[i - 1];
                     x.black(false);
                     y.black(true);
                     x.left = y.right;
                     y.right = x;
      ...
      if (p.black()) {
         for (; i > 0; i--) {
            ...
            if (!this.dirPath[i - 1]) {
               ...
               ObjectRBTreeSet.Entry<K> w = this.nodePath[i - 1].left;
               if (!w.black()) {
                  w.black(true);
                  this.nodePath[i - 1].black(false);
               ...
               if (!w.pred() && !w.left.black() || !w.succ() && !w.right.black()) {
                  if (w.pred() || w.left.black()) {
                     ObjectRBTreeSet.Entry<K> y = w.right;
                     y.black(true);
                     w.black(false);
                     w.right = y.left;
                     y.left = w;
   ...
   private final class Subset extends AbstractObjectSortedSet<K> implements Serializable, ObjectSortedSet<K> {
      ...
      final boolean in(K k) {
         return (this.bottom || ObjectRBTreeSet.this.compare(k, (K)this.from) >= 0) && (this.top || ObjectRBTreeSet.this.compare(k, (K)this.to) < 0);
      }
      ...
      public ObjectSortedSet<K> headSet(K to) {
         if (this.top) {
            ...
            return ObjectRBTreeSet.this.compare(to, (K)this.to) < 0 ? ObjectRBTreeSet.this.new Subset(this.from, this.bottom, to, false) : this;
         }
      }
      ...
      public ObjectSortedSet<K> tailSet(K from) {
         if (this.bottom) {
            ...
            return ObjectRBTreeSet.this.compare(from, (K)this.from) > 0 ? ObjectRBTreeSet.this.new Subset(from, false, this.to, this.top) : this;
         }
      }
      ...
      public ObjectSortedSet<K> subSet(K from, K to) {
         ...
         if (!this.top) {
            to = (K)(ObjectRBTreeSet.this.compare(to, (K)this.to) < 0 ? to : this.to);
         }

         if (!this.bottom) {
            from = (K)(ObjectRBTreeSet.this.compare(from, (K)this.from) > 0 ? from : this.from);
         }
      ...
      public ObjectRBTreeSet.Entry<K> firstEntry() {
         ...
         if (this.bottom) {
            ...
            e = ObjectRBTreeSet.this.locateKey((K)this.from);
            if (ObjectRBTreeSet.this.compare(e.key, (K)this.from) < 0) {
               e = e.next();
            }
         }

         return e != null && (this.top || ObjectRBTreeSet.this.compare(e.key, (K)this.to) < 0) ? e : null;
      }
      ...
      public ObjectRBTreeSet.Entry<K> lastEntry() {
         ...
         if (this.top) {
            ...
            e = ObjectRBTreeSet.this.locateKey((K)this.to);
            if (ObjectRBTreeSet.this.compare(e.key, (K)this.to) >= 0) {
               e = e.prev();
            }
         }

         return e != null && (this.bottom || ObjectRBTreeSet.this.compare(e.key, (K)this.from) >= 0) ? e : null;
      }
      ...
      private final class SubsetIterator extends ObjectRBTreeSet<K>.SetIterator {
         ...
         void updatePrevious() {
            this.prev = this.prev.prev();
            if (!Subset.this.bottom && this.prev != null && ObjectRBTreeSet.this.compare(this.prev.key, (K)Subset.this.from) < 0) {
               this.prev = null;
            }
         }
         ...
         void updateNext() {
            this.next = this.next.next();
            if (!Subset.this.top && this.next != null && ObjectRBTreeSet.this.compare(this.next.key, (K)Subset.this.to) >= 0) {
               this.next = null;
            }
         }
      }
