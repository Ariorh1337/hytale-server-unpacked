...
public interface Object2CharMap<K> extends Object2CharFunction<K>, Map<K, Character> {
   ...
   default void forEach(BiConsumer<? super K, ? super Character> consumer) {
      ObjectSet<Object2CharMap.Entry<K>> entrySet = this.object2CharEntrySet();
      Consumer<Object2CharMap.Entry<K>> wrappingConsumer = entry -> consumer.accept(entry.getKey(), entry.getCharValue());
      if (entrySet instanceof Object2CharMap.FastEntrySet) {
         ((Object2CharMap.FastEntrySet)entrySet).fastForEach(wrappingConsumer);
      } else {
   ...
   interface Entry<K> extends java.util.Map.Entry<K, Character> {
      char getCharValue();
