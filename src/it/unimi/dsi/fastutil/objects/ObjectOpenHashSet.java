   ...
   private final class SetSpliterator implements ObjectSpliterator<K> {
      ...
      public long estimateSize() {
         return !this.hasSplit
            ? ObjectOpenHashSet.this.size - this.c
            : Math.min(
               ObjectOpenHashSet.this.size - this.c,
               (long)((double)ObjectOpenHashSet.this.realSize() / ObjectOpenHashSet.this.n * (this.max - this.pos)) + (this.mustReturnNull ? 1 : 0)
            );
      }
      ...
      public ObjectOpenHashSet<K>.SetSpliterator trySplit() {
         ...
         int retPos = this.pos;
         int retMax = myNewPos;
         ObjectOpenHashSet<K>.SetSpliterator split = ObjectOpenHashSet.this.new SetSpliterator(retPos, retMax, this.mustReturnNull, true);
         this.pos = myNewPos;
         this.mustReturnNull = false;
         this.hasSplit = true;
         return split;
