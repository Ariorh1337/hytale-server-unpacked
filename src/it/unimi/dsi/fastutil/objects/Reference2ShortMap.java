...
public interface Reference2ShortMap<K> extends Reference2ShortFunction<K>, Map<K, Short> {
   ...
   default void forEach(BiConsumer<? super K, ? super Short> consumer) {
      ObjectSet<Reference2ShortMap.Entry<K>> entrySet = this.reference2ShortEntrySet();
      Consumer<Reference2ShortMap.Entry<K>> wrappingConsumer = entry -> consumer.accept(entry.getKey(), entry.getShortValue());
      if (entrySet instanceof Reference2ShortMap.FastEntrySet) {
         ((Reference2ShortMap.FastEntrySet)entrySet).fastForEach(wrappingConsumer);
      } else {
   ...
   interface Entry<K> extends java.util.Map.Entry<K, Short> {
      short getShortValue();
