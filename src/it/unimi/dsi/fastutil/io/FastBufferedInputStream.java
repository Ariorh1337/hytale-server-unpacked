...
public class FastBufferedInputStream extends MeasurableInputStream implements RepositionableStream {
   ...
   private long skipByReading(long n) throws IOException {
      ...
      while (toSkip > 0L) {
         int len = this.is.read(this.buffer, 0, (int)Math.min(this.buffer.length, toSkip));
         if (len <= 0) {
            break;
         }
   ...
   public int available() throws IOException {
      return (int)Math.min((long)this.is.available() + this.avail, 2147483647L);
   }
   ...
   public void flush() {
      if (this.is != null) {
         this.readBytes = this.readBytes + this.avail;
         this.avail = this.pos = 0;
      }
   }
