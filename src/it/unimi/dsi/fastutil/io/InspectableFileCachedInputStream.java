...
public class InspectableFileCachedInputStream extends MeasurableInputStream implements RepositionableStream, WritableByteChannel {
   ...
   public void clear() throws IOException {
      ...
      this.writePosition = this.position = this.inspectable = 0;
      this.mark = -1L;
   }
   ...
   public int write(ByteBuffer byteBuffer) throws IOException {
      ...
      if (byteBuffer.hasRemaining()) {
         ...
         this.writePosition = this.writePosition + this.fileChannel.write(byteBuffer);
      }
   ...
   public int read(byte[] b, int offset, int length) throws IOException {
      ...
      if (length > 0) {
         ...
         int toRead = (int)Math.min(this.length() - this.position, length);
