...
final class SpdySession {
   ...
   private final Map<Integer, SpdySession.StreamState> activeStreams = new ConcurrentHashMap<>();
   ...
   void acceptStream(int streamId, byte priority, boolean remoteSideClosed, boolean localSideClosed, int sendWindowSize, int receiveWindowSize, boolean remote) {
      if (!remoteSideClosed || !localSideClosed) {
         SpdySession.StreamState state = this.activeStreams
            .put(streamId, new SpdySession.StreamState(priority, remoteSideClosed, localSideClosed, sendWindowSize, receiveWindowSize));
         if (state == null) {
            if (remote) {
   ...
   private SpdySession.StreamState removeActiveStream(int streamId, boolean remote) {
      SpdySession.StreamState state = this.activeStreams.remove(streamId);
      if (state != null) {
         if (remote) {
   ...
   boolean isRemoteSideClosed(int streamId) {
      SpdySession.StreamState state = this.activeStreams.get(streamId);
   ...
   SpdySession.PendingWrite getPendingWrite(int streamId) {
      if (streamId == 0) {
         for (Entry<Integer, SpdySession.StreamState> e : this.activeStreams().entrySet()) {
            SpdySession.StreamState state = e.getValue();
   ...
   private final class StreamComparator implements Comparator<Integer> {
      ...
      public int compare(Integer id1, Integer id2) {
         SpdySession.StreamState state1 = SpdySession.this.activeStreams.get(id1);
         SpdySession.StreamState state2 = SpdySession.this.activeStreams.get(id2);
