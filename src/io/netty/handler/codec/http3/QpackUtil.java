...
final class QpackUtil {
   private static final QpackException PREFIXED_INTEGER_TOO_LONG = QpackException.newStatic(
      QpackDecoder.class, "toIntOrThrow(...)", "QPACK - invalid prefixed integer"
   );
   static final int[] HUFFMAN_CODES = new int[]{
      8184,
      8388568,
      268435426,
      268435427,
   ...
   static final byte[] HUFFMAN_CODE_LENGTHS = new byte[]{
      13,
      23,
      28,
      28,
   ...
   static void encodePrefixedInteger(ByteBuf out, byte mask, int prefixLength, long toEncode) {
      ObjectUtil.checkInRange(toEncode, 0L, 4294967295L, "toEncode");
      ...
      if (toEncode < nbits) {
         ...
         long remainder;
         for (remainder = toEncode - nbits; remainder > 128L; remainder /= 128L) {
            byte next = (byte)(remainder % 128L | 128L);
            out.writeByte(next);
         }
