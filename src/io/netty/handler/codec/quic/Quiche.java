...
final class Quiche {
   ...
   static Exception convertToException(int result, long code) {
      ...
      if (holder == null) {
         ...
         return error == QuicheError.STREAM_RESET ? new QuicStreamResetException(error.message(), code) : new QuicException(error.message());
      } else {
         ...
         if (result == QUICHE_ERR_TLS_FAIL) {
            ...
            return result == QUICHE_ERR_CRYPTO_FAIL ? new SSLException(exception) : exception;
         }
      }
   }
