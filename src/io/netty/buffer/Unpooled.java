...
public final class Unpooled {
   ...
   public static ByteBuf wrappedBuffer(ByteBuffer buffer) {
      ...
      if (PlatformDependent.hasUnsafe()) {
         if (buffer.isReadOnly()) {
            return buffer.isDirect() ? new ReadOnlyUnsafeDirectByteBuf(ALLOC, buffer) : new ReadOnlyByteBufferBuf(ALLOC, buffer);
         } else {
            return new UnpooledUnsafeDirectByteBuf(ALLOC, buffer, buffer.remaining());
         }
      } else {
         return buffer.isReadOnly() ? new ReadOnlyByteBufferBuf(ALLOC, buffer) : new UnpooledDirectByteBuf(ALLOC, buffer, buffer.remaining());
      }
   }
   ...
   public static ByteBuf unmodifiableBuffer(ByteBuf buffer) {
      ...
      return endianness == BIG_ENDIAN ? newReadyOnlyBuffer(buffer) : newReadyOnlyBuffer(buffer.order(BIG_ENDIAN)).order(LITTLE_ENDIAN);
   }
