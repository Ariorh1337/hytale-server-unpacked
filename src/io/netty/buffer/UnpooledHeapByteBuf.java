...
public class UnpooledHeapByteBuf extends AbstractReferenceCountedByteBuf {
   ...
   private int getBytes(int index, GatheringByteChannel out, int length, boolean internal) throws IOException {
      ...
      return out.write((ByteBuffer)((Buffer)tmpBuf).clear().position(index).limit(index + length));
   }

   private int getBytes(int index, FileChannel out, long position, int length, boolean internal) throws IOException {
      this.ensureAccessible();
      ByteBuffer tmpBuf = internal ? this.internalNioBuffer() : ByteBuffer.wrap(this.array);
      return out.write((ByteBuffer)((Buffer)tmpBuf).clear().position(index).limit(index + length), position);
   }
   ...
   public int setBytes(int index, ScatteringByteChannel in, int length) throws IOException {
      ...
      try {
         return in.read((ByteBuffer)((Buffer)this.internalNioBuffer()).clear().position(index).limit(index + length));
   ...
   public int setBytes(int index, FileChannel in, long position, int length) throws IOException {
      ...
      try {
         return in.read((ByteBuffer)((Buffer)this.internalNioBuffer()).clear().position(index).limit(index + length), position);
      } catch (ClosedChannelException ignored) {
         return -1;
      }
   }
