...
class ReadOnlyByteBufferBuf extends AbstractReferenceCountedByteBuf {
   ...
   protected ByteBuf getBytes(int index, byte[] dst, int dstIndex, int length, boolean internal) {
      ...
      ((Buffer)tmpBuf).clear().position(index).limit(index + length);
   ...
   private ByteBuf getBytes(int index, ByteBuffer dst, boolean internal) {
      ...
      ByteBuffer tmpBuf = this.nioBuffer(internal);
      ((Buffer)tmpBuf).clear().position(index).limit(index + dst.remaining());
      dst.put(tmpBuf);
      return this;
   }
   ...
   private ByteBuf getBytes(int index, OutputStream out, int length, boolean internal) throws IOException {
      ...
      if (this.buffer.hasArray()) {
         ...
         ByteBuffer tmpBuf = this.nioBuffer(internal);
         ((Buffer)tmpBuf).clear().position(index);
   ...
   public ByteBuffer nioBuffer(int index, int length) {
      ...
      return (ByteBuffer)((Buffer)this.buffer.duplicate()).position(index).limit(index + length);
   }
   ...
   public ByteBuf duplicate() {
      return new ReadOnlyByteBufferBuf.ReadOnlyDuplicatedByteBuf(this);
   }

   @Override
   public ByteBuf slice(int index, int length) {
      return new ReadOnlyByteBufferBuf.ReadOnlySlicedByteBuf(this, index, length);
   }
