...
public abstract class AbstractByteBuf extends ByteBuf {
   ...
   private static void checkIndexBounds(int readerIndex, int writerIndex, int capacity) {
      if (readerIndex < 0 || readerIndex > writerIndex || writerIndex > capacity) {
         throw new IndexOutOfBoundsException(
            String.format("readerIndex: %d, writerIndex: %d (expected: 0 <= readerIndex <= writerIndex <= capacity(%d))", readerIndex, writerIndex, capacity)
         );
      }
   }
   ...
   final void ensureWritable0(int minWritableBytes) {
      ...
      if (targetCapacity >= 0 & targetCapacity <= this.capacity()) {
         ...
         this.ensureAccessible();
         throw new IndexOutOfBoundsException(
            String.format("writerIndex(%d) + minWritableBytes(%d) exceeds maxCapacity(%d): %s", writerIndex, minWritableBytes, this.maxCapacity, this)
         );
      }
   }
   ...
   public int ensureWritable(int minWritableBytes, boolean force) {
      ...
      if (minWritableBytes > maxCapacity - writerIndex) {
         ...
         int newCapacity = fastWritable >= minWritableBytes
            ...
            : this.alloc().calculateNewCapacity(writerIndex + minWritableBytes, maxCapacity);
         this.capacity(newCapacity);
         return 2;
      }
   }
   ...
   public long getUnsignedInt(int index) {
      return this.getInt(index) & 4294967295L;
   }

   @Override
   public long getUnsignedIntLE(int index) {
      return this.getIntLE(index) & 4294967295L;
   }
   ...
   public CharSequence getCharSequence(int index, int length, Charset charset) {
      return !CharsetUtil.US_ASCII.equals(charset) && !CharsetUtil.ISO_8859_1.equals(charset)
         ? this.toString(index, length, charset)
         : new AsciiString(ByteBufUtil.getBytes(this, index, length, true), false);
   }
   ...
   public long readUnsignedIntLE() {
      return this.readIntLE() & 4294967295L;
   }
   ...
   public int readBytes(FileChannel out, long position, int length) throws IOException {
      this.checkReadableBytes(length);
      int readBytes = this.getBytes(this.readerIndex, out, (long)position, length);
      this.readerIndex += readBytes;
      return readBytes;
   }
   ...
   public int writeBytes(FileChannel in, long position, int length) throws IOException {
      this.ensureWritable(length);
      int writtenBytes = this.setBytes(this.writerIndex, in, (long)position, length);
      if (writtenBytes > 0) {
         this.writerIndex += writtenBytes;
      }
   ...
   public String toString() {
      ...
      StringBuilder buf = new StringBuilder()
         ...
         .append("(ridx: ")
         .append(this.readerIndex)
         .append(", widx: ")
         .append(this.writerIndex)
         .append(", cap: ")
   ...
   final void checkIndex0(int index, int fieldLength) {
      if (checkBounds) {
         checkRangeBoundsTrustedCapacity("index", index, fieldLength, this.capacity());
      }
   }
   ...
   private void checkReadableBytes0(int minimumReadableBytes) {
      ...
      if (checkBounds && this.readerIndex > this.writerIndex - minimumReadableBytes) {
         throw new IndexOutOfBoundsException(
            String.format("readerIndex(%d) + length(%d) exceeds writerIndex(%d): %s", this.readerIndex, minimumReadableBytes, this.writerIndex, this)
         );
      }
   }
