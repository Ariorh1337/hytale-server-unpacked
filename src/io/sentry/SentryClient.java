...
public final class SentryClient implements ISentryClient {
   ...
   private SentryLogEvent processLogEvent(@NotNull SentryLogEvent event, @NotNull List<EventProcessor> eventProcessors) {
      for (EventProcessor processor : eventProcessors) {
         try {
            ...
            this.options
               .getLogger()
               .log(SentryLevel.ERROR, e, "An exception occurred while processing log event by processor: %s", processor.getClass().getName());
         }
   ...
   private SentryTransaction processTransaction(@NotNull SentryTransaction transaction, @NotNull Hint hint, @NotNull List<EventProcessor> eventProcessors) {
      for (EventProcessor processor : eventProcessors) {
         ...
         try {
            ...
            this.options
               .getLogger()
               .log(SentryLevel.ERROR, e, "An exception occurred while processing transaction by processor: %s", processor.getClass().getName());
         }
   ...
   private SentryReplayEvent processReplayEvent(@NotNull SentryReplayEvent replayEvent, @NotNull Hint hint, @NotNull List<EventProcessor> eventProcessors) {
      for (EventProcessor processor : eventProcessors) {
         try {
            ...
            this.options
               .getLogger()
               .log(SentryLevel.ERROR, e, "An exception occurred while processing replay event by processor: %s", processor.getClass().getName());
         }
   ...
   private SentryEvent processFeedbackEvent(@NotNull SentryEvent feedbackEvent, @NotNull Hint hint, @NotNull List<EventProcessor> eventProcessors) {
      for (EventProcessor processor : eventProcessors) {
         try {
            ...
            this.options
               .getLogger()
               .log(SentryLevel.ERROR, e, "An exception occurred while processing feedback event by processor: %s", processor.getClass().getName());
         }
   ...
   public void captureLog(@Nullable SentryLogEvent logEvent, @Nullable IScope scope) {
      ...
      if (logEvent != null) {
         SentryLogEvent tmpLogEvent = logEvent;
         ...
         if (logEvent == null) {
            ...
            long logEventNumberOfBytes = JsonSerializationUtils.byteSizeOf(this.options.getSerializer(), this.options.getLogger(), tmpLogEvent);
   ...
   private <T extends SentryBaseEvent> T applyScope(@NotNull T sentryBaseEvent, @Nullable IScope scope) {
      if (scope != null) {
         ...
         if (sentryBaseEvent.getExtras() == null) {
            ...
            for (Entry<String, Object> item : scope.getExtras().entrySet()) {
   ...
   private SentryTransaction executeBeforeSendTransaction(@NotNull SentryTransaction transaction, @NotNull Hint hint) {
      ...
      if (beforeSendTransaction != null) {
         try {
            ...
            this.options
               .getLogger()
               .log(SentryLevel.ERROR, "The BeforeSendTransaction callback threw an exception. It will be added as breadcrumb and continue.", e);
            transaction = null;
         }
      }
   ...
   private SentryReplayEvent executeBeforeSendReplay(@NotNull SentryReplayEvent event, @NotNull Hint hint) {
      ...
      if (beforeSendReplay != null) {
         try {
            ...
            this.options
               .getLogger()
               .log(SentryLevel.ERROR, "The BeforeSendReplay callback threw an exception. It will be added as breadcrumb and continue.", e);
            event = null;
         }
      }
