      ...
      try {
         ...
         if (callback != null) {
            try {
               ...
               options.getLogger()
                  .log(SentryLevel.ERROR, "The onOversizedEvent callback threw an exception. It will be ignored and automatic reduction will continue.", e);
               reducedEvent = event;
            }
         }
         ...
         if (!isSizeOk(reducedEvent, options)) {
            options.getLogger()
               .log(SentryLevel.WARNING, "Event %s still exceeds size limit after reducing all fields. Event may be rejected by server.", event.getEventId());
         }
