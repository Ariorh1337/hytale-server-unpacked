...
public final class Scope implements IScope {
   ...
   private Scope(@NotNull Scope scope) {
      ...
      for (Entry<String, Object> item : extraRef.entrySet()) {
         if (item != null) {
   ...
   private Breadcrumb executeBeforeBreadcrumb(@NotNull SentryOptions.BeforeBreadcrumbCallback callback, @NotNull Breadcrumb breadcrumb, @NotNull Hint hint) {
      try {
         ...
         this.options
            .getLogger()
            .log(SentryLevel.ERROR, "The BeforeBreadcrumbCallback callback threw an exception. Exception details will be added to the breadcrumb.", e);
   ...
   static Queue<Breadcrumb> createBreadcrumbsList(int maxBreadcrumb) {
      return maxBreadcrumb > 0 ? SynchronizedQueue.synchronizedQueue(new CircularFifoQueue<>(maxBreadcrumb)) : new DisabledQueue<>();
   }
