...
final class HttpConnection {
      ...
      if (optionsProxy != null) {
         ...
         if (port != null && host != null) {
            try {
               ...
               this.options.getLogger().log(SentryLevel.ERROR, e, "Failed to parse Sentry Proxy port: " + optionsProxy.getPort() + ". Proxy is ignored");
            }
         }
      }
   ...
   public TransportResult send(@NotNull SentryEnvelope envelope) throws IOException {
      ...
      try {
         ...
         this.options.getLogger().log(SentryLevel.ERROR, e, "An exception occurred while submitting the envelope to the Sentry server.");
      } finally {
   ...
   private TransportResult readAndLog(@NotNull HttpURLConnection connection) {
      try {
         ...
         this.options.getLogger().log(SentryLevel.ERROR, e, "Error reading and logging the response stream");
      } finally {
