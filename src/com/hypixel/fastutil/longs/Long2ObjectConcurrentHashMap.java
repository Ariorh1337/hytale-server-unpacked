...
public class Long2ObjectConcurrentHashMap<V> {
                        ...
                        while (true) {
                           ...
                           if (stack != null) {
                              ...
                              if ((index += s.length) >= n) {
                                 ...
                                 tab = s.tab;
                                 s.tab = null;
                                 Long2ObjectConcurrentHashMap.TableStack<V> anext = s.next;
                                 s.next = spare;
                                 stack = anext;
                                 spare = s;
                                 continue;
      ...
      while (tab != null && i < tab.length) {
         ...
         if (f == null) {
            ...
            if (f.hash == -1) {
               ...
               synchronized (f) {
                  if (tabAt(tab, i) == f) {
                     for (Long2ObjectConcurrentHashMap.Node<V> p = fh >= 0
                           ? f
                     ...
                     synchronized (f) {
                        if (tabAt(tab, i) == f) {
                           if (fh >= 0) {
                              ...
                              for (Long2ObjectConcurrentHashMap.Node<V> p = f; p != lastRun; p = p.next) {
                                 int ph = p.hash;
                                 long pk = p.key;
                                 V pv = p.val;
                                 if ((ph & n) == 0) {
                              ...
                              for (Long2ObjectConcurrentHashMap.Node<V> e = t.first; e != null; e = e.next) {
                                 int h = e.hash;
                                 Long2ObjectConcurrentHashMap.TreeNode<V> p = new Long2ObjectConcurrentHashMap.TreeNode<>(
                              ...
                              Long2ObjectConcurrentHashMap.Node<V> ln = lc <= 6
                                 ? this.untreeify(lo)
                                 : (hc != 0 ? new Long2ObjectConcurrentHashMap.TreeBin<>(this.EMPTY, lo) : t);
                              Long2ObjectConcurrentHashMap.Node<V> hn = hc <= 6
                                 ? this.untreeify(hi)
   ...
   public V valueMatching(Predicate<V> predicate) {
      ...
      while (next != null || !b) {
                     ...
                     while (true) {
                        ...
                        if (stack != null) {
                           ...
                           if ((index += s.length) >= n) {
                              ...
                              tab = s.tab;
                              s.tab = null;
                              Long2ObjectConcurrentHashMap.TableStack<V> next1 = s.next;
                              s.next = spare;
                              stack = next1;
                              spare = s;
                              continue;
         ...
         next = e;
         Long2ObjectConcurrentHashMap.Node<V> node = e;
         ...
         if (this.root != null && r.right != null) {
            ...
            if (r.left != null && rl.left != null) {
               ...
               try {
                  ...
                  if (replacement != p) {
                     Long2ObjectConcurrentHashMap.TreeNode<V> pp = replacement.parent = p.parent;
                     if (pp == null) {
                        r = replacement;
                     } else if (p == pp.left) {
                        pp.left = replacement;
            ...
            while (true) {
               ...
               label78: {
                  while (true) {
                     ...
                     if ((p = Long2ObjectConcurrentHashMap.tabAt(t, i)) != null && p.hash < 0) {
                        if (p instanceof Long2ObjectConcurrentHashMap.ForwardingNode) {
                           tab = ((Long2ObjectConcurrentHashMap.ForwardingNode)p).nextTable;
                           p = null;
                           Long2ObjectConcurrentHashMap.TableStack<V> s = spare;
