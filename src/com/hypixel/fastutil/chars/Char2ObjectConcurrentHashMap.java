...
public class Char2ObjectConcurrentHashMap<V> {
                        ...
                        while (true) {
                           ...
                           if (stack != null) {
                              ...
                              if ((index += s.length) >= n) {
                                 ...
                                 tab = s.tab;
                                 s.tab = null;
                                 Char2ObjectConcurrentHashMap.TableStack<V> anext = s.next;
                                 s.next = spare;
                                 stack = anext;
                                 spare = s;
                                 continue;
      ...
      while (tab != null && i < tab.length) {
         ...
         if (f == null) {
            ...
            if (f.hash == -1) {
               ...
               synchronized (f) {
                  if (tabAt(tab, i) == f) {
                     for (Char2ObjectConcurrentHashMap.Node<V> p = fh >= 0
                           ? f
                     ...
                     synchronized (f) {
                        if (tabAt(tab, i) == f) {
                           if (fh >= 0) {
                              ...
                              for (Char2ObjectConcurrentHashMap.Node<V> p = f; p != lastRun; p = p.next) {
                                 int ph = p.hash;
                                 char pk = p.key;
                                 V pv = p.val;
                                 if ((ph & n) == 0) {
                              ...
                              for (Char2ObjectConcurrentHashMap.Node<V> e = t.first; e != null; e = e.next) {
                                 int h = e.hash;
                                 Char2ObjectConcurrentHashMap.TreeNode<V> p = new Char2ObjectConcurrentHashMap.TreeNode<>(
                              ...
                              Char2ObjectConcurrentHashMap.Node<V> ln = lc <= 6
                                 ? this.untreeify(lo)
                                 : (hc != 0 ? new Char2ObjectConcurrentHashMap.TreeBin<>(this.EMPTY, lo) : t);
                              Char2ObjectConcurrentHashMap.Node<V> hn = hc <= 6
                                 ? this.untreeify(hi)
   ...
   public V valueMatching(Predicate<V> predicate) {
      ...
      while (next != null || !b) {
                     ...
                     while (true) {
                        ...
                        if (stack != null) {
                           ...
                           if ((index += s.length) >= n) {
                              ...
                              tab = s.tab;
                              s.tab = null;
                              Char2ObjectConcurrentHashMap.TableStack<V> next1 = s.next;
                              s.next = spare;
                              stack = next1;
                              spare = s;
                              continue;
         ...
         next = e;
         Char2ObjectConcurrentHashMap.Node<V> node = e;
         ...
         if (this.root != null && r.right != null) {
            ...
            if (r.left != null && rl.left != null) {
               ...
               try {
                  ...
                  if (replacement != p) {
                     Char2ObjectConcurrentHashMap.TreeNode<V> pp = replacement.parent = p.parent;
                     if (pp == null) {
                        r = replacement;
                     } else if (p == pp.left) {
                        pp.left = replacement;
