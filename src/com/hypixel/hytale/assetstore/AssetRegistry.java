...
public class AssetRegistry {
   ...
   public static int getOrCreateTagIndex(@Nonnull String tag) {
      ...
      try {
         return TAG_MAP.computeIfAbsent(tag.intern(), k -> NEXT_TAG_INDEX.getAndIncrement());
      } finally {
   ...
   public static boolean registerClientTag(@Nonnull String tag) {
      ...
      try {
         return CLIENT_TAG_MAP.put(tag, TAG_MAP.computeIfAbsent(tag, k -> NEXT_TAG_INDEX.getAndIncrement())) == Integer.MIN_VALUE;
      } finally {
         TAG_LOCK.unlockWrite(stamp);
      }
   }
   ...
   public static Object2IntMap<String> getClientTags() {
      ...
      try {
         return new Object2IntOpenHashMap<>(CLIENT_TAG_MAP);
      } finally {
         TAG_LOCK.unlockRead(stamp);
      }
   }
