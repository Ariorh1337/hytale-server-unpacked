...
public abstract class AssetStore<K, T extends JsonAssetWithMap<K, M>, M extends AssetMap<K, T>> {
   ...
   public Set<K> removeAssets(@Nonnull String packKey, boolean all, @Nonnull Collection<K> keys, @Nonnull AssetUpdateQuery assetUpdateQuery) {
      ...
      try {
         ...
         List<Entry<String, Object>> pathsToReload = null;
         if (all) {
      ...
      try {
         ...
         if (this.replaceOnRemove != null && !toBeRemoved.isEmpty()) {
            Map<K, T> replacements = toBeRemoved.stream()
               ...
               .collect(Collectors.toMap(Function.identity(), keyx -> {
                  T replacement = this.replaceOnRemove.apply((K)keyx);
                  Objects.requireNonNull(replacement, "Replacement can't be null!");
                  K replacementKey = this.keyFunction.apply(replacement);
                  if (replacementKey == null) {
   ...
   private void reloadChildrenContainerAssets(@Nonnull String packKey, @Nonnull Map<K, T> loadedAssets) {
      ...
      if (!toReloadTypes.isEmpty()) {
         ...
         for (Entry<Class<? extends JsonAssetWithMap<?, ?>>, Set<Path>> entry : toReloadTypes.entrySet()) {
            AssetStore assetStore = AssetRegistry.getAssetStore((Class<T>)entry.getKey());
            assetStore.loadAssetsFromPaths(packKey, entry.getValue(), query, true);
         }
      }
   }
         ...
         while (!waitingForParent.isEmpty()) {
                        ...
                        () -> {
                           ...
                           try {
                              ...
                              try {
                                 ...
                                 this.logger.at(Level.SEVERE).withCause(e).log("Failed to close asset reader: %s", path);
                              }
            ...
            if (processedAssets == 0) {
               for (Entry<K, RawAsset<K>> entry : waitingForParent.entrySet()) {
                  ...
                  K parentKey = entry.getValue().getParentKey();
      ...
      return CompletableFuture.supplyAsync(() -> {
         ...
         try {
            ...
            try {
               ...
               this.logger.at(Level.SEVERE).withCause(e).log("Failed to close asset reader: %s", this.path);
            }
         }
      ...
      try {
         ...
         if (GithubMessageUtil.isGithub()) {
            ...
            if (e instanceof CodecException codecException) {
               message = codecException.getRawMessage();
         ...
         this.logger.at(Level.SEVERE).withCause(new SkipSentryException(e)).log("Failed to decode asset: %s, %s:\n%s", key, assetPath, reader);
      } catch (Throwable e) {
         this.recordFailedToLoad(failedToLoadKeys, failedToLoadPaths, key, assetPath);
         if (GithubMessageUtil.isGithub()) {
            String pathStr = assetPath == null ? (key == null ? "unknown" : key.toString()) : assetPath.toString();
         ...
         this.logger.at(Level.SEVERE).withCause(e).log("Failed to decode asset: %s, %s", key, assetPath);
      }
