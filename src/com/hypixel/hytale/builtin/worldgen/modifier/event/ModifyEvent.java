...
public interface ModifyEvent<T> extends IEvent<EventType> {
   ...
   static <E extends ModifyEvent<?>> void dispatch(@Nonnull Class<E> type, @Nonnull E event) throws Error {
      try {
         ...
         throw new Error(String.format("Failed to invoke ModifyEvent %s for file %s", event.type(), event.file().getContentPath()), error);
      }
   }
   ...
   class SeedGenerator<K extends SeedStringResource> {
