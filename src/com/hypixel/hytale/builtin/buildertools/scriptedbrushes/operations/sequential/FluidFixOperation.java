   ...
   private void floodFillFluids(BrushConfigEditStore edit, int[][][] blockBuffer, int[][][] fluidBuffer) {
      ...
      for (int bufY = this.bufferHeight - 1; bufY >= 0; bufY--) {
         ...
         while (!queue.isEmpty()) {
            ...
            propagateDown[bx][bz] = true;
            int existingFluidHere = fluidBuffer[bx][bufY][bz];
            if (existingFluidHere != 0) {
               carriedFluid = existingFluidHere;
            } else if (carriedFluid == 0) {
               carriedFluid = this.findNearbyFluid(fluidBuffer, bx, bufY, bz);
            }

            if (carriedFluid != 0) {
               int wx = this.originX - this.halfBufferWidth + bx;
               int wz = this.originZ - this.halfBufferWidth + bz;
               ...
               fluidBuffer[bx][bufY][bz] = carriedFluid;
               resolvedFluidId[bx][bz] = carriedFluid;
            }

            if (bx > 0 && !visited[bx - 1][bz] && blockBuffer[bx - 1][bufY][bz] <= 0) {
               visited[bx - 1][bz] = true;
               queue.add(new FluidFixOperation.FluidFillEntry(bx - 1, bz, carriedFluid));
            }

            if (bx < this.bufferWidth - 1 && !visited[bx + 1][bz] && blockBuffer[bx + 1][bufY][bz] <= 0) {
               visited[bx + 1][bz] = true;
               queue.add(new FluidFixOperation.FluidFillEntry(bx + 1, bz, carriedFluid));
            }

            if (bz > 0 && !visited[bx][bz - 1] && blockBuffer[bx][bufY][bz - 1] <= 0) {
               visited[bx][bz - 1] = true;
               queue.add(new FluidFixOperation.FluidFillEntry(bx, bz - 1, carriedFluid));
            }

            if (bz < this.bufferWidth - 1 && !visited[bx][bz + 1] && blockBuffer[bx][bufY][bz + 1] <= 0) {
               visited[bx][bz + 1] = true;
               queue.add(new FluidFixOperation.FluidFillEntry(bx, bz + 1, carriedFluid));
            }
         }
