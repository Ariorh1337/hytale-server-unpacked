...
public class RevolveOperation extends ToolOperation {
   ...
   public RevolveOperation(
      ...
      if (selection == null) {
         ...
         if (useDistance) {
            customDistance = (Integer)this.args.tool().getOrDefault("fDistance", 20);
         }
                  ...
                  for (int x = localXMin; x <= localXMax; x++) {
                     for (int z = localZMin; z <= localZMax; z++) {
                        for (int y = yMax; y >= yMin; y--) {
                           ...
                           if (block != 0
                              && filler == 0
                              && (this.edit.getBlockMask() == null || !this.edit.getBlockMask().isExcluded(accessor, x, y, z, min, max, block, 0))) {
                              this.currentSelection.addBlockAtWorldPos(x, y, z, block, rotation, filler, support, holder);
                           }

                           if (fluid != 0 && (this.edit.getBlockMask() == null || !this.edit.getBlockMask().isExcluded(accessor, x, y, z, min, max, 0, fluid))) {
         ...
         if (!fullRevolve && copyEntities) {
            ...
            if (totalEntities <= 1000) {
               for (Ref<EntityStore> e : entities) {
                  ...
                  this.currentSelection.addEntityFromWorld(holder);
                  entityRefs.add(e);
               }
            } else {
         ...
         if (customDistance > 0) {
            ...
            if (naturalDist > 0.0) {
               double scale = customDistance / naturalDist;
