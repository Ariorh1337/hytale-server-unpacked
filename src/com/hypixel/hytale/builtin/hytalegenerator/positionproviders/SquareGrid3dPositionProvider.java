...
public class SquareGrid3dPositionProvider extends PositionProvider {
   ...
   public void generate(@NonNullDecl PositionProvider.Context context) {
      this.rGridBounds.min.set(Math.floor(context.bounds.min.x), Math.floor(context.bounds.min.y), Math.floor(context.bounds.min.z));
      this.rGridBounds.max.set(Math.ceil(context.bounds.max.x), Math.ceil(context.bounds.max.y), Math.ceil(context.bounds.max.z));
