...
public interface BiomeEditor {
      ...
      return AssetPackUtil.getOrCreatePack(config.group, config.pack)
         ...
         .thenApplyAsync(
            SneakyThrow.sneakyFunction(
               context -> {
                  LOGGER.atInfo().log("Creating Biome %s with Template %s", context.config.name, context.config.biomeTemplate);
         ...
         .thenApplyAsync(
            SneakyThrow.sneakyFunction(
               context -> {
                  LOGGER.atInfo().log("Creating WorldStructure %s with Template %s", config.name, config.worldStructureTemplate);
                  createAssetFromTemplate(
         ...
         .thenComposeAsync(context -> {
            LOGGER.atInfo().log("Creating Instance %s", config.name);
         ...
         .whenComplete((context, error) -> {
            if (error != null) {
               LOGGER.atWarning().withCause(error).log("Error occurred whilst setting up assets");
            }
         });
   }
   ...
   static CompletableFuture<Void> setupViewport(@Nonnull CompletableFuture<BiomeEditor.Context> future, @Nonnull Ref<EntityStore> ref) {
      ...
      return future.<Viewport>thenComposeAsync(
            context -> {
               ...
               LOGGER.atInfo().log("Creating world Instance %s", context.config.name);
               CompletableFuture<World> instance = InstancesPlugin.get()
                  .spawnInstance(context.config.name, world, new Transform(new Vector3d(transform.getPosition()), new Rotation3f(transform.getRotation())));
               LOGGER.atInfo().log("Teleporting player to Instance %s", context.config.name);
   ...
   class Context implements Function<Object, BiomeEditor.Context> {
   ...
   class Defaults {
      ...
      protected static <T extends JsonAssetWithMap<String, M>, M extends AssetMap<String, T>> String[] resolveAssetTags(
         ...
         M assetMap = store.getAssetMap();
         Set<String> uniqueIds = tags.length == 0 ? assetMap.getAssetMap().keySet() : new ObjectOpenHashSet<>();
