...
public class PondFillerProp extends Prop {
   ...
   private List<Vector3i> renderFluidBlocks(@Nonnull Vector3i origin, @Nonnull VoxelSpace<Material> materialSpace) {
      ...
      for (int var30 = min.y + 1; var30 < max.y; var30++) {
         ...
         for (int x = min.x; x < max.x; x++) {
            for (int z = min.z; z < max.z; z++) {
               if (!isTraversed(mask.get(x, var30, z))) {
                  int maskValueUnder = mask.get(x, underY, z);
                  Material material = materialSpace.get(x, var30, z);
                  int contextMaterialHash = material.hashMaterialIds();
                  if (this.solidSet.test(contextMaterialHash)) {
                     int maskValue = 0;
                     maskValue |= 1;
                     maskValue |= 256;
                     mask.set(maskValue, x, var30, z);
                  } else if (isLeaks(maskValueUnder) || x == min.x || x == max.x - 1 || z == min.z || z == max.z - 1) {
                     ArrayDeque<Vector3i> stack = new ArrayDeque<>();
                     stack.push(new Vector3i(x, var30, z));
                     mask.set(4096, x, var30, z);
      ...
      for (int var31 = maskBounds.min.y + 1; var31 < maskBounds.max.y; var31++) {
         for (int x = maskBounds.min.x + 1; x < maskBounds.max.x - 1; x++) {
            for (int z = maskBounds.min.z + 1; z < maskBounds.max.z - 1; z++) {
               int maskValue = mask.get(x, var31, z);
               if (!isSolid(maskValue) && !isLeaks(maskValue)) {
                  fluidBlocks.add(new Vector3i(x, var31, z));
               }
            }
         }
      }
         ...
         for (Vector3i position : fluidBlocks) {
            if (context.materialWriteSpace.getBounds().contains(position.x, position.y, position.z)) {
               MaterialProvider.Context materialsContext = new MaterialProvider.Context(
                  position, 0.0, 0, 0, 0, 0, null, context.distanceToBiomeEdge, null, null
               );
