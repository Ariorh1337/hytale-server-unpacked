...
public class PortalDeviceActivePage extends InteractiveCustomUIPage<PortalDeviceActivePage.Data> {
   ...
   public void build(
      ...
      if (state != PortalDeviceActivePage.Error.INVALID_BLOCK) {
         ...
         if (state == PortalDeviceActivePage.Error.PORTAL_LOADING) {
            ...
            commandBuilder.set("#PortalTitle.TextSpans", Message.translation("server.customUI.portalDevice.portalTitle").param("name", var18.getDisplayName()));
            commandBuilder.set("#PortalDescription.TextSpans", PortalDeviceSummonPage.createDescription(var18, portalWorld.getTimeLimitSeconds()));
   ...
   private enum Error implements PortalDeviceActivePage.State {
      INVALID_BLOCK,
      INVALID_WORLD,
