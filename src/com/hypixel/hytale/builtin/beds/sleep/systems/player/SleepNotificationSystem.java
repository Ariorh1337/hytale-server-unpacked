...
public class SleepNotificationSystem extends DelayedSystem<EntityStore> {
      ...
      if (CanSleepInWorld.check(world).isNegative()) {
         ...
         if (worldSleep instanceof WorldSlumber) {
            ...
            if (playerCount <= 4) {
               ...
               return ready ? new SleepNotificationSystem.SmallServer(readyToSleep, playerCount) : SleepNotificationSystem.NotReady.INSTANCE;
            } else {
               double sleepersRatio = (double)readyToSleep / playerCount;
               boolean ready = sleepersRatio >= 0.5;
               return ready ? new SleepNotificationSystem.BigServer(sleepersRatio, readyToSleep, playerCount) : SleepNotificationSystem.NotReady.INSTANCE;
            }
         }
      }
   }
   ...
   private enum NotReady implements SleepNotificationSystem.NotificationState {
      INSTANCE;
