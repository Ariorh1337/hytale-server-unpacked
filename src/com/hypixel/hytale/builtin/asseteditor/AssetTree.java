...
public class AssetTree {
   ...
   public boolean isDirectoryEmpty(@Nonnull Path path) {
      ...
      try {
         ...
         boolean hasFile = assets.get(fileIndex).path.startsWith(pathString + "/");
         return !hasFile;
      } finally {
         this.lock.unlockRead(stamp);
      }
   ...
   private void load(@Nonnull Collection<AssetTypeHandler> assetTypes) {
      try {
         ...
         LOGGER.at(Level.WARNING).withCause(e).log("Failed to load server asset tree!");
      }
      ...
      try {
         ...
         LOGGER.at(Level.WARNING).withCause(e).log("Failed to load common asset tree!");
      }
