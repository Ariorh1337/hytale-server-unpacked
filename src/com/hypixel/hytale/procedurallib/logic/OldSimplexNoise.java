...
public class OldSimplexNoise implements NoiseFunction {
   ...
   public double get(int seed, int offsetSeed, double x, double y, double z) {
      ...
      if (inSum <= 1.0) {
         ...
         if (attn2 > 0.0) {
            attn2 *= attn2;
            value += attn2 * attn2 * extrapolate(offsetSeed, xsb + 0, ysb + 1, zsb + 0, dx2, dy2, dz2);
         }
         ...
         if (attn3 > 0.0) {
            attn3 *= attn3;
            value += attn3 * attn3 * extrapolate(offsetSeed, xsb + 0, ysb + 0, zsb + 1, dx3, dy3, dz3);
         }
      } else if (inSum >= 2.0) {
         byte aPoint = 6;
         double aScore = xins;
         ...
         if (attn2 > 0.0) {
            attn2 *= attn2;
            value += attn2 * attn2 * extrapolate(offsetSeed, xsb + 1, ysb + 0, zsb + 1, dx2, dy2, dz2);
         }
         ...
         if (attn1 > 0.0) {
            attn1 *= attn1;
            value += attn1 * attn1 * extrapolate(offsetSeed, xsb + 0, ysb + 1, zsb + 1, dx1, dy1, dz1);
         }
         ...
         if (attn5 > 0.0) {
            attn5 *= attn5;
            value += attn5 * attn5 * extrapolate(offsetSeed, xsb + 1, ysb + 0, zsb + 1, dx5, dy5, dz5);
         }
         ...
         if (attn6 > 0.0) {
            attn6 *= attn6;
            value += attn6 * attn6 * extrapolate(offsetSeed, xsb + 0, ysb + 1, zsb + 1, dx6, dy6, dz6);
         }
      }
