...
class QuicheConnection {
      ...
      try (Arena arena = Arena.ofConfined()) {
         ...
         while (!thread.isInterrupted()) {
            ...
            while ((streamId = QuicheNative.connStreamReadableNext(this.connHandle)) != -1L) {
               ...
               if (stream.isNew) {
                  LOGGER.atInfo().log("Stream %d created", (long)streamId);
               ...
               while (QuicheNative.connStreamReadable(this.connHandle, streamId)) {
                  while (true) {
                     ...
                     if (read < 0L) {
                        if (read != -1L) {
                           LOGGER.atSevere().log("Error reading stream %d: %s (%d)", streamId, QuicheNative.QuicheError.fromCode((int)read), read);
                        }
                        break;
                     }
                     ...
                     try {
                        ...
                        LOGGER.at(Level.SEVERE).withCause(e).log("Exception processing packet on stream %d", (long)streamId);
               ...
               if (QuicheNative.connStreamFinished(this.connHandle, streamId)) {
                  LOGGER.atInfo().log("Stream %d finished", (long)streamId);
                  QuicheChannel finished = this.streams.remove(streamId);
                  if (finished != null) {
                     finished.close();
                  }
               }
         ...
         if (!QuicheNative.connIsClosed(this.connHandle)) {
            QuicheChannel gameStream = this.streams.get(0L);
            if (gameStream != null) {
         ...
         LOGGER.atInfo().log("Connection %s shutting down", QuicheChannel.formatDcid(this.dcid));
      } catch (Exception e) {
         LOGGER.at(Level.SEVERE).withCause(e).log("Unhandled exception in connection %s processing", QuicheChannel.formatDcid(this.dcid));
         ...
         try {
            ...
            LOGGER.at(Level.WARNING).withCause(cleanup).log("Error during connection close cleanup for %s", QuicheChannel.formatDcid(this.dcid));
         }
      } finally {
         for (QuicheChannel stream : this.streams.values()) {
            try {
               if (stream.handler != null) {
                  stream.handler.logCloseMessage();
               }
            } catch (Exception e) {
               LOGGER.at(Level.WARNING).withCause(e).log("Error logging close message for stream %d", (long)stream.getStreamId());
            }

            try {
               stream.close();
            } catch (Exception e) {
               LOGGER.at(Level.WARNING).withCause(e).log("Error closing stream %d", (long)stream.getStreamId());
            }
         }

         this.active.set(false);

         QuicheListener.DatagramBuffer remaining;
   ...
   private void initChannel(QuicheChannel stream) {
      ...
      if (streamId == 0L) {
         ...
         if (auxHandler == null) {
            LOGGER.atWarning().log("No aux stream handler configured, closing stream %d", (long)streamId);
            ...
            if (primaryStream != null && primaryStream.handler != null) {
               ...
               LOGGER.atWarning().log("Primary stream not ready, closing aux stream %d", (long)streamId);
               QuicheNative.connStreamShutdown(this.connHandle, streamId, 0, 0L);
               QuicheNative.connStreamShutdown(this.connHandle, streamId, 1, 0L);
            }
         }
      } else {
         LOGGER.atWarning().log("Unexpected stream type for stream %d (low bits: %d), ignoring", (long)streamId, (long)(streamId & 3L));
      }
   }
   ...
   private void logCloseReason(Arena arena) {
      ...
      if (QuicheNative.connIsTimedOut(this.connHandle)) {
         LOGGER.atInfo().log("Connection %s closed: timed out", dcidHex);
      } else {
         ...
         if (QuicheNative.connPeerError(this.connHandle, isApp, errCode, reason, reasonLen)) {
            ...
            LOGGER.atInfo().log("Connection %s closed by peer: %s error code %d%s", dcidHex, app ? "application" : "transport", code, reasonStr);
         } else if (QuicheNative.connLocalError(this.connHandle, isApp, errCode, reason, reasonLen)) {
            long code = errCode.get(ValueLayout.JAVA_LONG, 0L);
            boolean app = isApp.get(ValueLayout.JAVA_BOOLEAN, 0L);
            LOGGER.atInfo().log("Connection %s closed locally: %s error code %d", dcidHex, app ? "application" : "transport", code);
         } else {
            LOGGER.atInfo().log("Connection %s closed (reason unknown)", dcidHex);
         }
      }
   }
   ...
   private void drainSendQueue() {
      try (Arena drainArena = Arena.ofConfined()) {
         ...
         LOGGER.atWarning().log("Failed to drain send queue for %s: %s", QuicheChannel.formatDcid(this.dcid), e.getMessage());
      }
   }
   ...
   public void submit(SocketAddress source, QuicheListener.DatagramBuffer buf) {
      if (!this.incomingBuffers.offer(buf)) {
         ...
         LOGGER.atInfo().log("%s: isn't processing packets fast enough, discarding", QuicheChannel.formatDcid(this.dcid));
      } else {
   ...
   QuicheChannel createChannel(NetworkChannel networkChannel) {
      if (networkChannel != NetworkChannel.Default && networkChannel != NetworkChannel.Voice) {
         ...
         QuicheChannel channel = this.streams.computeIfAbsent(streamId, id -> new QuicheChannel(this, id, networkChannel));
         this.writableStreams.add(streamId);
         return channel;
      } else {
