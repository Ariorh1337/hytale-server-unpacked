...
public final class TelemetryStorage {
   ...
   public TelemetryStorage(@Nonnull String sessionId) {
      ...
      try {
         ...
         LOGGER.at(Level.WARNING).withCause(e).log("Failed to create telemetry directory");
      }
      ...
      try {
         ...
         LOGGER.at(Level.WARNING).withCause(e).log("Failed to create telemetry session file");
      }
   }
      ...
      synchronized (this.writerLock) {
         if (this.sessionWriter != null) {
            try {
               ...
               LOGGER.at(Level.WARNING).withCause(e).log("Failed to write telemetry packet to local storage");
            }
         }
      }
   }
   ...
   private void performMaintenance() {
      try {
         ...
         LOGGER.at(Level.WARNING).withCause(e).log("Failed to perform telemetry storage maintenance");
      }
   }
      ...
      try {
         ...
         if (lastModified.isBefore(cutoffTime)) {
            try {
               ...
               LOGGER.at(Level.WARNING).withCause(e).log("Failed to delete expired telemetry file: %s", fileName);
            }
         ...
         LOGGER.at(Level.WARNING).withCause(e).log("Failed to process telemetry file during maintenance");
      }
   }
   ...
   private static void compressFile(@Nonnull Path sourcePath) {
      ...
      if (!Files.exists(compressedPath)) {
         try {
            ...
            LOGGER.at(Level.WARNING).withCause(e).log("Failed to compress telemetry file: %s", sourcePath.getFileName());
         }
      }
   }
   ...
   public void closeAndCompress() {
      synchronized (this.writerLock) {
         try {
            ...
            LOGGER.at(Level.WARNING).withCause(e).log("Failed to close and compress telemetry session file");
         }
      }
   }
}
