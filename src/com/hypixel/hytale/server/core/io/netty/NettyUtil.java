...
public class NettyUtil {
   ...
   public static EventLoopGroup getEventLoopGroup(int nThreads, String name) {
      ...
      if (Epoll.isAvailable()) {
         ...
         return KQueue.isAvailable() ? new KQueueEventLoopGroup(nThreads, factory) : new NioEventLoopGroup(nThreads, factory);
      }
   }
         ...
         if (universe != null) {
            if (newLevel == Level.OFF) {
               for (PlayerRef p : universe.getPlayers()) {
                  if (p.getPacketHandler().getChannel() instanceof NettyUtil.NettyChannelConnection(Channel channel1)) {
                     uninjectLogger(channel1);
                  }
               }
            } else {
               for (PlayerRef p : universe.getPlayers()) {
                  if (p.getPacketHandler().getChannel() instanceof NettyUtil.NettyChannelConnection(Channel channel1)) {
                     injectLogger(channel1);
                  }
               }
            }
         }
   ...
   public record NettyChannelConnection(Channel channel) implements ChannelConnection {
      ...
      public X509Certificate getClientCertificate() {
         return this.channel.attr(QUICTransport.CLIENT_CERTIFICATE_ATTR).get();
      }
      ...
      public void logConnectionTimings(@Nonnull String message, @Nonnull Level level) {
         ...
         Long before = this.channel.attr(LOGIN_START_KEY).getAndSet(now);
         if (before == null) {
      ...
      public boolean equals(Object obj) {
         if (this == obj) {
            ...
            return obj instanceof NettyUtil.NettyChannelConnection(Channel channel1) ? this.channel.equals(channel1) : false;
         }
      }
