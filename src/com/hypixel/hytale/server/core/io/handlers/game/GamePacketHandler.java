...
public class GamePacketHandler extends GenericPacketHandler implements IPacketHandler {
   ...
   protected void registerHandlers() {
      ...
      this.registerHandler(246, p -> this.handle((CreateUserMarker)p));
   ...
   public void handle(@Nonnull RemoveMapMarker packet) {
      ...
      if (ref != null && ref.isValid()) {
         Store<EntityStore> store = ref.getStore();
         World world = store.getExternalData().getWorld();
         world.execute(() -> world.getWorldMapManager().handleUserRemoveMarker(this.playerRef, packet));
      }
   }
   ...
   public void handle(@Nonnull UpdateServerAccess packet) {
      ...
      for (HostAddress host : packet.hosts) {
         publicAddresses.add(InetSocketAddress.createUnresolved(host.host, host.port & 65535));
      }
   ...
   public void handle(@Nonnull CreateUserMarker packet) {
      Ref<EntityStore> ref = this.playerRef.getReference();
      if (ref != null && ref.isValid()) {
         Store<EntityStore> store = ref.getStore();
         World world = store.getExternalData().getWorld();
