...
public class ServerFileBrowser {
   ...
   private List<FileListProvider.FileEntry> buildDirectoryListing() {
      ...
      try (DirectoryStream<Path> stream = Files.newDirectoryStream(path)) {
         ...
         LOGGER.atSevere().withCause(e).log("Error listing directory: %s", path);
      }
   ...
   private List<FileListProvider.FileEntry> buildSearchResults() {
      ...
      try {
         ...
         LOGGER.atSevere().withCause(e).log("Error walking directory: %s", this.root);
      }
            ...
            if (packSubPath != null) {
               ...
               if (targetDir != null && Files.isDirectory(targetDir)) {
                  try (DirectoryStream<Path> stream = Files.newDirectoryStream(targetDir)) {
                     ...
                     LOGGER.atSevere().withCause(e).log("Error listing asset pack directory: %s", targetDir);
                  }
               }
            }
         }
   ...
   private void searchInAssetPackDirectory(
      ...
      try {
         ...
         LOGGER.atSevere().withCause(e).log("Error searching asset pack directory: %s", searchRoot);
      }
   }
