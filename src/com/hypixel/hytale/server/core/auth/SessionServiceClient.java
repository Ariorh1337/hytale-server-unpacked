...
public class SessionServiceClient {
         ...
         () -> {
            try {
               ...
               if (response.statusCode() != 200) {
                  LOGGER.at(Level.WARNING).log("Failed to request authorization grant: HTTP %d - %s", (int)response.statusCode(), response.body());
                  return null;
               } else {
      ...
      return CompletableFuture.supplyAsync(
         () -> {
            try {
               ...
               HttpResponse<String> response = this.httpClient.send(request, BodyHandlers.ofString());
               if (response.statusCode() != 200) {
                  LOGGER.at(Level.WARNING).log("Failed to exchange auth grant: HTTP %d - %s", (int)response.statusCode(), response.body());
                  return null;
               } else {
   ...
   public SessionServiceClient.JwksResponse getJwks() {
      try {
         ...
         HttpResponse<String> response = this.httpClient.send(request, BodyHandlers.ofString());
         if (response.statusCode() != 200) {
            LOGGER.at(Level.WARNING).log("Failed to fetch JWKS: HTTP %d - %s", (int)response.statusCode(), response.body());
            return null;
         } else {
            ...
            if (jwks != null && jwks.keys != null && jwks.keys.length != 0) {
               LOGGER.at(Level.INFO).log("Successfully fetched JWKS with %d keys", (int)jwks.keys.length);
               return jwks;
            } else {
   ...
   public SessionServiceClient.GameProfile[] getGameProfiles(@Nonnull String oauthAccessToken) {
      try {
         ...
         HttpResponse<String> response = this.httpClient.send(request, BodyHandlers.ofString());
         if (response.statusCode() != 200) {
            LOGGER.at(Level.WARNING).log("Failed to fetch profiles: HTTP %d - %s", (int)response.statusCode(), response.body());
            return null;
         } else {
            ...
            if (data != null && data.profiles != null) {
               LOGGER.at(Level.INFO).log("Found %d game profile(s)", (int)data.profiles.length);
   ...
   public SessionServiceClient.GameSessionResponse createGameSession(@Nonnull String oauthAccessToken, @Nonnull UUID profileUuid) {
      try {
         ...
         if (response.statusCode() != 200 && response.statusCode() != 201) {
            LOGGER.at(Level.WARNING).log("Failed to create game session: HTTP %d - %s", (int)response.statusCode(), response.body());
            return null;
         } else {
      ...
      if (sessionToken != null && !sessionToken.isEmpty()) {
         try {
            ...
            if (response.statusCode() != 200 && response.statusCode() != 204) {
               LOGGER.at(Level.WARNING).log("Failed to terminate session: HTTP %d - %s", (int)response.statusCode(), response.body());
            } else {
