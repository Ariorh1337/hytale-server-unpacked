...
public class PathWatcherThread implements Runnable {
   ...
   public final void run() {
      try {
         while (!Thread.interrupted()) {
            ...
            for (WatchEvent<?> event : key.pollEvents()) {
               ...
               if (kind == StandardWatchEventKinds.OVERFLOW) {
                  ...
                  Path path = directory.resolve(pathEvent.context());
                  if (!HAS_FILE_TREE_SUPPORT && pathEvent.kind() == StandardWatchEventKinds.ENTRY_CREATE && Files.isDirectory(path)) {
                     this.addPath(path);
                  }
         ...
         LOGGER.at(Level.SEVERE).withCause(t).log("Exception occurred when polling:");
      }
