...
public class FileChangeTask implements Runnable {
   ...
   public void run() {
      try {
         ...
         LOGGER.at(Level.SEVERE).withCause(new SkipSentryException(e)).log("The asset file '%s' was deleted before we could load/update it!", this.path);
      } catch (Throwable e) {
         LOGGER.at(Level.SEVERE).withCause(e).log("Failed to handle file change %s", this.path);
      }
   }
