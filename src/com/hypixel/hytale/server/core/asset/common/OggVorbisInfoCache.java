...
public class OggVorbisInfoCache {
   ...
   static OggVorbisInfoCache.OggVorbisInfo parse(byte[] bytes) {
      ...
      if (id >= 0 && id + 16 <= len) {
         ...
         int sampleRate = segment.get(MemorySegmentUtil.INT_LE, id + 12);
         double duration = -1.0;
         if (sampleRate > 0) {
            for (int ix = Math.max(0, len - 14); ix >= 0; ix--) {
               if (bytes[ix] == 79 && segment.get(MemorySegmentUtil.INT_BE, ix) == 1332176723) {
                  int headerType = Byte.toUnsignedInt(bytes[ix + 5]);
                  if ((headerType & 4) != 0) {
                     long granule = segment.get(MemorySegmentUtil.LONG_LE, ix + 6);
                     if (granule >= 0L) {
