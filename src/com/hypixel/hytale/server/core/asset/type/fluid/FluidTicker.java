         ...
         for (Vector2i offset : ORTO_OFFSETS) {
            ...
            if (otherFluidSection != null && otherBlockSection != null) {
               ...
               if (supportId != Integer.MIN_VALUE && otherFluid == supportId) {
                  BlockType sourceBlock = blockMap.getAsset(otherBlockSection.get(blockX, blockY, blockZ));
                  if (sourceBlock != null) {
                     int sourceRotation = otherBlockSection.getRotationIndex(blockX, blockY, blockZ);
                     int sourceFiller = otherBlockSection.getFiller(blockX, blockY, blockZ);
                     if (!blocksFluidFrom(sourceBlock, sourceRotation, x, z, sourceFiller) && !blocksFluidFrom(thisBlock, thisRotation, -x, -z, thisFiller)) {
                        return FluidTicker.AliveStatus.ALIVE;
                     }
                  }
               } else {
   ...
   public static boolean blocksFluidFrom(@Nonnull BlockType blockType, int rotationIndex, int offsetX, int offsetZ, int filler) {
      ...
      if (!hitboxAsset.protrudesUnitBox() && filler == 0) {
         ...
         for (Box box : detailBoxes) {
            double clampedMinX = Math.max(box.min.x, fillerX);
            double clampedMaxX = Math.min(box.max.x, fillerX + 1);
            double clampedMinY = Math.max(box.min.y, fillerY);
            double clampedMaxY = Math.min(box.max.y, fillerY + 1);
            double clampedMinZ = Math.max(box.min.z, fillerZ);
            double clampedMaxZ = Math.min(box.max.z, fillerZ + 1);
