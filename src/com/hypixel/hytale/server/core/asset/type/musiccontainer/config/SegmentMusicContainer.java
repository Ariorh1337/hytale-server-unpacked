...
public class SegmentMusicContainer extends MusicContainer {
   @Nonnull
   public static final BuilderCodec<SegmentMusicContainer> CODEC = BuilderCodec.<MusicContainer>builder(
                  ...
                  for (LayerPlacement layer : segment.layers) {
                     if (layer.name != null && !layer.name.isBlank()) {
                        if (!layerNames.add(layer.name)) {
                           results.fail("Duplicate layer name '" + layer.name + "' in Layers array");
                        }

                        if (layer.clipStart != null) {
                           float clipStartMs = layer.clipStart.toMs(bpm, bpb);
                           if (clipStartMs > 0.0F) {
                              results.fail(
                                 "Layer '"
                                    + layer.name
