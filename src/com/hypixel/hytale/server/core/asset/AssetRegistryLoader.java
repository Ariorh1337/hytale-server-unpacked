...
public class AssetRegistryLoader {
   ...
   private static void preLoadAssets0(@Nonnull LoadAssetEvent event) {
      ...
      LOGGER.at(Level.INFO).log("Loading %s asset stores...", (int)values.size());
         ...
         try {
            while (iterator.hasNext()) {
               ...
               try {
                  ...
                  LOGGER.at(Level.SEVERE).withCause(e).log("Failed to pre-add %s took %s", assetClass.getSimpleName(), FormatUtil.nanosToString(diff));
               }
            }
