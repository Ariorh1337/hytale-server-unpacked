...
public class ChangeActiveSlotInteraction extends Interaction {
   ...
   @Nonnull
   public static final BuilderCodec<ChangeActiveSlotInteraction> CODEC = BuilderCodec.<Interaction>builder(
      ...
      if (!firstRun) {
         ...
         if (hotbarComponent != null) {
            ...
            if (this.targetSlot == Integer.MIN_VALUE) {
               slot = metaStore.getMetaObject(TARGET_SLOT).byteValue();
            } else {
               if (hotbarComponent.getActiveSlot() == this.targetSlot) {
                  context.getState().state = InteractionState.Finished;
                  return;
               }
            ...
            hotbarComponent.setActiveSlot(slot, ref, commandBuffer);
