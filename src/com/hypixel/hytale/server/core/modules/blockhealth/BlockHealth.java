...
public class BlockHealth implements Cloneable {
   ...
   public void deserialize(@Nonnull MemorySegment data, int offset, byte version) {
      this.health = data.get(MemorySegmentUtil.FLOAT_BE, offset);
      this.lastDamageGameTime = Instant.ofEpochMilli(data.get(MemorySegmentUtil.LONG_BE, offset + 4));
   }

   public void serialize(@Nonnull MemorySegment data, int offset) {
      data.set(MemorySegmentUtil.FLOAT_BE, offset, this.health);
      data.set(MemorySegmentUtil.LONG_BE, offset + 4, this.lastDamageGameTime.toEpochMilli());
   }
