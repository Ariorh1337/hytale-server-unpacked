...
public class I18nModule extends JavaPlugin {
   ...
   private void loadBundledDefaultsFromClasspath() {
      ...
      try {
         ...
         this.getLogger().at(Level.SEVERE).withCause(e).log("Failed to enumerate %s resources", "bundledDefaults/");
      }
   }
   ...
   private void loadBundledDefaultsFromUrl(@Nonnull Map<String, String> messages, @Nonnull URL url) {
      try {
         ...
         this.getLogger().at(Level.SEVERE).withCause(new SkipSentryException(e)).log("Failed to load bundled defaults from %s", url);
      }
   }
   ...
   private void loadMessagesFromPack(AssetPack pack) {
      ...
      if (Files.isDirectory(languagesPath)) {
         ...
         try (DirectoryStream<Path> directoryStream = Files.newDirectoryStream(languagesPath, x$0 -> Files.isDirectory(x$0))) {
            ...
            this.getLogger().at(Level.SEVERE).withCause(e).log("Failed to load languages from: %s", languagesPath);
         }
      }
      ...
      if (Files.exists(fallbackPath)) {
         ...
         try {
            ...
            this.getLogger().at(Level.SEVERE).withCause(e).log("Failed to load fallback languages from: %s", fallbackPath);
         }
      }
   }
