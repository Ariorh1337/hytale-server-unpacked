...
public class VoiceModule extends JavaPlugin {
   ...
   protected void start() {
      ...
      this.getLogger().at(Level.INFO).log("[Voice] Started position cache update task (%dms interval)", (long)100L);
   }
   ...
   public void setMaxHearingDistance(float distance) {
      ...
      if (oldDistance != distance) {
         this.getLogger().at(Level.INFO).log("[Voice] Max hearing distance changed: %.1f -> %.1f", (float)oldDistance, (float)distance);
         this.broadcastConfigToAllPlayers();
         this.config.save();
      }
   }
   ...
   public void setReferenceDistance(float distance) {
      ...
      if (oldDistance != distance) {
         this.getLogger().at(Level.INFO).log("[Voice] Full volume distance changed: %.1f -> %.1f", (float)oldDistance, (float)distance);
         this.broadcastConfigToAllPlayers();
         this.config.save();
      }
   }
