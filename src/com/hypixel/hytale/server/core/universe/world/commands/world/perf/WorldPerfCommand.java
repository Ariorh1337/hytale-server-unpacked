            ...
            if (showDelta) {
               ...
               msg.insert(
                  Message.translation("server.commands.world.perf.tpsTime")
                     .param("time", length)
                     .param("tps", FormatUtil.simpleFormat(min, average, max, d1 -> tpsFromDelta(d1, tickStepNanos), 2))
                     .insert("\n")
               );
            }
         }
            ...
            if (showDelta) {
               ...
               msg.insert(
                  Message.translation("server.commands.world.perf.tpsMinMaxMetric")
                     .param("time", length)
                     .param("min", tpsFromDelta(max, tickStepNanos))
                     .param("avg", tpsFromDelta(average, tickStepNanos))
                     .param("max", tpsFromDelta(min, tickStepNanos))
                     .insert("\n")
               );
            }
