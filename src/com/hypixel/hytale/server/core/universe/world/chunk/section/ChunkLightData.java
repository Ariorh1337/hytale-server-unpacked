...
public class ChunkLightData {
   ...
   protected static short getTraverse(@Nonnull MemorySegment local, int index, int pointer, int depth) {
      ...
      try {
         ...
         byte mask = local.get(VL_BYTE, position);
         int innerIndex = index >> 12 - depth & 7;
         loc = innerIndex * 2 + position + 1;
         result = Short.toUnsignedInt(local.get(VL_SHORT, loc));
   ...
   public int serialize(@Nonnull MemorySegment data, int offset) {
      data.set(MemorySegmentUtil.SHORT_BE, offset, this.changeId);
      ...
      if (hasLight) {
         ...
         data.set(MemorySegmentUtil.INT_BE, offset + 2 + 1, size);
         return 7 + size;
      } else {
         return 3;
      }
   ...
   private int serializeOctree(@Nonnull MemorySegment data, int baseOffset, int position) {
      ...
      data.set(ValueLayout.JAVA_BYTE, offset, (byte)mask);
      offset++;
      ...
      for (int i = 0; i < 8; i++) {
         ...
         if ((mask >> i & 1) == 1) {
            ...
            data.set(MemorySegmentUtil.SHORT_BE, offset, (short)val);
            offset += 2;
         }
      }
   ...
   private int serializeOctreeForPacket(@Nonnull MemorySegment data, int baseOffset, int position) {
      ...
      for (int i = 0; i < 8; i++) {
         ...
         if ((mask >> i & 1) == 1) {
            ...
            data.set(MemorySegmentUtil.SHORT_LE, offset, (short)val);
            offset += 2;
         }
      }
   ...
   public static ChunkLightData deserialize(@Nonnull MemorySegment data, int offset) {
      short changeId = data.get(MemorySegmentUtil.SHORT_BE, offset);
      boolean hasLight = data.get(ValueLayout.JAVA_BOOLEAN, offset + 2);
      ChunkLightData chunkLightData;
      if (hasLight) {
         int length = data.get(MemorySegmentUtil.INT_BE, offset + 2 + 1);
   ...
   private static long deserializeOctree(@Nonnull MemorySegment from, int baseOffset, @Nonnull MemorySegment to, int position, int segmentIndex) {
      int mask = from.get(ValueLayout.JAVA_BYTE, baseOffset);
      ...
      for (int i = 0; i < 8; i++) {
         ...
         if ((mask >> i & 1) == 1) {
            ...
            long result = deserializeOctree(from, offset, to, val, segmentIndex);
            offset += (int)(result >>> 32);
            segmentIndex = (int)result;
         } else {
            val = from.get(MemorySegmentUtil.SHORT_BE, offset);
            offset += 2;
         }
