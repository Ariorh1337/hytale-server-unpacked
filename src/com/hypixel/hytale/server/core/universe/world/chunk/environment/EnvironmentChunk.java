...
public class EnvironmentChunk implements Component<ChunkStore> {
   public static final BuilderCodec<EnvironmentChunk> CODEC = BuilderCodec.<EnvironmentChunk>builder(EnvironmentChunk.class, EnvironmentChunk::new)
   ...
   private byte[] serialize() {
      ...
      for (Int2LongMap.Entry entry : this.counts.int2LongEntrySet()) {
         ...
         String key = environment != null ? environment.getId() : Environment.UNKNOWN.getId();
         data.set(MemorySegmentUtil.INT_BE, offset, environmentId);
   ...
   private void deserialize(@Nonnull byte[] bytes) {
      ...
      for (int i = 0; i < mappingCount; i++) {
         int serialId = data.get(MemorySegmentUtil.INT_BE, offset);
         offset += 4;
   ...
   public static class BulkWriter {
      ...
      private static void transferCounts(@Nonnull Int2LongMap from, @Nonnull Int2LongMap into) {
         for (Int2LongMap.Entry entry : from.int2LongEntrySet()) {
