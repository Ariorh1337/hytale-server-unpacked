...
public interface IChunkStorageProvider<Data> {
   ...
   default <OtherData> Data migrateFrom(@Nonnull Store<ChunkStore> store, IChunkStorageProvider<OtherData> other) throws IOException {
      ...
      try (
         ...
         logger.atInfo().log("Migrating %d chunks", (int)chunks.size());
         ...
         while (iterator.hasNext()) {
            ...
            inFlight.add(
               oldLoader.loadHolder(chunkX, chunkZ)
                  ...
                  .exceptionally(t -> {
                     logger.atSevere().withCause(t).log("Failed to load chunk at %d, %d, skipping", (int)chunkX, (int)chunkZ);
                     return null;
                  })
            );
            compactHint[hintLen++] = chunk;
            if (++count % 100 == 0) {
               logger.atInfo().log("Migrated %d/%d chunks", (int)count, (int)chunks.size());
            ...
            if (hintLen == compactHint.length) {
               ...
               pendingCompacts.add(CompletableFuture.allOf(batchFutures).thenRun(() -> {
                  try {
                     ...
                     logger.atWarning().withCause(e).log("Compaction during migration failed; continuing");
                  }
               }));
         ...
         if (hintLen > 0) {
            ...
            pendingCompacts.add(CompletableFuture.runAsync(() -> {
               try {
                  ...
                  logger.atWarning().withCause(e).log("Final compaction during migration failed; continuing");
               }
            }));
         }

         CompletableFuture.allOf(pendingCompacts.toArray(CompletableFuture[]::new)).join();
         logger.atInfo().log("Finished migrating %d chunks", (int)chunks.size());
      } finally {
