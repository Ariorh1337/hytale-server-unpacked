...
public final class ShortSectionPalette extends AbstractSectionPalette {
   ...
   public static ShortSectionPalette fromBytePalette(@Nonnull ByteSectionPalette section) {
      ...
      for (Byte2IntMap.Entry entry : section.internalToExternal.byte2IntEntrySet()) {
         short internal = (short)(entry.getByteKey() & 0xFF);
      ...
      short[] newBlocks = new short[32768];

      for (int i = 0; i < 32768; i++) {
         newBlocks[i] = (short)(section.blocks[i] & 0xFF);
      }

      return new ShortSectionPalette(shortExternalToInternal, shortInternalToExternal, shortInternalIdSet, shortInternalIdCount, newBlocks);
   }
}
