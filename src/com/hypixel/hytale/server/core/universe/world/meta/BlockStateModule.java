   ...
   public static class LegacyLateInitBlockStateSystem<T extends BlockState> extends EntityTickingSystem<ChunkStore> implements DisableProcessingAssert {
      ...
      public void tick(
         ...
         try {
            ...
            LOGGER.at(Level.SEVERE).withCause(throwable).log("Exception while re-init BlockState! Removing!! %s", blockStateComponent);
            commandBuffer.removeEntity(archetypeChunk.getReferenceTo(index), RemoveReason.REMOVE);
         }
      }
   ...
   public static class LegacyTickingBlockStateSystem<T extends BlockState>
      ...
      public void tick(
         ...
         try {
            ...
            LOGGER.at(Level.SEVERE).withCause(throwable).log("Exception while ticking BlockState! Removing!! %s", blockState);
            commandBuffer.removeEntity(archetypeChunk.getReferenceTo(index), RemoveReason.REMOVE);
         }
      }
