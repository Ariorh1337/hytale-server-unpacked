               ...
               (aVoid, throwable) -> {
                  if (throwable != null) {
                     LOGGER.at(Level.SEVERE)
                        .withCause(throwable)
                        .log("Failed to save chunk (%d, %d):", (int)worldChunkComponent.getX(), (int)worldChunkComponent.getZ());
                  } else {
                     worldChunkComponent.setFlag(ChunkFlag.ON_DISK, true);
                     LOGGER.at(Level.FINEST).log("Finished saving chunk (%d, %d)", (int)worldChunkComponent.getX(), (int)worldChunkComponent.getZ());
                  }
               }
            );
