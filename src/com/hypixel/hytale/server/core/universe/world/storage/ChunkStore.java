         ...
         if ((flags & 3) == 3) {
            ...
            try {
               if (chunkState.reference != null) {
                  ...
                  return chunkState.future == null ? CompletableFuture.completedFuture(null) : chunkState.future;
               }
            } finally {
               chunkState.lock.unlockRead(stamp);
            }
            ...
            try {
               chunkState = this.chunkSections.get(x, y, z);
               if (chunkState == null) {
                  this.chunkSections.put(x, y, z, chunkState = new ChunkStore.ChunkLoadState());
               }
            } finally {
               this.lock.unlockWrite(sectionStamp);
            }
            ...
            try {
               ...
               ChunkStore.ChunkLoadState finalChunkState = chunkState;
               chunkState.future = this.getChunkReferenceAsync(ChunkUtil.indexChunk(x, z), flags).thenApply(v -> {
                  long stamp1 = finalChunkState.lock.readLock();

                  try {
                     return finalChunkState.reference;
                  } finally {
                     finalChunkState.lock.unlockRead(stamp1);
                  }
               });
               return chunkState.future;
            } finally {
            ...
            if ((isNew || (chunkState.flags & 1) != 0) && (flags & 1) == 0) {
               if (chunkState.future == null) {
                  chunkState.future = this.loader.loadHolder(x, z).thenApplyAsync(holder -> {
                     ...
                     LOGGER.at(Level.SEVERE).withCause(throwable).log("Failed to load chunk! %s, %s", (int)x, (int)z);
            ...
            if ((isNew || (chunkState.flags & 2) != 0) && (flags & 2) == 0) {
               ...
               if (chunkState.future == null) {
                  ...
                  chunkState.future = future.<Holder<ChunkStore>>thenApplyAsync(generatedChunk -> {
                     ...
                     LOGGER.at(Level.SEVERE).withCause(throwable).log("Failed to generate chunk! %s, %s", (int)x, (int)z);
                     chunkState.fail(throwable);
                     throw SneakyThrow.sneakyThrow(throwable);
                  });
               } else {
            ...
            if ((isNew || (chunkState.flags & 4) == 0) && (flags & 4) != 0) {
               ...
               if (chunkState.future != null) {
                  chunkState.future = chunkState.future.<Ref<ChunkStore>>thenApplyAsync(reference -> {
                     ...
                     LOGGER.at(Level.SEVERE).withCause(throwable).log("Failed to set chunk ticking! %s, %s", (int)x, (int)z);
                     chunkState.fail(throwable);
                     throw SneakyThrow.sneakyThrow(throwable);
                  });
               }
   ...
   public static class ChunkLoaderSaverSetupSystem extends StoreSystem<ChunkStore> {
      ...
      public void onSystemRemovedFromStore(@Nonnull Store<ChunkStore> store) {
         ...
         try {
            ...
            ChunkStore.LOGGER.at(Level.SEVERE).withCause(e).log("Failed to close storage!");
         }
      }
   }
