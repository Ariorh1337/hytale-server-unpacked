...
public class InteractionManager implements Component<EntityStore> {
   ...
   private boolean tryConsumePacketQueue(@Nonnull Ref<EntityStore> ref, @Nonnull Deque<SyncInteractionChain> packetQueue) {
      ...
      if (desynced && !packetQueue.isEmpty()) {
         HytaleLogger.Api ctx = LOGGER.at(Level.FINE);
         if (ctx.isEnabled()) {
            ctx.log("Discarding previous packets in queue: (before) %d", (int)packetQueue.size());
         }
         ...
         if (ctx.isEnabled()) {
            ctx.log("Discarded previous packets in queue: (after) %d", (int)packetQueue.size());
         }
      }
   ...
   private boolean syncStart(@Nonnull Ref<EntityStore> ref, @Nonnull SyncInteractionChain packet) {
      ...
      if (!packet.initial) {
         ...
         if (index <= 0) {
            HytaleLogger.Api ctx = LOGGER.at(Level.FINE);
            if (ctx.isEnabled()) {
               ctx.log("Invalid client chainId! Got %d but client id's should be > 0", (int)index);
            }
            ...
            HytaleLogger.Api ctx = LOGGER.at(Level.FINE);
            if (ctx.isEnabled()) {
               ctx.log("Invalid client chainId! The last clientChainId was %d but just got %d", (int)this.lastClientChainId, (int)index);
            }
            ...
            if (!UUIDUtil.isEmptyOrNull(proxyId)) {
               ...
               if (proxyTarget == null) {
                  if (this.packetQueueTime != 0L
                     && System.nanoTime() - this.packetQueueTime > TimeUnit.MILLISECONDS.toNanos(this.getOperationTimeoutThreshold()) / 2L) {
                     HytaleLogger.Api ctx = LOGGER.at(Level.FINE);
                     if (ctx.isEnabled()) {
                  ...
                  if (interaction != null && chainSyncStorage instanceof InteractionChain interactionChain) {
                     ...
                     if (context.isEnabled()) {
                        ...
                        context.log(
                           ...
                           packet.chainId,
                           index,
                           interaction.getTimeInSeconds(System.nanoTime()) * tickTimeDilation,
