            ...
            (k, v) -> {
               if (v.isInfinite() || !(v.getRemainingDuration() <= 0.0F)) {
                  ...
                  if (effect != null && effect.getStatModifiers() != null) {
                     for (Int2ObjectMap.Entry<StaticModifier[]> entry : effect.getStatModifiers().int2ObjectEntrySet()) {
   ...
   private static void applyEffectModifiers(
      ...
      for (int i = 0; i < statMap.size(); i++) {
         ...
         if (statModifiersForEntityStat == null) {
            ...
            for (Object2FloatMap.Entry<StaticModifier.CalculationType> entry : statModifiersForEntityStat.object2FloatEntrySet()) {
               StaticModifier.CalculationType calculationType = entry.getKey();
