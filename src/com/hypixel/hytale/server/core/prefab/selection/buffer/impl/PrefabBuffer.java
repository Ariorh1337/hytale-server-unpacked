...
public class PrefabBuffer {
   ...
   public static class Builder {
      ...
      private int buildColumn(Int2ObjectMap<PrefabBufferColumn> columns, PrefabBuffer.BuilderColumn column, MemorySegment data, int offset) {
         ...
         if (blockCount != 0 || column.entityHolders != null && column.entityHolders.length != 0) {
            ...
            if (blockCount > 0) {
               ...
               data.set(ValueLayout.JAVA_INT_UNALIGNED, writeOffset, initialY - 1);
               writeOffset += 4;
               ...
               for (int i = 0; i < blockCount; i++) {
                  ...
                  if (entry.rotation != 0) {
                     data.set(ValueLayout.JAVA_BYTE, writeOffset, (byte)entry.rotation);
                     writeOffset++;
                  }

                  if (entry.filler != 0) {
                     data.set(ValueLayout.JAVA_SHORT_UNALIGNED, writeOffset, (short)entry.filler);
                     writeOffset += 2;
                  }
      ...
      public PrefabBuffer build() {
         ...
         for (PrefabBuffer.BuilderColumn e : this.builderColumns.values()) {
         ...
         return new PrefabBuffer(this.anchor, this.min, this.max, columns, childPrefabArray);
      }
   }
   ...
   public static class PrefabBufferAccessor implements IPrefabBuffer {
      ...
      public int getMaxYAt(@Nonnull PrefabRotation rotation, int x, int z) {
         ...
         if (column.getBlockCount() > 0) {
            int offset = 0;
            MemorySegment data = column.getMemorySegment();
            int y = data.get(ValueLayout.JAVA_INT_UNALIGNED, offset);
            offset += 4;
            ...
            if (columnPredicate.test(xx, zx, blockCount, t)) {
               if (blockCount > 0) {
                  ...
                  for (int i = 0; i < blockCount; i++) {
                     ...
                     int rotation = 0;
                     if (PrefabBuffer.BlockMaskConstants.hasRotation(mask)) {
                        rotation = Byte.toUnsignedInt(data.get(ValueLayout.JAVA_BYTE, offset));
                        offset++;
                     }
                     ...
                     int filler = 0;
                     if (PrefabBuffer.BlockMaskConstants.hasFiller(mask)) {
                        filler = t.rotation.getFiller(Short.toUnsignedInt(data.get(ValueLayout.JAVA_SHORT_UNALIGNED, offset)));
                        offset += 2;
                     }
            ...
            if (columnPredicate.test(x, z, blockCount, t)) {
               if (blockCount > 0) {
                  ...
                  for (int i = 0; i < blockCount; i++) {
                     ...
                     float chance = 1.0F;
                     if (PrefabBuffer.BlockMaskConstants.hasChance(mask)) {
                        chance = data.get(ValueLayout.JAVA_FLOAT_UNALIGNED, offset);
                        offset += 4;
                     }
                     ...
                     int filler = 0;
                     if (PrefabBuffer.BlockMaskConstants.hasFiller(mask)) {
                        filler = Short.toUnsignedInt(data.get(ValueLayout.JAVA_SHORT_UNALIGNED, offset));
                        offset += 2;
                     }
      ...
      public <T> boolean forEachRaw(
         ...
         for (Int2ObjectMap.Entry<PrefabBufferColumn> entry : this.prefabBuffer.columns.int2ObjectEntrySet()) {
            int columnIndex = entry.getIntKey();
            int x = MathUtil.unpackLeft(columnIndex);
            int z = MathUtil.unpackRight(columnIndex);
            PrefabBufferColumn column = entry.getValue();
            ...
            while (columnIterator.hasNext()) {
               ...
               if (firstColumnBlockCount != 0 || secondColumnBlockCount != 0) {
                  ...
                  if (firstColumnBlockCount > 0) {
                     firstColumnY = firstData.get(ValueLayout.JAVA_INT_UNALIGNED, firstOffset);
                     firstOffset += 4;
                  } else {
                  ...
                  if (secondColumnBlockCount > 0) {
                     secondColumnY = secondData.get(ValueLayout.JAVA_INT_UNALIGNED, secondOffset);
                     secondOffset += 4;
                  } else {
                  ...
                  while (firstColumnBlocksRead < firstColumnBlockCount || secondColumnBlocksRead < secondColumnBlockCount) {
                     ...
                     if (firstColumnBlocksRead < firstColumnBlockCount) {
                        ...
                        if (PrefabBuffer.BlockMaskConstants.hasFiller(mask)) {
                           firstColumnFiller = t.rotation.getFiller(Short.toUnsignedInt(firstData.get(ValueLayout.JAVA_SHORT_UNALIGNED, firstOffset)));
                           firstOffset += 2;
                        } else {
                           firstColumnFiller = 0;
                        }
                     ...
                     if (secondColumnBlocksRead < secondColumnBlockCount) {
                        ...
                        if (PrefabBuffer.BlockMaskConstants.hasFiller(mask)) {
                           secondColumnFiller = t.rotation.getFiller(Short.toUnsignedInt(secondData.get(ValueLayout.JAVA_SHORT_UNALIGNED, secondOffset)));
                           secondOffset += 2;
                        } else {
                        ...
                        secondColumnComponents = PrefabBuffer.BlockMaskConstants.hasComponents(mask)
                           ...
                           : null;
                        secondOffset += PrefabBuffer.BlockMaskConstants.getFluidBytes(mask);
                     }
      ...
      public int getBlockId(int x, int y, int z) {
         ...
         MemorySegment data = column.getMemorySegment();
         int offset = 0;
         int blockY = data.get(ValueLayout.JAVA_INT_UNALIGNED, offset);
         offset += 4;
