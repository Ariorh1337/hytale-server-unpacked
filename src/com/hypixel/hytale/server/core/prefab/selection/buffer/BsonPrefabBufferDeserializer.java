   ...
   public PrefabBuffer deserialize(Path path, @Nonnull BsonDocument document) {
      ...
      if (fluidsValue != null) {
         ...
         for (BsonValue fluidValue : fluidsValue.asArray()) {
            ...
            PrefabBufferBlockEntry entry = column.computeIfAbsent(y, builder::newBlockEntry);
      ...
      columnMap.int2ObjectEntrySet().fastForEach(entryx -> {
         ...
         int zx = MathUtil.unpackRight(columnIndexx);
         ...
         builder.addColumn(xx, zx, entries, entityArray);
      });
      entityMap.int2ObjectEntrySet().fastForEach(entryx -> {
         ...
         if (entityArray != null) {
            builder.addColumn(xx, zx, PrefabBufferBlockEntry.EMPTY_ARRAY, entityArray);
         }
      });
   ...
   private static Int2ObjectOpenHashMap<List<Holder<EntityStore>>> deserializeEntityHolders(
      ...
      for (int size = entitiesArray.size(); i < size; i++) {
         ...
         try {
            ...
            throw new IllegalStateException("Failed to load entity wrapper #" + i + ": " + entityDocument, e);
         }
      }
