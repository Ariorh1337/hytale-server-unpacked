      ...
      public void tick(
         ...
         if (entity.getSpawnCount() > 0) {
                     ...
                     if (!entity.isDespawnStarted()) {
                        ...
                        for (int i = 0; i < tempStorageList.size(); i++) {
                           ...
                           if (modelComponent != null && modelComponent.getModel().getAnimationSetMap().containsKey("Despawn")) {
                              double despawnAnimationTime = npcComponent.getRole().getDespawnAnimationTime();
               ...
               if (storedFlock.hasStoredNPCs()) {
                  commandBuffer.run(_store -> {
                     ...
                     for (int bound = tempStorageList.size(); i < bound; i++) {
                        Ref<EntityStore> refx = tempStorageList.get(i);
            ...
            if (entity.tickSpawnLostTimeout(dt)) {
               ...
               if (refId != null) {
                  refId.increment();
                  SpawnMarkerSystems.LOGGER.at(Level.FINE).log("Marker lost spawned NPC and changed reference ID to %s", (int)refId.get());
               }
