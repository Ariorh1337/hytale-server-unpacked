...
public class WaterContainerJsonLoader extends JsonLoader<SeedStringResource, WaterContainer> {
   ...
   public WaterContainer load() {
      if (this.has("Block")) {
         ...
         String blockTypeKey = blockString;
         ...
         return new WaterContainer(
            new WaterContainer.Entry[]{
               new WaterContainer.Entry(
                  0, index, new DoubleRangeNoiseSupplier(DoubleRange.ZERO, ConstantNoiseProperty.DEFAULT_ZERO), height, DefaultCoordinateCondition.DEFAULT_TRUE
               )
            }
         );
      } else {
   ...
   private WaterContainer.Entry[] loadEntries() {
      ...
      for (int i = 0; i < arr.size(); i++) {
         try {
            entries[i] = new WaterContainerJsonLoader.WaterContainerEntryJsonLoader(this.seed.append(String.format("-%s", i)), this.dataFolder, arr.get(i))
               .load();
         } catch (Throwable e) {
