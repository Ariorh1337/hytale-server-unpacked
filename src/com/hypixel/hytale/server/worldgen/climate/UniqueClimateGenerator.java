...
public class UniqueClimateGenerator {
   ...
   public UniqueClimateGenerator apply(
      ...
      if (candidates.length != this.entries.length) {
         LogUtil.getLogger()
            .at(Level.WARNING)
            .log("Mismatched unique climate generator candidates: expected %d, got %d", (int)this.entries.length, (int)candidates.length);
         return this;
      }
   ...
   public UniqueClimateGenerator apply(int seed, @Nonnull ClimateNoise noise, @Nonnull ClimateGraph graph) {
      ...
      if (lookup.size() < unique.length) {
         LogUtil.getLogger().at(Level.WARNING).log("Could not resolve all unique climate zones, resolved %d out of %d", (int)lookup.size(), (int)unique.length);
