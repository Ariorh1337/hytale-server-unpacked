...
public class BuilderObjectReferenceHelper<T> extends BuilderObjectHelper<T> {
         ...
         if (this.modifier.exportedStateCount() != builder.getStateMappingHelper().importedStateCount()) {
            throw new SkipSentryException(
               new IllegalStateException(
                  String.format(
                     "Number of exported states does not match imported states in component %s", this.fileReference.get(builderSupport.getExecutionContext())
                  )
               )
            );
         }
   ...
   public boolean validate(
      ...
      if (builder == null) {
         ...
         errors.add(
            String.format("%s: %s is not a nullable component reference but a null component was passed", configName, this.fileReference.getExpressionString())
         );
         return false;
   ...
   private void validateComponentInterfaceMatch(String builderInterfaceCode) {
      ...
      throw new SkipSentryException(
         new IllegalStateException(
            String.format("Component code %s does not match any of slot codes: %s.", builderInterfaceCode, Arrays.toString(this.componentInterfaces))
         )
      );
   }
