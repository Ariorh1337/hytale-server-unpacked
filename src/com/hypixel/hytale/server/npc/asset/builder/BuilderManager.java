...
public class BuilderManager {
   ...
   public Object2IntMap<String> getNameToIndexMap() {
      ...
      try {
         ...
         while (iterator.hasNext()) {
            Object2IntMap.Entry<String> next = iterator.next();
         ...
         loadedBuilders.forEach(
            (index, builderInfo) -> {
               for (BuilderInfo info : this.builderCache.values()) {
                  ...
                  } catch (SkipSentryException | IllegalStateException | IllegalArgumentException e) {
                     NPCPlugin.get()
                        .getLogger()
                        .at(Level.WARNING)
                        .log("Could not check if builder %s was dependent: %s", this.lookupName(info.getIndex()), e.getMessage());
                     continue;
                  }
                     ...
                     for (BuilderInfo builderInfo : BuilderManager.this.builderCache.values()) {
                        try {
                           ...
                           NPCPlugin.get()
                              .getLogger()
                              .at(Level.WARNING)
                              .log("Could not check if builder %s was dependent: %s", BuilderManager.this.lookupName(builderInfo.getIndex()), e.getMessage());
                        }
                     }
