...
public class SensorCanPlace extends SensorBase {
   ...
   public boolean matches(@Nonnull Ref<EntityStore> ref, @Nonnull ExecutionSupport executionSupport, double dt, @Nonnull Store<EntityStore> store) {
      if (super.matches(ref, executionSupport, dt, store) && executionSupport.getWorldSupport().getBlockToPlace() != null) {
         ...
         if (boundingBoxComponent != null) {
            ...
            this.transform.normalize(magnitude);
            this.transform.add(xPositive ? 1.0 : 0.0, 0.0, zPositive ? 1.0 : 0.0);
         }

         this.offset.apply(this.transform);
         Vector3d position = transformComponent.getPosition();
         this.transform.add(position.x(), position.y(), position.z()).floor();
         int x = (int)this.transform.x();
         int y = (int)this.transform.y();
         int z = (int)this.transform.z();
         ...
         if (!((this.delay -= dt) > 0.0)) {
            ...
            this.cachedPosition.set(this.transform);
   ...
   public enum Direction implements Supplier<String> {
      Forward(Vector3dUtil.FORWARD),
      Backward(Vector3dUtil.BACKWARD),
      Left(Vector3dUtil.LEFT),
      Right(Vector3dUtil.RIGHT);

      private final Vector3dc direction;

      Direction(Vector3dc direction) {
         this.direction = direction;
      }

      @Nonnull
      public Vector3d apply(@Nonnull Vector3d target, float rotation) {
         return target.set(this.direction).rotateY(rotation);
      }
   ...
   public enum Offset implements Supplier<String> {
      HeadPosition(Vector3dUtil.UP),
      BodyPosition(Vector3dUtil.ZERO),
      FootPosition(Vector3dUtil.DOWN);

      private final Vector3dc offset;

      Offset(Vector3dc offset) {
         this.offset = offset;
      }
