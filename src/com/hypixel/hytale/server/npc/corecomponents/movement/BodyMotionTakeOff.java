...
public class BodyMotionTakeOff extends BodyMotionBase {
   ...
   public boolean computeSteering(
      ...
      if (!executionSupport.getMotionContextSupport().getActiveMotionController().matchesType(MotionControllerFly.class)) {
         TransformComponent transformComponent = componentAccessor.getComponent(ref, TransformComponent.getComponentType());
         assert transformComponent != null;
         NPCEntity npcComponent = componentAccessor.getComponent(ref, NPCEntity.getComponentType());
         assert npcComponent != null;
         executionSupport.getRole().setActiveMotionController(ref, npcComponent, "Fly", componentAccessor);
         Vector3d position = transformComponent.getPosition();
         position.y = transformComponent.getPosition().y() + 0.1;
         ((MotionControllerFly)executionSupport.getMotionContextSupport().getActiveMotionController()).takeOff(ref, this.jumpSpeed, componentAccessor);
      }
