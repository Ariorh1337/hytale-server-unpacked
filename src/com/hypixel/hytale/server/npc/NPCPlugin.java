...
public class NPCPlugin extends JavaPlugin {
   ...
   public Pair<Ref<EntityStore>, NPCEntity> spawnEntity(
      ...
      if (roleName == null) {
         get().getLogger().at(Level.WARNING).log("Unable to spawn entity with invalid role index: %s!", (int)roleIndex);
         return null;
      }
   ...
   public BuilderInfo prepareRoleBuilderInfo(int roleIndex) {
      try {
         ...
         throw new SkipSentryException(
            new RuntimeException(String.format("Cannot use role template '%s' (%s): %s", this.getName(roleIndex), roleIndex, e.getMessage()), e)
         );
      }
   }
