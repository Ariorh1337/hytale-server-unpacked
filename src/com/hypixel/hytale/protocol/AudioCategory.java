...
public class AudioCategory {
   ...
   public static AudioCategory deserialize(@Nonnull ByteBuf buf, int offset) {
      if (buf.readableBytes() - offset < 5) {
         throw ProtocolException.bufferTooSmall("AudioCategory", 5, buf.readableBytes() - offset);
      }
   ...
   public static AudioCategory toObject(MemorySegment mem, int offset) {
      if (offset + 5 > mem.byteSize()) {
         throw ProtocolException.bufferTooSmall("AudioCategory", offset + 5, (int)mem.byteSize());
      } else {
         return new AudioCategory(
            hasId(mem, offset) ? PacketIO.readVarString("Id", mem, offset + 5, 4096000, PacketIO.UTF8) : null, mem.get(PacketIO.PROTO_FLOAT, offset + 1)
         );
      }
   }
