...
public class KeyedCodec<T> {
   ...
   public Optional<T> get(@Nullable BsonDocument document, @Nonnull ExtraInfo extraInfo) {
      ...
      try {
         ...
         return Codec.isNullBsonValue(bsonValue) ? Optional.empty() : Optional.ofNullable(this.decode(bsonValue, extraInfo));
      } catch (Exception e) {
   ...
   public T getOrDefault(@Nullable BsonDocument document, @Nonnull ExtraInfo extraInfo, T def) {
      ...
      try {
         ...
         BsonValue bsonValue = document.get(this.key);
         return Codec.isNullBsonValue(bsonValue) ? def : this.codec.decode(bsonValue, extraInfo);
      } catch (Exception e) {
         throw new CodecException("Failed decode", document, extraInfo, e);
      } finally {
         extraInfo.popKey();
   ...
   public Optional<T> getAndInherit(@Nullable BsonDocument document, T parent, @Nonnull ExtraInfo extraInfo) {
      ...
      try {
         ...
         BsonValue bsonValue = document.get(this.key);
         return Codec.isNullBsonValue(bsonValue)
            ? Optional.ofNullable(this.decodeAndInherit(null, parent, extraInfo))
            : Optional.ofNullable(this.decodeAndInherit(bsonValue, parent, extraInfo));
      } catch (Exception e) {
         throw new CodecException("Failed decode", document, extraInfo, e);
      } finally {
         extraInfo.popKey();
