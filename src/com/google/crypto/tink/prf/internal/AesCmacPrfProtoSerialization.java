...
public final class AesCmacPrfProtoSerialization {
   ...
   private static final ParametersSerializer<AesCmacPrfParameters, ProtoParametersSerialization> PARAMETERS_SERIALIZER = ParametersSerializer.create(
      AesCmacPrfProtoSerialization::serializeParameters, AesCmacPrfParameters.class, ProtoParametersSerialization.class
   );
   private static final ParametersParser<ProtoParametersSerialization> PARAMETERS_PARSER = ParametersParser.create(
      AesCmacPrfProtoSerialization::parseParameters, TYPE_URL_BYTES, ProtoParametersSerialization.class
   );
   private static final KeySerializer<AesCmacPrfKey, ProtoKeySerialization> KEY_SERIALIZER = KeySerializer.create(
      AesCmacPrfProtoSerialization::serializeKey, AesCmacPrfKey.class, ProtoKeySerialization.class
   );
   ...
   private static ProtoParametersSerialization serializeParameters(AesCmacPrfParameters parameters) {
      return ProtoParametersSerialization.create(
         KeyTemplate.newBuilder()
            ...
            .setValue(AesCmacPrfKeyFormat.newBuilder().setKeySize(parameters.getKeySizeBytes()).build().toByteString())
   ...
   private static AesCmacPrfParameters parseParameters(ProtoParametersSerialization serialization) throws GeneralSecurityException {
      if (!serialization.getKeyTemplate().getTypeUrl().equals("type.googleapis.com/google.crypto.tink.AesCmacPrfKey")) {
         throw new IllegalArgumentException(
            "Wrong type URL in call to AesCmacPrfProtoSerialization.parseParameters: " + serialization.getKeyTemplate().getTypeUrl()
         );
      }
   ...
   private static AesCmacPrfKey parseKey(ProtoKeySerialization serialization, @Nullable SecretKeyAccess access) throws GeneralSecurityException {
      ...
      try {
         com.google.crypto.tink.proto.AesCmacPrfKey protoKey = com.google.crypto.tink.proto.AesCmacPrfKey.parseFrom(
