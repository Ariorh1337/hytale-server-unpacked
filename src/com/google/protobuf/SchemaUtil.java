...
final class SchemaUtil {
   ...
   static int computeSizeInt64List(int fieldNumber, List<Long> list, boolean packed) {
      ...
      return packed
         ? CodedOutputStream.computeTagSize(fieldNumber) + CodedOutputStream.computeLengthDelimitedFieldSize(size)
   ...
   static int computeSizeBoolList(int fieldNumber, List<?> list, boolean packed) {
      ...
      if (length == 0) {
         ...
         return packed
            ? CodedOutputStream.computeTagSize(fieldNumber) + CodedOutputStream.computeLengthDelimitedFieldSize(length)
            : length * CodedOutputStream.computeBoolSize(fieldNumber, true);
      }
   }
   ...
   static int computeSizeMessage(int fieldNumber, Object value, Schema<?> schema) {
      return value instanceof LazyFieldLite
         ? ((LazyFieldLite)value).computeSize(fieldNumber)
         : computeMessageSize(fieldNumber, (AbstractMessageLite)value, schema);
   }

   static int computeMessageSize(final int fieldNumber, final AbstractMessageLite value, final Schema schema) {
   ...
   public static boolean shouldUseTableSwitch(int lo, int hi, int numFields) {
      ...
      long tableSpaceCost = (long)hi - lo + 1L;
