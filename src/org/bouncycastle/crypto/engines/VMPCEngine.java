...
public class VMPCEngine implements StreamCipher {
   ...
   public void init(boolean var1, CipherParameters var2) {
      ...
      if (this.workingIV != null && this.workingIV.length >= 1 && this.workingIV.length <= 768) {
         ...
         CryptoServicesRegistrar.checkConstraints(
            new DefaultServiceProperties(this.getAlgorithmName(), this.workingKey.length >= 32 ? 256 : this.workingKey.length * 8, var2, Utils.getPurpose(var1))
         );
      } else {
   ...
   public int processBytes(byte[] var1, int var2, int var3, byte[] var4, int var5) {
      ...
      for (int var6 = 0; var6 < var3; var6++) {
         ...
         byte var7 = this.P[this.P[this.P[this.s & 0xFF] & 0xFF] + 1 & 0xFF];
         byte var8 = this.P[this.n & 0xFF];
   ...
   public byte returnByte(byte var1) {
      this.s = this.P[this.s + this.P[this.n & 0xFF] & 0xFF];
      byte var2 = this.P[this.P[this.P[this.s & 0xFF] & 0xFF] + 1 & 0xFF];
