...
public class DVCSRequestInfo {
   ...
   public static boolean validate(DVCSRequestInfo var0, DVCSRequestInfo var1) {
      ...
      if (!clientEqualsServer(var2.getService(), var3.getService())) {
         return false;
      }

      if (!clientEqualsServer(var2.getRequestTime(), var3.getRequestTime())) {
         return false;
      }

      if (!clientEqualsServer(var2.getRequestPolicy(), var3.getRequestPolicy())) {
         return false;
      }

      if (!clientEqualsServer(var2.getExtensions(), var3.getExtensions())) {
         return false;
      }
      ...
      if (var2.getNonce() != null) {
         ...
         if (!Arrays.areEqual(var4, Arrays.copyOfRange(var5, 0, var4.length))) {
            return false;
         }
      }
