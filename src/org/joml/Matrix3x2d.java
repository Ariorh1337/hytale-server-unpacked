...
public class Matrix3x2d implements Matrix3x2dc, Cloneable, Externalizable {
   ...
   public boolean testAar(double minX, double minY, double maxX, double maxY) {
      ...
      double pyY = -this.m11;
      double pyW = 1.0 - this.m21;
      return nxX * (nxX < 0.0 ? minX : maxX) + nxY * (nxY < 0.0 ? minY : maxY) >= -nxW
         && pxX * (pxX < 0.0 ? minX : maxX) + pxY * (pxY < 0.0 ? minY : maxY) >= -pxW
         && nyX * (nyX < 0.0 ? minX : maxX) + nyY * (nyY < 0.0 ? minY : maxY) >= -nyW
         && pyX * (pyX < 0.0 ? minX : maxX) + pyY * (pyY < 0.0 ? minY : maxY) >= -pyW;
   }
   ...
   public boolean equals(Object obj) {
      if (this == obj) {
         ...
         return false;
      } else if (!(obj instanceof Matrix3x2d)) {
         return false;
      } else {
         ...
         if (Double.doubleToLongBits(this.m00) != Double.doubleToLongBits(other.m00)) {
            ...
            return Double.doubleToLongBits(this.m20) != Double.doubleToLongBits(other.m20)
               ? false
               : Double.doubleToLongBits(this.m21) == Double.doubleToLongBits(other.m21);
         }
      }
   }
