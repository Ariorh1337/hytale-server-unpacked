...
public class Matrix3x2f implements Matrix3x2fc, Externalizable, Cloneable {
   ...
   public boolean testAar(float minX, float minY, float maxX, float maxY) {
      ...
      float pyY = -this.m11;
      float pyW = 1.0F - this.m21;
      return nxX * (nxX < 0.0F ? minX : maxX) + nxY * (nxY < 0.0F ? minY : maxY) >= -nxW
         && pxX * (pxX < 0.0F ? minX : maxX) + pxY * (pxY < 0.0F ? minY : maxY) >= -pxW
         && nyX * (nyX < 0.0F ? minX : maxX) + nyY * (nyY < 0.0F ? minY : maxY) >= -nyW
         && pyX * (pyX < 0.0F ? minX : maxX) + pyY * (pyY < 0.0F ? minY : maxY) >= -pyW;
   }
   ...
   public int hashCode() {
      ...
      return 31 * result + Float.floatToIntBits(this.m21);
   }
   ...
   public boolean equals(Object obj) {
      if (this == obj) {
         ...
         return false;
      } else if (!(obj instanceof Matrix3x2f)) {
         return false;
      } else {
         ...
         if (Float.floatToIntBits(this.m00) != Float.floatToIntBits(other.m00)) {
            ...
            return Float.floatToIntBits(this.m20) != Float.floatToIntBits(other.m20)
               ? false
               : Float.floatToIntBits(this.m21) == Float.floatToIntBits(other.m21);
         }
      }
   }
   ...
   public boolean equals(Matrix3x2fc m, float delta) {
      if (this == m) {
         ...
         return !Runtime.equals(this.m20, m.m20(), delta) ? false : Runtime.equals(this.m21, m.m21(), delta);
      }
   }
