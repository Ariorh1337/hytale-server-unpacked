...
public class Vector3i implements Externalizable, Cloneable, Vector3ic {
   ...
   public double length() {
      return Math.sqrt((float)((long)this.x * this.x + (long)this.y * this.y + (long)this.z * this.z));
   }

   public static double length(int x, int y, int z) {
      return Math.sqrt((float)((long)x * x + (long)y * y + (long)z * z));
   }
   ...
   public double distance(Vector3ic v) {
      ...
      int dy = this.y - v.y();
      int dz = this.z - v.z();
      return Math.sqrt((float)((long)dx * dx + (long)dy * dy + (long)dz * dz));
   }
