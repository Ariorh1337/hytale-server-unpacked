...
public class Quaterniond implements Externalizable, Cloneable, Quaterniondc {
   ...
   public Vector3d transformUnit(double x, double y, double z, Vector3d dest) {
      ...
      double zw = this.z * this.w;
      return dest.set(
         Math.fma(Math.fma(-2.0, yy + zz, 1.0), x, Math.fma(2.0 * (xy - zw), y, 2.0 * (xz + yw) * z)),
         Math.fma(2.0 * (xy + zw), x, Math.fma(Math.fma(-2.0, xx + zz, 1.0), y, 2.0 * (yz - xw) * z)),
   ...
   public Vector4f transform(double x, double y, double z, Vector4f dest) {
      ...
      double k = 1.0 / (xx + yy + zz + ww);
      return dest.set(
         (float)Math.fma((xx - yy - zz + ww) * k, x, Math.fma(2.0 * (xy - zw) * k, y, 2.0 * (xz + yw) * k * z)),
         (float)Math.fma(2.0 * (xy + zw) * k, x, Math.fma((yy - xx - zz + ww) * k, y, 2.0 * (yz - xw) * k * z)),
         (float)Math.fma(2.0 * (xz - yw) * k, x, Math.fma(2.0 * (yz + xw) * k, y, (zz - xx - yy + ww) * k * z)),
         dest.w
      );
   }
   ...
   public Vector3f transformUnit(double x, double y, double z, Vector3f dest) {
      ...
      double zw = this.z * this.w;
      return dest.set(
         (float)Math.fma(Math.fma(-2.0, yy + zz, 1.0), x, Math.fma(2.0 * (xy - zw), y, 2.0 * (xz + yw) * z)),
         (float)Math.fma(2.0 * (xy + zw), x, Math.fma(Math.fma(-2.0, xx + zz, 1.0), y, 2.0 * (yz - xw) * z)),
         (float)Math.fma(2.0 * (xz - yw), x, Math.fma(2.0 * (yz + xw), y, Math.fma(-2.0, xx + yy, 1.0) * z))
      );
   }
   ...
   public boolean equals(Quaterniondc q, double delta) {
      if (this == q) {
         ...
         return false;
      } else if (this.getClass() != q.getClass()) {
         return false;
         ...
         return !Runtime.equals(this.z, q.z(), delta) ? false : Runtime.equals(this.w, q.w(), delta);
      }
   }
