...
extends WorldStructureAsset {
    @Nonnull
    public static final BuilderCodec<BasicWorldStructureAsset> CODEC = ((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)BuilderCodec.builder(BasicWorldStructureAsset.class, BasicWorldStructureAsset::new, WorldStructureAsset.ABSTRACT_CODEC).append(new KeyedCodec<T[]>("Biomes", new ArrayCodec(BiomeRangeAsset.CODEC, BiomeRangeAsset[]::new), true), (asset, value) -> {
        asset.biomeRangeAssets = value;
    }, asset -> asset.biomeRangeAssets).add()).append(new KeyedCodec("Density", DensityAsset.CODEC, true), (asset, value) -> {
        asset.densityAsset = value;
    }, asset -> asset.densityAsset).add()).append(new KeyedCodec("DefaultBiome", new ContainedAssetCodec(BiomeAsset.class, BiomeAsset.CODEC), true), (asset, value) -> {
        asset.defaultBiomeId = value;
    }, asset -> asset.defaultBiomeId).addValidatorLate(() -> BiomeAsset.VALIDATOR_CACHE.getValidator().late()).add()).append(new KeyedCodec<Integer>("DefaultTransitionDistance", Codec.INTEGER, true), (asset, value) -> {
        asset.biomeTransitionDistance = value;
    }, asset -> asset.biomeTransitionDistance).addValidator(Validators.greaterThan(0)).add()).append(new KeyedCodec<Integer>("MaxBiomeEdgeDistance", Codec.INTEGER, true), (asset, value) -> {
        asset.maxBiomeEdgeDistance = value;
    }, asset -> asset.maxBiomeEdgeDistance).addValidator(Validators.greaterThanOrEqual(0)).add()).append(new KeyedCodec<T[]>("Framework", new ArrayCodec(FrameworkAsset.CODEC, FrameworkAsset[]::new), false), (asset, value) -> {
        asset.frameworkAssets = value;
    }, asset -> asset.frameworkAssets).add()).append(new KeyedCodec("SpawnPositions", PositionProviderAsset.CODEC, false), (asset, value) -> {
        asset.spawnPositionsAsset = value;
    }, asset -> asset.spawnPositionsAsset).add()).build();
    ...
    public WorldStructure build(@Nonnull WorldStructureAsset.Argument argument) {
        ...
        HashMap<BiomeAsset, Biome> biomeAssetToBiomeType = new HashMap<BiomeAsset, Biome>();
