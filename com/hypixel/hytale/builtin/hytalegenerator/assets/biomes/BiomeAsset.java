...
implements JsonAssetWithMap<String, DefaultAssetMap<String, BiomeAsset>>,
Cleanable {
    @Nonnull
    ...
    public void cleanUp() {
        ...
        for (Cleanable cleanable : this.floatingFunctionNodeAssets) {
            ((DensityAsset)cleanable).cleanUp();
        }
    }
    ...
    static {
        CODEC = ((AssetBuilderCodec.Builder)((AssetBuilderCodec.Builder)((AssetBuilderCodec.Builder)((AssetBuilderCodec.Builder)((AssetBuilderCodec.Builder)((AssetBuilderCodec.Builder)((AssetBuilderCodec.Builder)AssetBuilderCodec.builder(BiomeAsset.class, BiomeAsset::new, Codec.STRING, (asset, id) -> {
            asset.id = id;
        }, config -> config.id, (config, data) -> {
            config.data = data;
        }, config -> config.data).append(new KeyedCodec("Terrain", TerrainAsset.CODEC, true), (asset, t) -> {
            asset.terrainAsset = t;
        }, asset -> asset.terrainAsset).add()).append(new KeyedCodec<T[]>("FloatingFunctionNodes", new ArrayCodec(DensityAsset.CODEC, DensityAsset[]::new), true), (asset, t) -> {
            asset.floatingFunctionNodeAssets = t;
        }, asset -> asset.floatingFunctionNodeAssets).add()).append(new KeyedCodec<String>("Name", Codec.STRING, true), (asset, t) -> {
            asset.biomeName = t;
        }, asset -> asset.biomeName).add()).append(new KeyedCodec("MaterialProvider", MaterialProviderAsset.CODEC, true), (asset, materialProvider) -> {
            asset.materialProviderAsset = materialProvider;
        }, asset -> asset.materialProviderAsset).add()).append(new KeyedCodec<T[]>("Props", new ArrayCodec(PropRuntimeAsset.CODEC, PropRuntimeAsset[]::new), true), (asset, materialProvider) -> {
            asset.propRuntimeAssets = materialProvider;
        }, asset -> asset.propRuntimeAssets).add()).append(new KeyedCodec("EnvironmentProvider", EnvironmentProviderAsset.CODEC, true), (asset, environmentProvider) -> {
            asset.environmentProviderAsset = environmentProvider;
        }, asset -> asset.environmentProviderAsset).add()).append(new KeyedCodec("TintProvider", TintProviderAsset.CODEC, true), (asset, tintProvider) -> {
            asset.tintProviderAsset = tintProvider;
        }, asset -> asset.tintProviderAsset).add()).build();
    }
}
