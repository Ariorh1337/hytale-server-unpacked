...
extends PropAsset {
    ...
    }).add()).append(new KeyedCodec("PlacementMask", BlockMaskAsset.CODEC, true), (asset, v) -> {
        asset.placementMaskAsset = v;
    }, asset -> asset.placementMaskAsset).add()).append(new KeyedCodec("Pattern", PatternAsset.CODEC, true), (asset, v) -> {
    ...
    }, asset -> asset.scannerAsset).add()).append(new KeyedCodec("Density", DensityAsset.CODEC, true), (asset, v) -> {
        asset.densityAsset = v;
    }, asset -> asset.densityAsset).add()).append(new KeyedCodec("Material", MaterialProviderAsset.CODEC, true), (asset, v) -> {
        asset.materialProviderAsset = v;
    }, asset -> asset.materialProviderAsset).add()).build();
    ...
    public Prop build(@Nonnull PropAsset.Argument argument) {
        ...
        if (this.placementMaskAsset == null) {
            return Prop.noProp();
        }
        BlockMask placementMask = this.placementMaskAsset.build(argument.materialCache);
        if (this.scannerAsset == null || this.patternAsset == null || this.densityAsset == null || this.materialProviderAsset == null) {
            return Prop.noProp();
        }
        return new DensityProp(this.range, this.densityAsset.build(DensityAsset.from(argument)), this.materialProviderAsset.build(MaterialProviderAsset.argumentFrom(argument)), this.scannerAsset.build(ScannerAsset.argumentFrom(argument)), this.patternAsset.build(PatternAsset.argumentFrom(argument)), placementMask, new Material(argument.materialCache.EMPTY_AIR, argument.materialCache.EMPTY_FLUID));
    }
