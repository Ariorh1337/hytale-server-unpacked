    ...
    implements RunWhenPausedSystem<ChunkStore> {
        ...
        public void tick(float dt, int index, @Nonnull ArchetypeChunk<ChunkStore> archetypeChunk, @Nonnull Store<ChunkStore> store, @Nonnull CommandBuffer<ChunkStore> commandBuffer) {
            ...
            if (changes.size() == 1) {
                ...
                ServerSetFluid packet2 = new ServerSetFluid(x, y, z, fluid, level);
                for (PlayerRef playerRef : playerRefs) {
                    ChunkTracker tracker;
                    Ref<EntityStore> ref = playerRef.getReference();
                    if (ref == null || !ref.isValid() || !(tracker = playerRef.getChunkTracker()).isLoaded(chunkIndex)) continue;
                    playerRef.getPacketHandler().writeNoCache(packet2);
                }
            } else {
                ...
                ServerSetFluids packet3 = new ServerSetFluids(fluidSectionComponent.getX(), fluidSectionComponent.getY(), fluidSectionComponent.getZ(), cmds);
                for (PlayerRef playerRef : playerRefs) {
                    ChunkTracker tracker;
                    Ref<EntityStore> ref = playerRef.getReference();
                    if (ref == null || !ref.isValid() || !(tracker = playerRef.getChunkTracker()).isLoaded(chunkIndex)) continue;
                    playerRef.getPacketHandler().writeNoCache(packet3);
                }
            }
            changes.clear();
        }
