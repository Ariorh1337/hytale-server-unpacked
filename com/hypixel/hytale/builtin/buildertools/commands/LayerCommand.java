...
extends AbstractPlayerCommand {
    private static Map<String, Vector3i> directions = Map.of("up", Vector3i.UP, "down", Vector3i.DOWN, "north", Vector3i.NORTH, "south", Vector3i.SOUTH, "east", Vector3i.EAST, "west", Vector3i.WEST);
    @Nonnull
    ...
    @Nonnull
    private final RequiredArg<List<Pair<Integer, String>>> layersArg = this.withListRequiredArg("layers", "server.commands.layer.layers.desc", ArgTypes.LAYER_ENTRY_TYPE);
    ...
    protected void execute(@Nonnull CommandContext context, @Nonnull Store<EntityStore> store, @Nonnull Ref<EntityStore> ref, @Nonnull PlayerRef playerRef, @Nonnull World world) {
        ...
        List layers = (List)this.layersArg.get(context);
        ...
        if (directionValid) {
            BuilderToolsPlugin.addToQueue(playerComponent, playerRef, (r, s, componentAccessor) -> {
                ...
                layerDirection = direction.equalsIgnoreCase("camera") ? headRotationComponent.getAxisDirection() : directions.get(direction);
                s.layer(layers, layerDirection, (ComponentAccessor<EntityStore>)componentAccessor);
            });
        } else {
