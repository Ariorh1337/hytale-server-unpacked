...
extends DelayedEntitySystem<EntityStore> {
    ...
    public void tick(float dt, int index, @Nonnull ArchetypeChunk<EntityStore> archetypeChunk, @Nonnull Store<EntityStore> store, @Nonnull CommandBuffer<EntityStore> commandBuffer) {
        ...
        if (this.findPortalSpawnPos == null) {
            SpatialHashGrid<Ref<EntityStore>> spawners = VoidInvasionPortalsSpawnSystem.cleanupAndGetSpawners(voidEventComponent);
            if (spawners.size() >= 24) {
                return;
            }
            this.findPortalSpawnPos = VoidInvasionPortalsSpawnSystem.findPortalSpawnPosition(world, voidEventComponent, commandBuffer);
            return;
        }
    ...
    private static CompletableFuture<Vector3d> findPortalSpawnPosition(@Nonnull World world, @Nonnull VoidEvent voidEvent, @Nonnull CommandBuffer<EntityStore> commandBuffer) {
        ...
        Transform playerTransform = VoidInvasionPortalsSpawnSystem.findRandomPlayerTransform(world, commandBuffer);
        ...
        return CompletableFuture.supplyAsync(() -> new SearchCone(direction, 48.0, 64.0, 90.0, 8).filter(noNearbySpawners).filter(new NotNearPointXZ(spawnPos, 18.0)).then(new SearchBelow(12)).filter(new FitsAPortal()).execute(world, originPosition).orElse(null), world);
    }
