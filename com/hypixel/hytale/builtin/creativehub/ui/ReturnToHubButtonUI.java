...
public final class ReturnToHubButtonUI {
    ...
    public static void register() {
        AnchorActionModule.get().register(ACTION_RETURN_TO_HUB, (playerRef, ref, store, data) -> ReturnToHubButtonUI.executeReturnToHub(playerRef, ref, store));
    }

    public static void send(@Nonnull PlayerRef playerRef) {
        ReturnToHubButtonUI.send(playerRef, false);
    }
    ...
    public static void send(@Nonnull PlayerRef playerRef, boolean disabled) {
        ...
        if (!disabled) {
            eventBuilder.addEventBinding(CustomUIEventBindingType.Activating, "#ReturnToHubButton", EventData.of("action", ACTION_RETURN_TO_HUB), false);
        }
    ...
    public static void clear(@Nonnull PlayerRef playerRef) {
        playerRef.getPacketHandler().writeNoCache(new UpdateAnchorUI(ANCHOR_ID, true, null, null));
    }
    ...
    public static void executeReturnToHub(@Nonnull PlayerRef playerRef, @Nonnull Ref<EntityStore> ref, @Nonnull Store<EntityStore> store) {
        World currentHub;
        World parentWorld = ReturnToHubButtonUI.findParentHubWorld(store, ref);
        if (parentWorld == null) {
            return;
        }
        ...
        if (world.equals(currentHub = CreativeHubPlugin.get().getActiveHubInstance(parentWorld))) {
            return;
        }
    ...
    private static World findParentHubWorld(@Nonnull Store<EntityStore> store, @Nonnull Ref<EntityStore> ref) {
        ...
        if (hubEntityConfig != null && hubEntityConfig.getParentHubWorldUuid() != null && (parentWorld = Universe.get().getWorld(hubEntityConfig.getParentHubWorldUuid())) != null && (hubConfig = CreativeHubWorldConfig.get(parentWorld.getWorldConfig())) != null && hubConfig.getStartupInstance() != null) {
            return parentWorld;
        }
        return null;
    }
