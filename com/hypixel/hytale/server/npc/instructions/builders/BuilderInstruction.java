...
extends BuilderBase<Instruction> {
    ...
    public Builder<Instruction> readConfig(@Nonnull JsonElement data) {
        ...
        this.getBoolean(data, "ActionsBlocking", (boolean b) -> {
            this.actionsBlocking = b;
        }, false, BuilderDescriptorState.Stable, "Do not execute an action unless the previous action could execute", null);
        this.getBoolean(data, "ActionsAtomic", (boolean b) -> {
            this.actionsAtomic = b;
        }, false, BuilderDescriptorState.Stable, "Only execute actions if all actions can be executed", null);
    ...
    public Instruction[] getSteps(@Nonnull BuilderSupport support) {
        Object stepList = this.steps.build(support);
        return stepList == null ? Instruction.EMPTY_ARRAY : (Instruction[])stepList.toArray(Instruction[]::new);
    }
