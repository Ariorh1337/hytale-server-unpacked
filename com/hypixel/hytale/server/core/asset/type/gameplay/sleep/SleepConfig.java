...
public class SleepConfig {
    public static final BuilderCodec<SleepConfig> CODEC = ((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)BuilderCodec.builder(SleepConfig.class, SleepConfig::new).append(new KeyedCodec<Float>("WakeUpHour", Codec.FLOAT), (sleepConfig, i) -> {
    ...
    }, o -> o.allowedSleepHoursRange).addValidator(Validators.doubleArraySize(2)).documentation("The in-game hours during which players can sleep to skip to the WakeUpHour. If missing, there is no restriction.").add()).append(new KeyedCodec<SleepSoundsConfig>("Sounds", SleepSoundsConfig.CODEC), (sleepConfig, i) -> {
        sleepConfig.sounds = i;
    }, sleepConfig -> sleepConfig.sounds).add()).build();
